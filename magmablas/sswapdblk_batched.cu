/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zswapdblk_batched.cu normal z -> s, Fri Sep 11 18:29:22 2015
...
__global__ void 
sswapdblk_batched_kernel( int nb, int n_mod_nb,
                  float **dA_array, int ldda, int inca,
                  float **dB_array, int lddb, int incb )
{
...
/**
    ...
    dA_array Array of pointers, dimension (batchCount). 
             Each is a REAL array dA, dimension (ldda,n)
             The matrix dA.
    ...
    dB_array Array of pointers, dimension (batchCount).
             Each is a REAL array dB, dimension (lddb,n)
             The matrix dB.
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    if ( nblocks > 0 ) {
        sswapdblk_batched_kernel<<< dimGrid, dimBlock, 0, queue >>>
...
{
    magmablas_sswapdblk_batched_q( n, nb, dA_array, ldda, inca, dB_array, lddb, incb, batchCount, magma_stream );
}
