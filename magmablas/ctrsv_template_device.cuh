/*
    -- MAGMA (version 2.0.0-beta2) --
       ...
       @generated from magmablas/ztrsv_template_device.cuh normal z -> c, Wed Jan  6 17:59:51 2016
*/
    ...
    */
    ...
    for (int step=0; step < n; step++)
    {
        if (tx < n)
        {
            ...
            {
                a = MAGMA_C_CNJG(A[ (n-1) + (n-1) * lda - tx * lda  - step]); // columwise access data, not in a coalesced way
            }
    ...
    */
    ...
    for (int step=0; step < n; step++)
    {
        if (tx < n) // hard code to BLOCK_SIZE and test divisible case only make 1Gflop/s difference
        {
            ...
            {
                a = MAGMA_C_CNJG(A[ tx * lda  + step]); // columwise access data, not in a coalesced way
            }
