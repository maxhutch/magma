                                ...
                                magmaDoubleComplex        *dB, magma_int_t lddb, magma_int_t nb )
{
    ...
    for(i=nb; i < n; i += nb) {
       ...
       magmablas_ztranspose_stream( m, nb, dB+(j%2)*nb*lddb, lddb, dat+i-nb, ldda, stream[j%2] );
    }

    /* Transpose the last part of the matrix.                            */
    j++;
    magmablas_ztranspose_stream( m, ib, dB+(j%2)*nb*lddb, lddb, dat+i-nb, ldda, stream[j%2] );
