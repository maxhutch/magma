/*
    -- MAGMA (version 1.5.0-beta3) --
       ...
       @generated from zlaset.cu normal z -> s, Fri Jul 18 17:34:12 2014
...
/**
    Purpose
    -------
    SLASET_STREAM initializes a 2-D array A to DIAG on the diagonal and
    ...
    magma_queue_t stream)
{
    ...
    if (uplo == MagmaLower)
        slaset_lower<<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
    else if (uplo == MagmaUpper)
        slaset_upper<<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
    else
        slaset_full <<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
}


/**
    @see magmablas_slaset_stream
    @ingroup magma_saux2
    ********************************************************************/
extern "C"
void magmablas_slaset(
    ...
    float *dA, magma_int_t ldda )
{
    magmablas_slaset_stream( uplo, m, n, offdiag, diag, dA, ldda, magma_stream );
}
...
/**
    ...
    ********************************************************************/
extern "C" void
magmablas_slaset_band_stream(
    magma_uplo_t uplo, magma_int_t m, magma_int_t n, magma_int_t k,
    float offdiag, float diag,
    float *dA, magma_int_t ldda, magma_queue_t stream)
{
    magma_int_t info = 0;
    ...
    if (uplo == MagmaUpper) {
        ...
        slaset_band_upper<<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
}
    else if (uplo == MagmaLower) {
        dim3 threads( min(k,m) );
        dim3 grid( (min(m,n) - 1)/LASET_BAND_NB + 1 );
        slaset_band_lower<<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
    }
}


/**
    @see magmablas_slaset_band_stream
    @ingroup magma_saux2
    ********************************************************************/
extern "C" void
magmablas_slaset_band(
    ...
    float *dA, magma_int_t ldda)
{
    magmablas_slaset_band_stream(uplo, m, n, k, offdiag, diag, dA, ldda, magma_stream);
}
