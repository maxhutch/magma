/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated s Tue Aug 13 16:45:22 2013
...
__global__ void
magmablas_ssymv_200_L_special_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float  beta,
    ...
    int kstan)
{
    ...
    float res  = MAGMA_S_ZERO; // used in scan the row
    float res_ = MAGMA_S_ZERO; // used in scan the column
    float res1 = MAGMA_S_ZERO; // tem for res
    float res2 = MAGMA_S_ZERO; // tem for res_

    __shared__ float la   [ssymv_bs][bank_shift];
    __shared__ float sdata   [ssymv_bs][9];
    __shared__ float buff [ssymv_bs];
    __shared__ float buff2 [ssymv_bs];

    int break_d   =  ssymv_bs * blkc;
    ...
    {
        A += lda * (blkc/num_gpus) * ssymv_bs; // change

        #pragma unroll
        for(int j =0; j < ssymv_bs; j += 8)
        ...
        for(int  i=ty*4; i < (ty * 4 + 4); i++) {
            if ( i < tx ) {
                la[0][bank_shift * tx + i] = ( la[0][ i * bank_shift + tx] );
            }
        }
        __syncthreads();
        ...
        #pragma unroll
        for(int j=0; j < 4; j++)
            res += ( la[0][bank_shift * tx + j + ty * 4] ) * buff[j + ty * 4];
        __syncthreads();

        A -= lda * (blkc/num_gpus) * ssymv_bs;
            
        flag = 1;
    }

    x -= blkc * ssymv_bs  *incx;
    ...
    if( blkc > my_gpu_id) {
        for(int s=0; s < num_blocks_iters; s++)
        {
            ...
            for(int j=0; j < 4; j++)
            {
                ...
                res_ += ( la[0][bank_shift * tx + j + ty * 4] ) * buff[j + ty * 4]; //iterate colum
            }
 ...
 */
__global__ void
magmablas_ssymv_200_L_generic_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float beta,
    ...
    if ( (blkc % num_gpus) == my_gpu_id)
    {
        ...
        if( blkc == ( gridDim.x - 1 ) ) {
            ...
            for(int j =0; j < ssymv_bs; j += 8) {
                if( ( ty + j ) > m_mod_nb )
                {
                    MAGMA_S_SET2REAL(la[0][bank_shift*(ty+j)+tx], 9999);
                }
                else
                    la[0][bank_shift*(ty+j)+tx] =  A[ j * lda];
            }
...
__global__ void
magmablas_ssymv_200_L_update_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float beta,
    ...
    int kstan )
{
    int i;
    int tx  = threadIdx.x;
    int ind = blockIdx.x * ssymv_bs + tx;
    float Ca;

    MAGMA_S_SET2REAL(Ca, 0);
    WC += ind + lda * blockIdx.x;

    for(i = blockIdx.x* ssymv_bs; i < n; i += ssymv_bs) {
        Ca += WC[0];
        WC += ssymv_bs;
    }
    if( ind < n && ind >= kstan)
...
extern "C"
void magmablas_ssymv_200_L_mgpu_offset_32(
    magma_int_t m, float alpha,
    ...
    magma_int_t num_blocks_skipped)
{
    ...
    if(m % ssymv_bs == 0 )
    {
        magmablas_ssymv_200_L_special_mgpu_offset_32 <<< grid, threads, 0, magma_stream >>>(
            m, alpha, A, lda, X, incx, beta, Y, incy, dC_work, my_gpu_id, num_gpus, nb, kstan);
    }
    else
    {
        magma_int_t m_mod_nb = m%ssymv_bs - 1;

        magmablas_ssymv_200_L_generic_mgpu_offset_32 <<< grid, threads, 0, magma_stream >>> (
            m, alpha, A, lda, X, incx ,beta, Y, incy, dC_work, m_mod_nb, my_gpu_id, num_gpus, nb, kstan);
    }

    magmablas_ssymv_200_L_update_mgpu_offset_32<<< grid, threads_u, 0, magma_stream >>>(
        m, alpha, A, lda, X, incx, beta, Y, incy, dC_work, my_gpu_id, num_gpus, nb, kstan);
}
...
__global__ void
magmablas_ssymv_200_U_special_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float  beta,
...
__global__ void
magmablas_ssymv_200_U_generic_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float beta,
    ...
    for(int s=0; s < num_blocks_iters; s++)
    {
        ...
        if(my_gpu_id == the_right_gpu && s == num_blocks_iters-1)
        {
            ...
            for(int j =0; j < ssymv_bs; j += 8)
            {
                if( ( ty + j ) > m_mod_thread_x )
                {
                    MAGMA_S_SET2REAL(la[0][bank_shift*(ty+j)+tx], 0);
                }
                else
                    la[0][bank_shift*(ty+j)+tx] =  A[ j * lda];
            }
...
__global__ void
magmablas_ssymv_200_U_update_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float beta,
    ...
    int kstan )
{
    ...
    for(i = 0; i < (blockIdx.x+1)*ssymv_bs; i += ssymv_bs)
    {
        Ca += WC[0];
        WC += ssymv_bs;
    }
...
extern "C"
void magmablas_ssymv_200_U_mgpu_offset_32(
    magma_int_t m, float alpha,
    float *A, magma_int_t lda,
    float *X, magma_int_t incx,
    float beta,
    ...
    magma_int_t the_right_gpu)
{
     ...
     */
    if(m % ssymv_bs == 0 ) {
        magmablas_ssymv_200_U_special_mgpu_offset_32 <<< grid, threads, 0, magma_stream >>>(
            m, alpha, A, lda, X, incx, beta, Y, incy, dC_work, my_gpu_id, num_gpus, nb, kstan);
    }
    else {
        magma_int_t m_mod_thread_x = m%ssymv_bs - 1;

        magmablas_ssymv_200_U_generic_mgpu_offset_32 <<< grid, threads, 0, magma_stream >>> (
            m, alpha, A, lda, X, incx ,beta, Y, incy, dC_work, m_mod_thread_x, my_gpu_id, num_gpus, nb, kstan, the_right_gpu);
    }

    magmablas_ssymv_200_U_update_mgpu_offset_32<<< grid, threads_u, 0, magma_stream >>>(
        m, alpha, A, lda, X, incx, beta, Y, incy, dC_work, my_gpu_id, num_gpus, nb, kstan);
}
...
extern "C"
magma_int_t
magmablas_ssymv_mgpu_32_offset(
    ...
    magma_queue_t stream[][10])
{
    ...
    magma_int_t blocks    = n / ssymv_bs + (n % ssymv_bs != 0);
    magma_int_t workspace = lda * (blocks + 1);

    if (lwork < workspace) {
        printf("Not enough work space in magmablas_ssymv: passed %d, required %d\n",
               (int) lwork, (int) workspace);
        exit(1);
    }
    if(nb != 32)
    {
        printf("Error in magmablas_ssymv_200_mgpu: nb != 32, program will exit! please reallocate your matrix among GPUs\n");
        exit(0);
    }
    magma_int_t i = 0;
    for(i=0; i < num_gpus; i++)
    {
        ...
        if ( upper)
        {
            magmablas_ssymv_200_U_mgpu_offset_32(n, alpha, A[i], lda, X[i], incx, beta, Y[i], incy, work[i],
                             new_gpu_id, num_gpus, nb, offset, num_blocks_skipped, the_right_gpu);
        }
        else
        {
            magmablas_ssymv_200_L_mgpu_offset_32(n, alpha, A[i], lda, X[i], incx, beta, Y[i], incy, work[i],
