/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zhemv_fermi_mgpu_32.cu normal z -> s, Sat Nov 15 19:53:59 2014
...
__global__ void
ssymv_kernel_fermi_L_special_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float  beta,
    ...
    int kstan)
{
    ...
    __shared__ float la   [hemv_bs][bank_shift];
    __shared__ float sdata   [hemv_bs][9];
    __shared__ float buff [hemv_bs];
    __shared__ float buff2 [hemv_bs];
 ...
 */
__global__ void
ssymv_kernel_fermi_L_generic_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float beta,
    ...
    int kstan)
{
    ...
    if ( (blkc % ngpu) == my_gpu_id ) {
        ...
        if ( blkc == ( gridDim.x - 1 ) ) {
            ...
            for(int j=0; j < hemv_bs; j += 8) {
                if ( ( ty + j ) > m_mod_nb ) {
                    la[0][bank_shift*(ty+j)+tx] = MAGMA_S_MAKE( 9999, 0 );
                }
                else
                    la[0][bank_shift*(ty+j)+tx] = A[ j * lda];
            }
...
__global__ void
ssymv_kernel_fermi_L_update_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float beta,
...
extern "C"
void magmablas_ssymv_fermi_L_mgpu_offset_32(
    ...
    magma_int_t num_blocks_skipped)
{
     ...
     */
    if ( n % hemv_bs == 0 ) {
        ssymv_kernel_fermi_L_special_mgpu_offset_32<<< grid, threads, 0, magma_stream >>>(
            n, alpha, dA, ldda, dx, incx, beta, dy, incy, dwork,
            my_gpu_id, ngpu, nb, kstan);
    }
    else {
        magma_int_t m_mod_nb = (n % hemv_bs) - 1;
        ssymv_kernel_fermi_L_generic_mgpu_offset_32<<< grid, threads, 0, magma_stream >>>(
            n, alpha, dA, ldda, dx, incx, beta, dy, incy, dwork,
            m_mod_nb, my_gpu_id, ngpu, nb, kstan);
    }

    ssymv_kernel_fermi_L_update_mgpu_offset_32<<< grid, threads_u, 0, magma_stream >>>(
        n, alpha, dA, ldda, dx, incx, beta, dy, incy, dwork,
        my_gpu_id, ngpu, nb, kstan);
}
...
__global__ void
ssymv_kernel_fermi_U_special_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float  beta,
...
__global__ void
ssymv_kernel_fermi_U_generic_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float beta,
    ...
    int the_right_gpu)
{
    ...
    for(int s=0; s < num_blocks_iters; s++) {
        ...
        if ( my_gpu_id == the_right_gpu && s == num_blocks_iters-1 ) {
            ...
            for(int j=0; j < hemv_bs; j += 8) {
                if ( ( ty + j ) > m_mod_thread_x ) {
                    la[0][bank_shift*(ty+j)+tx] = MAGMA_S_ZERO;
                }
                else
                    la[0][bank_shift*(ty+j)+tx] = A[ j * lda];
            }
...
__global__ void
ssymv_kernel_fermi_U_update_mgpu_offset_32(
    int n, float alpha,
    float *A, int lda,
    float *x, int incx,
    float beta,
...
extern "C"
void magmablas_ssymv_fermi_U_mgpu_offset_32(
    magma_int_t n, float alpha,
    float *dA, magma_int_t ldda,
    float *dx, magma_int_t incx,
    float beta,
    ...
    magma_int_t the_right_gpu)
{
     ...
     */
    if ( n % hemv_bs == 0 ) {
        ssymv_kernel_fermi_U_special_mgpu_offset_32<<< grid, threads, 0, magma_stream >>>(
            n, alpha, dA, ldda, dx, incx, beta, dy, incy, dwork,
            my_gpu_id, ngpu, nb, kstan);
    }
    else {
        magma_int_t m_mod_thread_x = (n % hemv_bs) - 1;
        ssymv_kernel_fermi_U_generic_mgpu_offset_32<<< grid, threads, 0, magma_stream >>>(
            n, alpha, dA, ldda, dx, incx, beta, dy, incy, dwork,
            m_mod_thread_x, my_gpu_id, ngpu, nb, kstan, the_right_gpu);
    }

    ssymv_kernel_fermi_U_update_mgpu_offset_32<<< grid, threads_u, 0, magma_stream >>>(
        n, alpha, dA, ldda, dx, incx, beta, dy, incy, dwork,
        my_gpu_id, ngpu, nb, kstan);
}
