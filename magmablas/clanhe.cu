...
__global__ void
l_clanhe_special (int n, const magmaFloatComplex* A, int lda,  float *y){
  int tx = threadIdx.x ; 
  int ty = threadIdx.y ; 
  int ind = blockIdx.x*  dgemv_bs + tx ;
  float res = 0.;

  __shared__ magmaFloatComplex la[dgemv_bs][dgemv_bs+1];
...
__global__ void
l_clanhe_generic(int n, const magmaFloatComplex* A, int lda,  float *y, int m_full_block, 
                 int m_mod_32)
{ 
  int tx = threadIdx.x ; 
  int ty = threadIdx.y ; 
  ...
  ****************************************
  ...
  -----------------------------------------------*/
   ...
   #pragma unroll 8
    for(int j=0; j<dgemv_bs; j+=4){
       if( tx < m_mod_32 ) 
         la[ty+j][tx] = MAGMA_C_MUL( MAGMA_C_ONE,  A[ j * lda] );
       else
         la[ty+j][tx] = MAGMA_C_MUL( MAGMA_C_ZERO, A[ j * lda] );
...
__global__ void
u_clanhe_generic (int n, const magmaFloatComplex* A, int lda, float *y, int m_full_block, int m_mod_32){
  ...
  -----------------------------------------------*/
   ...
   if( ty == 0  ) {
        for( int j = 0 ;  j < m_mod_32 ; j++){
                res+=  cuCabsf (  A1[ j + lda * (blockIdx.x) * 32 ] ) ;
        }
   }
    __syncthreads();
...
__global__ void
u_clanhe_special (int n, const magmaFloatComplex* A, int lda, float *y ){
  int tx = threadIdx.x ; 
  int ty = threadIdx.y ; 
  int ind = blockIdx.x*  dgemv_bs + tx ;
  float res = 0.;
...
extern "C" void mclanhe (char uplo, int m, const magmaFloatComplex *A, int lda,  float *Y  )
{
/*
Note:
...
*/
    ...
    if( m % dgemv_bs == 0 ) {
            if( uplo == 'L' || uplo == 'l'){        
                    l_clanhe_special <<< grid, threads, 0, magma_stream >>> (m, A, lda, Y);
            }
            else{
                    u_clanhe_special <<< grid, threads, 0, magma_stream >>> (m, A, lda,  Y);
            } 
...
__global__ void
l_clanhe_max (int m, const magmaFloatComplex* A, int lda,  float *y){
    int tx  = threadIdx.x ;
    int ind =  blockIdx.x * clanhe_bs + tx ;
    float res = 0., res1;

    int break_d = blockIdx.x* clanhe_bs;

    if (ind < m)
    {
       A += ind;

       for(int i=0; i<break_d; i += clanhe_bs ){
          #pragma unroll 8
          for(int j=0; j< clanhe_bs; j++){
...
__global__ void
u_clanhe_max (int m, const magmaFloatComplex* A, int lda,  float *y){
...
extern "C" void clanhe_max (char uplo, int m, const magmaFloatComplex *A, int lda, float *y){
    int blocks;
    if (m % clanhe_bs==0)
        blocks = m/ clanhe_bs;
    else
        blocks = m/ clanhe_bs + 1;

    dim3 grid(blocks, 1, 1);
    dim3 threads(clanhe_bs, 1, 1);

    if( uplo == 'L' || uplo == 'l'){
      l_clanhe_max <<< grid, threads, 0, magma_stream >>> (m, A, lda, y);
    }
    else{
      u_clanhe_max <<< grid, threads, 0, magma_stream >>> (m, A, lda, y);
    }
}
