/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zlaqps3_gpu.cu normal z -> s, Fri Sep 11 18:29:21 2015
...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
            magmablas_sswap( k, dF(pvt,0), lddf, dF(k,0), lddf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
        ...
        if (k < n-1) {
            magma_sgetvector( 1, &dtau[k], 1, &tauk, 1 );
            magma_sgemv( MagmaConjTrans, m-rk, n,
                         tauk,   dA( rk,  0 ), ldda,
                                 dA( rk,  k   ), 1,
                         c_zero, dauxv, 1 );
            if (k == 0)
