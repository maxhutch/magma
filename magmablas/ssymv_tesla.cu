/*
    -- MAGMA (version 1.5.0-beta1) --
       ...
       @generated from csymv_tesla.cu normal c -> s, Fri Apr 25 15:05:24 2014
...
/* TODO: generate ssymv_tesla.cu from ssymv_tesla.cu somehow.
    ...
    float * __restrict__ WC)
{
    ...
    // (was thread_x+1 here, but thread_x+3 in ssymv_tesla.cu; +1 is insufficient)
    __shared__ float la   [quarter_thread_x][thread_x+3]; /* Why +3? */
    ...
    float *dwork)
{
     ...
     */
    if ( n % symv_bs == 0 ) {
        ssymv_kernel_tesla_L_special<<< grid, threads, 0, magma_stream >>>
            (n, alpha, A, lda, x, incx, beta, y, incy, dwork);
    }
    else{
        magma_int_t m_mod_thread_x = (n % symv_bs) - 1;
        ssymv_kernel_tesla_L_generic<<< grid, threads, 0, magma_stream >>>
            (n, alpha, A, lda, x, incx, beta, y, incy, dwork, m_mod_thread_x);
    }

    dim3 threads_u(symv_bs, 1, 1);
    ssymv_kernel_tesla_L_update<<< grid, threads_u, 0, magma_stream >>>
        (n, alpha, A, lda, x, incx, beta, y, incy, dwork);
}
...
/**
    ...
    If users need to call ssymv frequently, we suggest to use magmablas_ssymv_work instead of magmablas_ssymv.
    ...
    float *dwork, magma_int_t lwork)
{
    ...
    if ( upper ) {
        ...
        #else
        cublasSsymv( lapacke_uplo_const(uplo), n, alpha, A, lda, x, incx, beta, y, incy);
        #endif
    }
    else {
        magmablas_ssymv_tesla_L(n, alpha, A, lda, x, incx, beta, y, incy, dwork);
    }
    return MAGMA_SUCCESS;
}
