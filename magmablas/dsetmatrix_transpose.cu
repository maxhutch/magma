/*
    -- MAGMA (version 1.5.0-beta3) --
       ...
       @generated from zsetmatrix_transpose.cu normal z -> d, Fri Jul 18 17:34:12 2014
                                ...
                                double        *dB, magma_int_t lddb, magma_int_t nb )
{
    ...
    for(i=nb; i < n; i += nb) {
       ...
       magmablas_dtranspose_stream( m, nb, dB+(j%2)*nb*lddb, lddb, dat+i-nb, ldda, stream[j%2] );
    }

    /* Transpose the last part of the matrix.                            */
    j++;
    magmablas_dtranspose_stream( m, ib, dB+(j%2)*nb*lddb, lddb, dat+i-nb, ldda, stream[j%2] );
