/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zlascl_diag.cu normal z -> s, Fri Sep 11 18:29:21 2015
*/
...
__global__ void
slascl_diag_lower(int m, int n, float const* D, int ldd, 
                                float*       A, int lda)
{
...
__global__ void
slascl_diag_upper(int m, int n, float const* D, int ldd, 
                                float*       A, int lda)
{
    int ind = blockIdx.x * NB + threadIdx.x;
...
magmablas_slascl_diag(
    ...
    magma_int_t *info )
{
    magmablas_slascl_diag_q( type, m, n, dD, lddd, dA, ldda, magma_stream, info );
}
