/*
    -- MAGMA (version 1.6.3-beta1) --
       ...
       @generated from ztrsv.cu normal z -> d, Tue Aug 25 16:35:09 2015
*/
    ...
    double *x, magma_queue_t queue)
{
    ...
    //Init x with zero
    //magmablas_dlaset(MagmaFull, n, incb, MAGMA_D_ZERO, MAGMA_D_ZERO, x, n);
        ...
        for (magma_int_t i=0; i < n; i+= NB)
        {
            ...
            if (uplo == MagmaUpper)
            {
                ...
                magma_dgemv(MagmaNoTrans, jb, i, MAGMA_D_ONE, A(col, col+jb), lda,
                                x+col+jb, 1, MAGMA_D_ONE, x+col, 1);
            }
            else
            {
                col = i;
                magma_dgemv(MagmaNoTrans, jb, i, MAGMA_D_ONE, A(col, 0), lda,
                                x, 1, MAGMA_D_ONE, x+col, 1);
            }
    ...
    else
    {
        for (magma_int_t i=0; i < n; i += NB)
        {
            ...
            if (uplo == MagmaLower)
            {
                ...
                magma_dgemv(MagmaConjTrans, i, jb, MAGMA_D_ONE, A(col+jb, col), lda, x+col+jb, 1, MAGMA_D_ONE, x+col, 1);
            }
            else
            {
                col = i;
                
                magma_dgemv(MagmaConjTrans, i, jb, MAGMA_D_ONE, A(0, col), lda, x, 1, MAGMA_D_ONE, x+col, 1);
            }
...
/**
    ...
    @param[in]
    A       DOUBLE_PRECISION array of dimension ( lda, n )
    ...
    @param[in]
    b       DOUBLE_PRECISION array of dimension  n
    ...
    magma_queue_t queue)
{
    ...
    magma_dmalloc( &x, size_x);

    magmablas_dlaset(MagmaFull, n, 1, MAGMA_D_ZERO, MAGMA_D_ZERO, x, n);

    magmablas_dtrsv_recursive_outofplace(uplo, trans, diag, n, A, lda, b, incb, x, queue);

    magmablas_dlacpy( MagmaFull, n, 1, x, n, b, n);
