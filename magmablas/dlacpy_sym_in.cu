/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zlacpy_sym_in.cu normal z -> d, Fri Sep 11 18:29:21 2015
    ...
    if ( ind < m ) {
        if ( full ) {
            ...
            for( int jj=0; jj < BLK_Y; ++jj ) 
            {
                int j = rows[2*(iby+jj)];
                if (perm[ind] <= j)
                    dB[ind + (iby+jj)*lddb] = MAGMA_D_CNJG( dA[j + perm[ind]*ldda] );
                else
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        dlacpy_sym_in_lower_kernel<<< grid, threads, 0, queue >>> ( m, n, rows, perm, dA, ldda, dB, lddb );
    }
    else if ( uplo == MagmaUpper ) {
        dlacpy_sym_in_upper_kernel<<< grid, threads, 0, queue >>> ( m, n, dA, ldda, dB, lddb );
    }
    else {
        dlacpy_sym_in_full_kernel <<< grid, threads, 0, queue >>> ( m, n, dA, ldda, dB, lddb );
    }
}
...
magmablas_dlacpy_sym_in(
    ...
    magmaDouble_ptr       dB, magma_int_t lddb )
{
    magmablas_dlacpy_sym_in_q( uplo, m, n, rows, perm, dA, ldda, dB, lddb, magma_stream );
}
