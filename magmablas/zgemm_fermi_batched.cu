    ...
    magmaDoubleComplex **dC_array, magma_int_t lddc, magma_int_t batchCount )
{
    ...
    if      ( transB == MagmaTrans )
        ...
        magmaDoubleComplex *cpuAarray[2], *cpuBarray[2];
        magma_getvector( min(2,batchCount), sizeof(magmaDoubleComplex*), dA_array, 1, cpuAarray, 1);
        magma_getvector( min(2,batchCount), sizeof(magmaDoubleComplex*), dB_array, 1, cpuBarray, 1);
        ...
        cudaError_t err;
        err = cudaBindTexture(&offsetA, tex_ref_A, cpuAarray[0], sizeA*sizeof(magmaDoubleComplex));
        if ( err != cudaSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", cudaGetErrorString(err), err );
            return;
        }	
        err = cudaBindTexture(&offsetB, tex_ref_B, cpuBarray[0], sizeB*sizeof(magmaDoubleComplex));
        if ( err != cudaSuccess ) {
    ...
    if ( TransA == 0 && TransB == 0 ) {
        dim3 dimGrid( (m - 1)/BLK_M_nn + 1,
                      (n - 1)/BLK_N_nn + 1 ,
                      batchCount );
        zgemm_kernel_fermi_nn_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
        ...
        dim3 dimGrid( (m - 1)/BLK_M_nt + 1,
                      (n - 1)/BLK_N_nt + 1 ,
                      batchCount );
        zgemm_kernel_fermi_nt_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_nc + 1,
                      (n - 1)/BLK_N_nc + 1 ,
                      batchCount );
        zgemm_kernel_fermi_nc_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_tn + 1,
                      (n - 1)/BLK_N_tn + 1 ,
                      batchCount );
        zgemm_kernel_fermi_tn_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_tt + 1,
                      (n - 1)/BLK_N_tt + 1 ,
                      batchCount );
        zgemm_kernel_fermi_tt_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_tc + 1,
                      (n - 1)/BLK_N_tc + 1 ,
                      batchCount );
        zgemm_kernel_fermi_tc_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_cn + 1,
                      (n - 1)/BLK_N_cn + 1 ,
                      batchCount );
        zgemm_kernel_fermi_cn_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_ct + 1,
                      (n - 1)/BLK_N_ct + 1 ,
                      batchCount );
        zgemm_kernel_fermi_ct_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_cc + 1,
                      (n - 1)/BLK_N_cc + 1 ,
                      batchCount );
        zgemm_kernel_fermi_cc_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
