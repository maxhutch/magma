/*
    -- MAGMA (version 1.5.0) --
       ...
       @generated from zsetmatrix_transpose.cu normal z -> s, Tue Sep  2 12:38:16 2014
    ...
    magma_queue_t queues[2] )
{
    ...
    for(i=nb; i < n; i += nb) {
       ...
       magmablas_stranspose_q( m, nb, dB+(j%2)*nb*lddb, lddb, dAT+i-nb, ldda, queues[j%2] );
    }

    /* Transpose the last part of the matrix.                            */
    j++;
    magmablas_stranspose_q( m, ib, dB+(j%2)*nb*lddb, lddb, dAT+i-nb, ldda, queues[j%2] );
}
    ...
    float        *dB, magma_int_t lddb, magma_int_t nb )
{
    ...
    magmablas_ssetmatrix_transpose_q( m, n, hA, lda, dAT, ldda, dB, lddb, nb, queues );
