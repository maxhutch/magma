/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zswapblk.cu normal z -> s, Sat Nov 15 19:53:59 2014
    ...
    magma_queue_t queue )
{
    ...
    if ( order == MagmaColMajor ) {
        for( k=(i1-1); k<i2; k+=BLOCK_SIZE )
        {
            ...
            magmagpu_sswapblk_params_t params = { dA+k, dB, n, ldda, lddb, sb };
        ...
        {
            magma_int_t sb = min(BLOCK_SIZE, i2-k);
            magmagpu_sswapblk_params_t params = { dA+k*ldda, dB, n, ldda, lddb, sb };
            for( magma_int_t j = 0; j < sb; j++ )
            {
                im = ipiv[(k+j)*inci] - 1;
                if ( (k+j) == im )
