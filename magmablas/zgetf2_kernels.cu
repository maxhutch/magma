...

{
    ...
    else
    {
        ...
        tree_izamax_kernel_batched<<<grid, zamax>>>(length, x_array, incx, step, lda, ipiv_array, info_array, gbstep, data_pool_array, id_pool_array);

        if( num_blocks > 1)
        {
            // second level tree reduction
            dim3 grid2(1, 1, batchCount);
            tree_izamax_kernel2_batched<<<grid2, zamax>>>(num_blocks, step,  ipiv_array, info_array, gbstep, data_pool_array, id_pool_array);
        }
    }
...
*/
    ...
    dim3 grid(1,1, batchCount);
    zswap_kernel_batched<<< grid, n, 0, magma_stream >>>(n, x_array, incx, step, ipiv_array);
    return 0;
}
...
*/
    ...
    zscal_zgeru_kernel_batched<<< grid, min(m, MAX_NTHREADS), shared_size, magma_stream>>>(m, n, step, dA_array, lda, info_array, gbstep);
    return 0;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
...
*/
    ...
    dim3 grid(1, 1, batchCount);
    zgetf2trsm_kernel_batched<<< grid, max(n,ib), shared_size, magma_stream>>>(ib, n, dA_array, step, lda);
}
