/*
    -- MAGMA (version 1.5.0) --
       ...
       @generated from zgeqr2x_gpu-v4.cu normal z -> c, Tue Sep  2 12:38:17 2014
...
magma_cgeqr2x4_gpu(
    ...
    magmaFloatComplex *dtau, magmaFloatComplex *dT, magmaFloatComplex *ddA,
    ...
    for (magma_int_t b=0; b < k; b += BS) {
        for (i = b; i < min(k, b+BS); ++i) {
            ...
            if (i-b > 0) {
                ...
                if ( i-1 > 0 ) {
                    magma_cgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_stream >>>
                        ( m-i+1, dA(i-1,0), ldda, dA(i-1, i-1), work, dtau+i-1);
                    magma_ctrmv_kernel2<<< i-1, i-1, 0, magma_stream >>>
                        ( dT(0,0), k, work, dT(0,i-1), dtau+i-1);
                }

                /* dwork = V' c */
                magma_cgemv_kernel1<<< i-b, BLOCK_SIZE, 0, magma_stream >>>
                    (m-b, dA(b, b),  ldda, dA(b,i), work);

                /* dwork = T' work */
                magma_ctrmv_tkernel<<< i-b, i-b, 0, magma_stream >>>
                ...
                if ( m-b > 0 ) {
                    ...
                    magma_cgemv_kernel2<<< blocks3, threads3, 0, magma_stream >>>
            ...
            if ( i > 0 ) {
                magma_scnrm2_adjust_kernel<<< 1, i, 0, magma_stream >>>(dnorm+i, dA(0, i));
            }
            ...
            if (i == 0) {
                ...
                magmablas_clacpy(MagmaUpperLower, 1, 1, dtau, 1, dT(0,0), 1);
                magma_csetmatrix_async(1, 1, &tt, 1, dA(i, i), 1, magma_stream);
            }
        }
        if ( i-1 > 0 ) {
            magma_cgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_stream >>>
