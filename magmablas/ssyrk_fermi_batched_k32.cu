/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zherk_fermi_batched_k32.cu normal z -> s, Sat Nov 15 19:53:59 2014
    ...
    float **dC_array, magma_int_t lddc, magma_int_t batchCount )
{
    ...
    if ( TransA == 0 && TransB == 1 ) {
        dim3 dimGrid( (n - 1)/BLK_M_nt + 1,
                      (n - 1)/BLK_N_nt + 1 ,
                      batchCount );
        magmablas_s_herk_kernel_fermi_nt_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
        ...
        dim3 dimGrid( (n - 1)/BLK_M_nc + 1,
                      (n - 1)/BLK_N_nc + 1 ,
                      batchCount );
         magmablas_s_herk_kernel_fermi_nc_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            uploA, n, k, dA_array, ldda, dA_array, ldda, dC_array, lddc, calpha, cbeta,
            (int)offsetA, (int)offsetA );
    }
        ...
        dim3 dimGrid( (n - 1)/BLK_M_tn + 1,
                      (n - 1)/BLK_N_tn + 1 ,
                      batchCount );
         magmablas_s_herk_kernel_fermi_tn_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            uploA, n, k, dA_array, ldda, dA_array, ldda, dC_array, lddc, calpha, cbeta,
            (int)offsetA, (int)offsetA );
    }
        ...
        dim3 dimGrid( (n - 1)/BLK_M_cn + 1,
                      (n - 1)/BLK_N_cn + 1 ,
                      batchCount );
         magmablas_s_herk_kernel_fermi_cn_batched<<< dimGrid, dimBlock, 0, magma_stream >>>(
            uploA, n, k, dA_array, ldda, dA_array, ldda, dC_array, lddc, calpha, cbeta,
            (int)offsetA, (int)offsetA );
    }
