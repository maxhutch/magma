/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zlaswp.cu normal z -> c, Sat Nov 15 19:53:59 2014
...
__global__ void claswp_kernel(
    int n, magmaFloatComplex *dAT, int ldda, claswp_params_t params )
{
    ...
    magma_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        claswp_kernel<<< blocks, threads, 0, queue >>>( n, dAT(k,0), ldda, params );
    }
...
magmablas_claswp(
    ...
    const magma_int_t *ipiv, magma_int_t inci )
{
    magmablas_claswp_q( n, dAT, ldda, k1, k2, ipiv, inci, magma_stream );
}
...
__global__ void claswpx_kernel(
    int n, magmaFloatComplex *dA, int ldx, int ldy, claswp_params_t params )
{
    int tid = threadIdx.x + blockDim.x*blockIdx.x;
    if ( tid < n ) {
        dA += tid*ldy;
    ...
    magma_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        claswpx_kernel<<< blocks, threads, 0, queue >>>( n, dA(k,0), ldx, ldy, params );
    }
...
__global__ void claswp2_kernel(
    int n, magmaFloatComplex *dAT, int ldda, int npivots,
    ...
    magma_queue_t queue )
{
    ...
    dim3 blocks( (n + NTHREADS - 1) / NTHREADS );
    dim3 threads( NTHREADS );
    claswp2_kernel<<< blocks, threads, 0, queue >>>
...
magmablas_claswp2(
    ...
    magmaInt_const_ptr d_ipiv, magma_int_t inci )
{
    magmablas_claswp2_q( n, dAT, ldda, k1, k2, d_ipiv, inci, magma_stream );
}
