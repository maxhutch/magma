/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zgetf2_kernels.cu normal z -> d, Sat Nov 15 19:53:59 2014
*/
...
__global__ void
tree_idamax_kernel2_batched(int n, int step,  magma_int_t** ipiv_array, magma_int_t *info_array, int gbstep, double** data_pool_array, magma_int_t** id_pool_array)
{
    __shared__ double shared_x[zamax];
    __shared__ int    shared_idx[zamax];
...
magma_int_t magma_idamax_lg_batched(magma_int_t length, double **x_array, magma_int_t incx, magma_int_t step,  magma_int_t lda,
...

{
    ...
    else
    {
        ...
        tree_idamax_kernel_batched<<<grid, zamax>>>(length, x_array, incx, step, lda, ipiv_array, info_array, gbstep, data_pool_array, id_pool_array);

        if( num_blocks > 1)
        {
            // second level tree reduction
            dim3 grid2(1, 1, batchCount);
            tree_idamax_kernel2_batched<<<grid2, zamax>>>(num_blocks, step,  ipiv_array, info_array, gbstep, data_pool_array, id_pool_array);
        }
    }
        ...
        magma_int_t** ipiv_array, magma_int_t *info_array, magma_int_t gbstep, magma_int_t batchCount)
{
    ...
    if(length == 0 ) return 0;
        ...
        idamax_kernel_batched<<< grid, zamax, zamax * (sizeof(double) + sizeof(int)), magma_stream >>>
                      (length, chunk, x_array, incx, step, lda, ipiv_array, info_array, gbstep);

#else
    // the magma_idamax_lg_batched is faster but when cuda launch it as 2 kernels the white space time between these 2 kernels and the next kernel is larger than using the idamax_kernel for that today we are using only idamax_kernel
    if( length <= 10 * zamax )
    {  
        dim3 grid(1, 1, batchCount);
        int chunk = (length-1)/zamax + 1;
        idamax_kernel_batched<<< grid, zamax, zamax * (sizeof(double) + sizeof(magma_int_t)), magma_stream >>>
                      (length, chunk, x_array, incx, step, lda, ipiv_array, info_array, gbstep);

    }
    else
    {
        magma_idamax_lg_batched(length, x_array, incx, step, lda, ipiv_array, info_array, gbstep, batchCount);
    }
#endif
...
__global__
void dswap_kernel_batched(magma_int_t n, double **x_array, magma_int_t incx, magma_int_t step, magma_int_t** ipiv_array)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_int_t magma_dswap_batched(magma_int_t n, double **x_array, magma_int_t incx, magma_int_t step, 
...
*/
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_dswap_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }
    dim3 grid(1,1, batchCount);
    dswap_kernel_batched<<< grid, n, 0, magma_stream >>>(n, x_array, incx, step, ipiv_array);
    return 0;
}

/////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
void dscal_dger_kernel_batched(int m, int n, int step, double **dA_array, int lda, magma_int_t *info_array, int gbstep)
{
    ...
    if (gbidx < m && gbidx > 0) {
        ...
        for(int i=1; i < n; i++) {
            ...
            A[gbidx + i*lda] += (MAGMA_D_NEG_ONE) * shared_y[i] * reg;
        }
    }
}
...
*/
    ...
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_dscal_dger_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }
    ...
    dscal_dger_kernel_batched<<< grid, min(m, MAX_NTHREADS), shared_size, magma_stream>>>(m, n, step, dA_array, lda, info_array, gbstep);
    return 0;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" void
magma_dgetf2trsm_batched(magma_int_t ib, magma_int_t n, double **dA_array,  magma_int_t step, magma_int_t lda,
                       magma_int_t batchCount)
{
/*
...
*/
    ...
    dim3 grid(1, 1, batchCount);
    dgetf2trsm_kernel_batched<<< grid, max(n,ib), shared_size, magma_stream>>>(ib, n, dA_array, step, lda);
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void 
zcomputecolumn_kernel_shared_batched(int m, int paneloffset, int step, double **dA_array, int lda, magma_int_t **ipiv_array, magma_int_t *info_array, int gbstep)
{
...
*/
    ...
    {
        printf("magma_dcomputecolumn_batched error out of shared memory \n");
        return -20;
    }
