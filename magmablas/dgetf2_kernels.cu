/*
    -- MAGMA (version 1.6.3-beta1) --
       ...
       @generated from zgetf2_kernels.cu normal z -> d, Tue Aug 25 16:35:10 2015
*/
        ...
        magma_int_t** ipiv_array, magma_int_t *info_array, magma_int_t gbstep, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    {  
        dim3 grid(1, 1, batchCount);
        int chunk = magma_ceildiv( length, zamax );
        idamax_kernel_batched<<< grid, zamax, zamax * (sizeof(double) + sizeof(magma_int_t)), queue >>>
            (length, chunk, x_array, incx, step, lda, ipiv_array, info_array, gbstep);
    }
    else
    {
    ...
    for (int j=0; j < ib; j++)
    {
        ...
        if (tx < zamax)
        {
            if ( tx < length)
            {
                ...
                // printf("res=%f\n", res);
                shared_x[tx] = fabs(MAGMA_D_REAL(res)) + fabs(MAGMA_D_IMAG(res));
                shared_idx[tx] = tx;
            }
            else
            {
        ...
        if (tx < length && tx > 0)
        {
            ...
            res *= MAGMA_D_DIV(MAGMA_D_ONE, shared_A[0 + offset]); // scaling
            ...
            {
                shared_A[tx + i*m + offset] += (MAGMA_D_NEG_ONE) * shared_A[i*m + offset] * res;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_int_t  magma_dgetf2_sm_batched(
    magma_int_t m, magma_int_t ib,
    double **dA_array, magma_int_t lda,
    magma_int_t **ipiv_array,
    magma_int_t *info, 
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    */
    size_t shared_size = sizeof(double) * m * ib + (zamax) * (sizeof(double) + sizeof(int)) + sizeof(int);

    if (shared_size > 47000)
    {
        printf("Shared memory in dgetf2 = %d, exceeds 48K, kernel can not lauched succesfully\n", int(shared_size) );
        return 1;
    }

    dim3 grid(1,1, batchCount);

    kernel_dgetf2_sm_batched<<<grid, max(max(zamax, m), ib), shared_size, queue>>>
