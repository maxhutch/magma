...
/*
   ...
   @generated from magmablas/zpotf2_devicesfunc.cuh normal z -> s, Wed Jan  6 17:59:51 2016
 */
...
static inline __device__ void spotf2_sminout_anywidth_device(const int m, const int n, float *A, const int lda)
{
    ...
    for (int iter=0; iter < n; iter++)
    {
        ...
        if ( tx > iter && tx < m )
        {
            ...
            {
                A [tx + j * lda] -= A[tx + iter * lda]  *  MAGMA_S_CNJG(A[iter * lda + j]);
            }   
        #ifdef ENABLE_COND1
        }
        #endif
...
static inline __device__ void spotf2_sminout_fixsize_device(const int m, float *A, const int lda)
{
    ...
    for (int iter=0; iter < POTF2_NB; iter++)
    {
        ...
        if ( tx >= iter && tx < m )
        {
            ...
            //A[ tx + iter * lda ]  = tx == iter ? MAGMA_S_MAKE(xreal, 0.0) : A[ tx + iter * lda ] * factor;
            //if (tx < POTF2_NB) row[ tx ] = MAGMA_S_CNJG( A[ tx + iter * lda ] );
            //if (tx < POTF2_NB) A[ iter + tx * lda ] = MAGMA_S_CNJG( A[ tx + iter * lda ] );
        #ifdef ENABLE_COND2
        }
        #endif
        ...
        float *sC, float  *sB)
{
    ...
    for (int iter=0; iter < k; iter += POTF2_NB)
    {
        ...
        {      
            ...
            for (int i=0; i < POTF2_NB; i++)
            {
                sB[tx + i * POTF2_NB] = MAGMA_S_CNJG(rp[i]);
            }
        }
