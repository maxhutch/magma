/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from magmablas/zgetf2_kernels.cu, normal z -> c, Sun Nov 20 20:20:32 2016
*/
                                 ...
                                 magma_int_t gbstep, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    {
        int chunk = magma_ceildiv( length, zamax );
        icamax_kernel_batched<<< grid, threads, zamax * (sizeof(float) + sizeof(magma_int_t)), queue->cuda_stream() >>>
            (length, chunk, x_array, incx, step, lda, ipiv_array, info_array, gbstep);
    }
    else
    {
...
*******************************************************************************/
extern "C" magma_int_t
magma_cswap_batched(magma_int_t n, magmaFloatComplex **x_array, magma_int_t incx,
    ...
    magma_int_t *info_array)
{
    ...
    magmaFloatComplex *A = dA_array[blockIdx.z];
    ...
    for (int j=0; j < ib; j++)
    {
        ...
        if (tx < zamax)
        {
            if ( tx < length)
            {
                ...
                // printf("res=%f\n", res);
                shared_x[tx] = fabs(MAGMA_C_REAL(res)) + fabs(MAGMA_C_IMAG(res));
                shared_idx[tx] = tx;
            }
            else
            {
        ...
        if ( jp != 0) //swap
        {
            if (tx < ib) {
                ...
                magmaFloatComplex tmp = shared_A[jp + j + tx*m];
        ...
        if (tx < length && tx > 0)
        {
            ...
            res *= MAGMA_C_DIV(MAGMA_C_ONE, shared_A[0 + offset]); // scaling
            ...
            {
                shared_A[tx + i*m + offset] += (MAGMA_C_NEG_ONE) * shared_A[i*m + offset] * res;
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    */
    size_t shared_size = sizeof(magmaFloatComplex) * m * ib + (zamax) * (sizeof(float) + sizeof(int)) + sizeof(int);
    ...
    kernel_cgetf2_sm_batched<<< grid, threads, shared_size, queue->cuda_stream() >>>
