...
/*
    -- MAGMA (version 1.5.0-beta1) --
       ...
       @generated from ztrtri_kernel.cu normal z -> s, Fri Apr 25 15:05:24 2014

       @author Peng Du
       @author Tingxing Dong
       
       File named strtri_kernel.cu to avoid name conflict with src/strtri.o
       in the library.
*/
...
__global__ void
diag_strtri_kernel_upper(magma_diag_t diag, const float *A, float *d_dinvA, int lda)
{
    int i, j;
    float Ystx = MAGMA_S_ZERO;
    float *y = NULL;
    float switcher;
    float neg_switcher;
    ...
    for( i=0; i < BLOCK_SIZE; i++ )
    {
        ...
        {
           Bs[i*BLOCK_SIZE+tx] = MAGMA_S_ZERO; 
        }
    } 
// read in the whole square block of my A and zero out the non data triangular
   ...
   // solve the diagonals
    ...
    if(diag == MagmaUnit)
    {
      Bs[tx*BLOCK_SIZE+tx] = MAGMA_S_ONE;
    }
    else
    {
      if( Bs[tx*BLOCK_SIZE+tx] == MAGMA_S_ZERO )
      {
         Bs[tx*BLOCK_SIZE+tx] = MAGMA_S_ONE;  
      }
      else
      {
         Bs[tx*BLOCK_SIZE+tx] = MAGMA_S_ONE / ( Bs[tx*BLOCK_SIZE+tx]) ;
      }      
    }
    ...
    for( i=0; i < BLOCK_SIZE; i++ ) {
        ...
        if( tx < i)
        {
          switcher = MAGMA_S_ONE;
        }
        else
        {
          switcher = MAGMA_S_ZERO;
        }
        ...
        {
           switcher = MAGMA_S_ONE;
           neg_switcher =  MAGMA_S_ZERO;
        }
        else
        {
          switcher = MAGMA_S_ZERO;
          neg_switcher =  MAGMA_S_ONE;
        }
...
__global__ void
diag_strtri_kernel_lower(magma_diag_t diag, const float *A, float *d_dinvA, int lda)
{
    int i, j;
    float Ystx=  MAGMA_S_ZERO;
...
__device__ void sgemm_kernel_16(
    float *A, int lda,
    float *B, int ldb,
    float *C, int ldc,
    float alpha, int blk, int inx, int iny, float *c)
{
...
extern "C"
void diag_strtri (magma_int_t M, 
magma_uplo_t uplo, 
magma_diag_t diag, 
const float *A, 
float *d_dinvA, 
