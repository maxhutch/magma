...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void sdot_kernel_batched(int n, float **x_array, int incx, int offset, magma_int_t *info_array, int gbstep)
{
    int tx = threadIdx.x;
    ...
    if (tx == 0) {
        ...
        //MAGMA_S_SET2REAL(x[n*incx], sqrt(xreal - sdata[0]));
        x[n*incx] = MAGMA_S_MAKE(sqrt(xreal - sdata[0]), 0);
        if(x[n*incx] == MAGMA_S_ZERO){
...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void sscal_kernel_batched(int n, float **x_array, int incx, int offset, magma_int_t *info_array)
{
...
*/
    ...
    for(int iter=0; iter<n; iter++)
    {
        ...
        if(tx<iter)
        {
            res = MAGMA_S_REAL (sdata_A[iter + tx * m] * MAGMA_S_CNJG(sdata_A[iter + tx * m]));         
            sum[tx] = res;
        }
        else
        {
        ...
        if (tx == 0) {
              float xreal = MAGMA_S_REAL(sdata_A[iter + iter * m]);        
              sdata_A[iter + iter * m] = MAGMA_S_MAKE(sqrt(xreal - sum[0]), 0);
              if(sdata_A[iter + iter * m] == MAGMA_S_ZERO){
                  *info = iter + gbstep + 1;
              }
        }
        __syncthreads();
        if(sdata_A[iter + iter * m] == MAGMA_S_ZERO) return;
        __syncthreads();
        ...
        if(tx < iter)
        {
             sdata_A[iter + tx * m] = MAGMA_S_CNJG(sdata_A[iter + tx * m]);
        }
        __syncthreads();  
        #endif
