/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from ztrsv_batched.cu normal z -> s, Fri Sep 11 18:29:22 2015
*/
            ...
            if (diag == MagmaNonUnit)
            {
                if (flag == 0) {
                    strsv_notrans_kernel_outplace_batched< BLOCK_SIZE_N, DIM_X_N, DIM_Y_N, MagmaBigTileSize, 0, MagmaUpper, MagmaNoTrans, MagmaNonUnit >
                        <<<blocks, threads, sizeof(float)*(n), queue>>>
    ...
    magma_queue_t queue)
{
    ...
    sset_pointer(x_array, x, n, 0, 0, size_x, batchCount, queue);
