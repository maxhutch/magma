...
/*
    -- MAGMA (version 1.5.0-beta1) --
       ...
       @generated from ztrtri_kernel.cu normal z -> d, Fri Apr 25 15:05:24 2014

       @author Peng Du
       @author Tingxing Dong
       
       File named dtrtri_kernel.cu to avoid name conflict with src/dtrtri.o
       in the library.
*/
...
diag_dtrtri_kernel_upper(magma_diag_t diag, const double *A, double *d_dinvA, int lda)
{
    ...
    for( i=0; i < BLOCK_SIZE; i++ )
    {
        ...
        {
           Bs[i*BLOCK_SIZE+tx] = MAGMA_D_ZERO; 
        }
    } 
// read in the whole square block of my A and zero out the non data triangular
   ...
   // solve the diagonals
    ...
    if(diag == MagmaUnit)
    {
      Bs[tx*BLOCK_SIZE+tx] = MAGMA_D_ONE;
    }
    else
    {
      if( Bs[tx*BLOCK_SIZE+tx] == MAGMA_D_ZERO )
      {
         Bs[tx*BLOCK_SIZE+tx] = MAGMA_D_ONE;  
      }
      else
      {
         Bs[tx*BLOCK_SIZE+tx] = MAGMA_D_ONE / ( Bs[tx*BLOCK_SIZE+tx]) ;
      }      
    }
    ...
    for( i=0; i < BLOCK_SIZE; i++ ) {
        ...
        if( tx < i)
        {
          switcher = MAGMA_D_ONE;
        }
        else
        {
          switcher = MAGMA_D_ZERO;
        }
        ...
        {
           switcher = MAGMA_D_ONE;
           neg_switcher =  MAGMA_D_ZERO;
        }
        else
        {
          switcher = MAGMA_D_ZERO;
          neg_switcher =  MAGMA_D_ONE;
        }
...
extern "C"
void diag_dtrtri (magma_int_t M, 
magma_uplo_t uplo, 
magma_diag_t diag, 
const double *A, 
double *d_dinvA, 
