/*
    -- MAGMA (version 1.5.0-beta3) --
       ...
       @generated from zlaset.cu normal z -> c, Fri Jul 18 17:34:12 2014
    ...
    magma_queue_t stream)
{
    ...
    if (uplo == MagmaLower)
        claset_lower<<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
    else if (uplo == MagmaUpper)
        claset_upper<<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
    else
        claset_full <<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
}


/**
    @see magmablas_claset_stream
    @ingroup magma_caux2
    ********************************************************************/
extern "C"
void magmablas_claset(
    ...
    magmaFloatComplex *dA, magma_int_t ldda )
{
    magmablas_claset_stream( uplo, m, n, offdiag, diag, dA, ldda, magma_stream );
}
...
/**
    ...
    ********************************************************************/
extern "C" void
magmablas_claset_band_stream(
    magma_uplo_t uplo, magma_int_t m, magma_int_t n, magma_int_t k,
    magmaFloatComplex offdiag, magmaFloatComplex diag,
    magmaFloatComplex *dA, magma_int_t ldda, magma_queue_t stream)
{
    magma_int_t info = 0;
    ...
    if (uplo == MagmaUpper) {
        ...
        claset_band_upper<<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
}
    else if (uplo == MagmaLower) {
        dim3 threads( min(k,m) );
        dim3 grid( (min(m,n) - 1)/LASET_BAND_NB + 1 );
        claset_band_lower<<< grid, threads, 0, stream >>> (m, n, offdiag, diag, dA, ldda);
    }
}


/**
    @see magmablas_claset_band_stream
    @ingroup magma_caux2
    ********************************************************************/
extern "C" void
magmablas_claset_band(
    ...
    magmaFloatComplex *dA, magma_int_t ldda)
{
    magmablas_claset_band_stream(uplo, m, n, k, offdiag, diag, dA, ldda, magma_stream);
}
