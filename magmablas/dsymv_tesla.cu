/*
    -- MAGMA (version 1.5.0-beta1) --
       ...
       @generated from csymv_tesla.cu normal c -> d, Fri Apr 25 15:05:24 2014
...
/* TODO: generate dsymv_tesla.cu from dsymv_tesla.cu somehow.
 * Basically just strip out cuConj calls. */
    ...
    double * __restrict__ WC)
{
    ...
    // (was thread_x+1 here, but thread_x+3 in dsymv_tesla.cu; +1 is insufficient)
    __shared__ double la   [quarter_thread_x][thread_x+3]; /* Why +3? */
    ...
    int m_mod_thread_x)
{
    ...
    __shared__ double la   [quarter_thread_x][thread_x+3];
    __shared__ double buff [thread_x];
    __shared__ double buff2[thread_x];
    ...
    double *dwork)
{
     ...
     */
    if ( n % symv_bs == 0 ) {
        dsymv_kernel_tesla_L_special<<< grid, threads, 0, magma_stream >>>
            (n, alpha, A, lda, x, incx, beta, y, incy, dwork);
    }
    else{
        magma_int_t m_mod_thread_x = (n % symv_bs) - 1;
        dsymv_kernel_tesla_L_generic<<< grid, threads, 0, magma_stream >>>
            (n, alpha, A, lda, x, incx, beta, y, incy, dwork, m_mod_thread_x);
    }

    dim3 threads_u(symv_bs, 1, 1);
    dsymv_kernel_tesla_L_update<<< grid, threads_u, 0, magma_stream >>>
        (n, alpha, A, lda, x, incx, beta, y, incy, dwork);
}
...
/**
    ...
    If users need to call dsymv frequently, we suggest to use magmablas_dsymv_work instead of magmablas_dsymv.
    ...
    double *dwork, magma_int_t lwork)
{
    ...
    if ( upper ) {
        ...
        #else
        cublasDsymv( lapacke_uplo_const(uplo), n, alpha, A, lda, x, incx, beta, y, incy);
        #endif
    }
    else {
        magmablas_dsymv_tesla_L(n, alpha, A, lda, x, incx, beta, y, incy, dwork);
    }
    return MAGMA_SUCCESS;
}
...
/*************************************************************************
    ...
    @param[in]
    alpha   DOUBLE_PRECISION*16.
            On entry, ALPHA specifies the scalar alpha.

    @param[in]
    A       DOUBLE_PRECISION*16 array of DIMENSION ( LDA, n ).
    ...
    @param[in]
    X       DOUBLE_PRECISION*16 array of dimension at least
            ( 1 + ( n - 1 )*abs( INCX ) ).
    ...
    @param[in]
    beta    DOUBLE_PRECISION*16.
            On entry, BETA specifies the scalar beta. When BETA is
            supplied as zero then Y need not be set on input.

    @param[in,out]
    Y       DOUBLE_PRECISION*16 array of dimension at least
            ( 1 + ( n - 1 )*abs( INCY ) ).
    ...
    double *y, magma_int_t incy)
{
    ...
    if ( upper ) {
        ...
        magmablas_dsymv_tesla_work(
            uplo, n, alpha, A, lda, x, incx, beta, y, incy, dwork, lwork);
