...
__global__ void
magmablas_dsymv_130_L_special( int n, double alpha,
                               ...
                               double *WC)
{
    ...
    __shared__ double la   [quarter_thread_x][thread_x+1]; /* Why +3? */ 
 ...
 */
__global__ void
magmablas_dsymv_130_L_generic(int n, double alpha,
                              const double *A, int lda,
                              const double *x, int incx,
                              double beta,
                              double *y, int incy,
...
__global__ void
magmablas_dsymv_130_L_update(int n, double alpha,
                         const double* A, int lda,
                         const double *x, int incx,
                         double beta,
                         double *y, int incy,
                           ...
                           double *dC_work)
{
     ...
     */
    if(m % dsymv_bs == 0 ) {
        magmablas_dsymv_130_L_special <<< grid, threads, 0, magma_stream >>>(
            m, alpha, A, lda, X, incx, beta, Y, incy, dC_work);
    }
    else{
        magma_int_t m_mod_thread_x = m%dsymv_bs - 1;
        magmablas_dsymv_130_L_generic <<< grid, threads, 0, magma_stream >>> (
            m, alpha, A, lda, X, incx ,beta, Y, incy, dC_work, m_mod_thread_x);
    }

    magmablas_dsymv_130_L_update<<< grid, threads_u, 0, magma_stream >>>(
        m, alpha, A, lda, X, incx, beta, Y, incy, dC_work);
}
                      ...
                      magma_int_t workspace)
{
    ...
    {
        magmablas_dsymv_130_L(n, alpha, A, lda, X, incx, beta, Y, incy, dC_work);
    }
    return MAGMA_SUCCESS;
}
                     ...
                     double *Y, magma_int_t incy)
{
    ...
    else
    {
        ...
        magmablas_dsymv2_130( uplo, n, alpha, 
                              A, lda, X, incx,
