...
__global__ void
magma_zswap_gemv_kernel(
    int m, int rk, int n, const magmaDoubleComplex * __restrict__ V, int ldv,
...
__global__ void
magma_zgemv_kernel(
    int m, int n, const magmaDoubleComplex * __restrict__ V, int ldv,
...
__global__
void magma_zscale_kernel(
    int n, magmaDoubleComplex* dx0,
...
__global__ void
magma_zgemv_kernel_adjust(
    int n, int k, magmaDoubleComplex * A, int lda, 
...
magmablas_dznrm2_check_kernel(
    int m, magmaDoubleComplex *da, int ldda, 
...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
            magmablas_zswap( k, dF(pvt,0), lddf, dF(k,0), lddf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
        ...
        if (k < n-1) {
            magma_zgetvector( 1, &dtau[k], 1, &tauk, 1 );
            magma_zgemv( MagmaConjTrans, m-rk, n,
                         tauk,   dA( rk,  0 ), ldda,
                                 dA( rk,  k   ), 1,
                         c_zero, dauxv, 1 );
            if (k == 0)
                magmablas_zlacpy(MagmaUpperLower, n-k-1, 1, dauxv+k+1, n-k-1, dF( k+1, k   ), n-k-1);
        }
