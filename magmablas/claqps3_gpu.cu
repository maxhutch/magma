/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zlaqps3_gpu.cu normal z -> c, Fri Sep 11 18:29:21 2015
...
__global__ void
magma_cswap_gemv_kernel(
    int m, int rk, int n, const magmaFloatComplex * __restrict__ V, int ldv,
...
__global__ void
magma_cgemv_kernel(
    int m, int n, const magmaFloatComplex * __restrict__ V, int ldv,
...
__global__
void magma_cscale_kernel(
    int n, magmaFloatComplex* dx0,
...
__global__ void
magma_cgemv_kernel_adjust(
    int n, int k, magmaFloatComplex * A, int lda, 
...
magmablas_scnrm2_check_kernel(
    int m, magmaFloatComplex *da, int ldda, 
...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        magma_cswap_gemv_kernel<<< magma_ceildiv( m, BLOCK_SIZE1 ), BLOCK_SIZE1, 0, magma_stream >>> 
        ...
        if (k > 0) {
            ...
            magma_cgemv_kernel<<< magma_ceildiv( n-k-1, BLOCK_SIZE1 ), BLOCK_SIZE1, 0, magma_stream >>>
        ...
        if (k < n-1) {
            ...
            magma_cgemv_kernel_adjust<<< magma_ceildiv( n-k-1, BLOCK_SIZE2 ), BLOCK_SIZE2, 0, magma_stream >>>
