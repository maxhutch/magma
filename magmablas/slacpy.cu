/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zlacpy.cu normal z -> s, Sat Nov 15 19:53:59 2014
       @author Mark Gates
       @author Azzam Haidar
*/
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        slacpy_lower_kernel<<< grid, threads, 0, queue >>> ( m, n, dA, ldda, dB, lddb );
    }
    else if ( uplo == MagmaUpper ) {
        slacpy_upper_kernel<<< grid, threads, 0, queue >>> ( m, n, dA, ldda, dB, lddb );
    }
    else {
        slacpy_full_kernel <<< grid, threads, 0, queue >>> ( m, n, dA, ldda, dB, lddb );
    }
}
...
/**
    Purpose
    -------
    SLACPY_BATCHED_Q copies all or part of each two-dimensional matrix
    dAarray[i] to matrix dBarray[i], for 0 <= i < batchcount.
    
    This is the same as SLACPY_BATCHED, but adds queue argument.
...
{
    magmablas_slacpy_batched_q( uplo, m, n, dAarray, ldda, dBarray, lddb, batchCount, magma_stream );
}
