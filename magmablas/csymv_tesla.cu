...
/* TODO: generate csymv_tesla.cu from chemv_tesla.cu somehow.
    ...
    magmaFloatComplex * __restrict__ WC)
{
    ...
    // (was thread_x+1 here, but thread_x+3 in chemv_tesla.cu; +1 is insufficient)
    ...
    magmaFloatComplex *dwork)
{
     ...
     */
    if ( n % symv_bs == 0 ) {
        csymv_kernel_tesla_L_special<<< grid, threads, 0, magma_stream >>>
            (n, alpha, A, lda, x, incx, beta, y, incy, dwork);
    }
    else{
        magma_int_t m_mod_thread_x = (n % symv_bs) - 1;
        csymv_kernel_tesla_L_generic<<< grid, threads, 0, magma_stream >>>
            (n, alpha, A, lda, x, incx, beta, y, incy, dwork, m_mod_thread_x);
    }

    dim3 threads_u(symv_bs, 1, 1);
    csymv_kernel_tesla_L_update<<< grid, threads_u, 0, magma_stream >>>
        (n, alpha, A, lda, x, incx, beta, y, incy, dwork);
}
...
/*************************************************************************
    ...
    If users need to call csymv frequently, we suggest to use magmablas_csymv_work instead of magmablas_csymv.
    ...
    magmaFloatComplex *dwork, magma_int_t lwork)
{
    ...
    if ( upper ) {
        ...
        magmablas_csymv_tesla_L(n, alpha, A, lda, x, incx, beta, y, incy, dwork);
    }
    return MAGMA_SUCCESS;
}
