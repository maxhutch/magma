...
extern "C"
void magmablas_ssymv_200_L(magma_int_t m, float alpha,
                           ...
                           float *dC_work)
{
     ...
     */
    if(m % ssymv_bs == 0 ) {
        magmablas_ssymv_200_L_special <<< grid, threads, 0, magma_stream >>>(
            m, alpha, A, lda, X, incx, beta, Y, incy, dC_work);
    }
    else{
        magma_int_t m_mod_thread_x = m%ssymv_bs - 1;
        magmablas_ssymv_200_L_generic <<< grid, threads, 0, magma_stream >>> (
            m, alpha, A, lda, X, incx ,beta, Y, incy, dC_work, m_mod_thread_x);
    }

    magmablas_ssymv_200_L_update<<< grid, threads_u, 0, magma_stream >>>(
        m, alpha, A, lda, X, incx, beta, Y, incy, dC_work);
}
...
extern "C"
magma_int_t
magmablas_ssymv_200( char uplo, magma_int_t n,
                     float alpha, 
                     const float *A, magma_int_t lda,
                     const float *X, magma_int_t incx,
                     float beta,  
                     float *Y, magma_int_t incy)
{
    ...
    else
    {
        ...
        magmablas_ssymv_200_L(n, alpha, A, lda, X, incx, beta, Y, incy, dC_work);
