    ...
    if ( ind < m ) {
        if ( full ) {
            ...
            for( int jj=0; jj < BLK_Y; ++jj ) {
                ...
                if (ind <= j)
                    dB[j + ind*ldda] = MAGMA_C_CNJG( dA[ind + (iby+jj)*lddb] );
                else
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        clacpy_sym_out_lower_kernel<<< grid, threads, 0, queue >>> ( m, n, rows, perm, dA, ldda, dB, lddb );
    }
    else if ( uplo == MagmaUpper ) {
        clacpy_sym_out_upper_kernel<<< grid, threads, 0, queue >>> ( m, n, dA, ldda, dB, lddb );
    }
    else {
        clacpy_sym_out_full_kernel <<< grid, threads, 0, queue >>> ( m, n, dA, ldda, dB, lddb );
    }
}
...
magmablas_clacpy_sym_out(
    ...
    magmaFloatComplex_ptr       dB, magma_int_t lddb )
{
    magmablas_clacpy_sym_out_q( uplo, m, n, rows, perm, dA, ldda, dB, lddb, magma_stream );
}
