/*
    -- MAGMA (version 1.6.3-beta1) --
       ...
       @generated from ztrsv.cu normal z -> c, Tue Aug 25 16:35:09 2015
*/
...
magmablas_ctrsv_outofplace(
    ...
    magmaFloatComplex *b, magma_int_t incb,
    magmaFloatComplex *x, magma_queue_t queue,
    magma_int_t flag=0)
{
    /* Check arguments */
    magma_int_t info = 0;
            ...
            if (diag == MagmaNonUnit)
            {
                if (flag == 0) {
                    ctrsv_notrans_kernel_outplace< BLOCK_SIZE_N, DIM_X_N, DIM_Y_N, MagmaBigTileSize, 0, MagmaUpper, MagmaNoTrans, MagmaNonUnit >
                        <<<blocks, threads, sizeof(magmaFloatComplex)*(n), queue>>>
                        (n, A, lda, b, incb, x);
                }
                else {
    ...
    magmaFloatComplex *x, magma_queue_t queue)
{
    ...
    //Init x with zero
    //magmablas_claset(MagmaFull, n, incb, MAGMA_C_ZERO, MAGMA_C_ZERO, x, n);
        ...
        for (magma_int_t i=0; i < n; i+= NB)
        {
            ...
            if (uplo == MagmaUpper)
            {
                ...
                magma_cgemv(MagmaNoTrans, jb, i, MAGMA_C_ONE, A(col, col+jb), lda,
                                x+col+jb, 1, MAGMA_C_ONE, x+col, 1);
            }
            else
            {
                col = i;
                magma_cgemv(MagmaNoTrans, jb, i, MAGMA_C_ONE, A(col, 0), lda,
                                x, 1, MAGMA_C_ONE, x+col, 1);
            }
    ...
    else
    {
        for (magma_int_t i=0; i < n; i += NB)
        {
            ...
            if (uplo == MagmaLower)
            {
                ...
                magma_cgemv(MagmaConjTrans, i, jb, MAGMA_C_ONE, A(col+jb, col), lda, x+col+jb, 1, MAGMA_C_ONE, x+col, 1);
            }
            else
            {
                col = i;
                
                magma_cgemv(MagmaConjTrans, i, jb, MAGMA_C_ONE, A(0, col), lda, x, 1, MAGMA_C_ONE, x+col, 1);
            }
    ...
    magma_queue_t queue)
{
    ...
    magma_cmalloc( &x, size_x);

    magmablas_claset(MagmaFull, n, 1, MAGMA_C_ZERO, MAGMA_C_ZERO, x, n);

    magmablas_ctrsv_recursive_outofplace(uplo, trans, diag, n, A, lda, b, incb, x, queue);

    magmablas_clacpy( MagmaFull, n, 1, x, n, b, n);
