...
/*
    ...
    float *R=(float *) malloc(siz*siz*sizeof(float));
    // collecte back A
...
*/
        ...
        for( magma_int_t i = fstblksiz; i < m; i += nb ) {
            ...
            for( magma_int_t dev = 0; dev < ngpu; ++dev ) {
                ...
                if(myrowsize>0){
                    ...
                    magma_sgemm( MagmaTrans, MagmaNoTrans, myrowsize, n, ib,
    ...
    for( magma_int_t i = fstblksiz; i < m; i += nb ) {
        ...
        if(ngpu==1){
            ...
            // magmablasSetKernelStream( streams[ dev ][ 0 ] );
            magma_sgemm( MagmaTrans, MagmaNoTrans, i, n, ib,
            ...
            for( magma_int_t idev = 0; idev < myngpu; ++idev ) {
                ...
                if(mycolsize>0){
                    ...
                    if(dev!=masterdev){
                        ...
                        cudaMemcpy2DAsync(&dwork2[masterdev][maxgsize*dev], m*sizeof(float),
                                          &dC[dev][0], lddc*sizeof(float),
                                          m*sizeof(float), n,
            ...
            for( magma_int_t k = 0; k < nbcmplx; ++k ) {
                if(k!=cmplxid){
                    ...
                    if(gmaster!=-1){ //real is active
                        ...
                        cudaMemcpy2DAsync(&dwork2[gmaster][maxgsize*masterdev], m*sizeof(float),
                                          &dC[masterdev][0], lddc*sizeof(float),
                                          m*sizeof(float), n,
        ...
        if(masterdev!=-1){ 
            ...
            for( magma_int_t l = 0; l < myngpu; ++l ) {
                ...
                if((lcdev!=masterdev)&&(lccolsize>0)){
                    ...
                    magma_queue_wait_event(streams[ masterdev ][ 0 ], redevents[masterdev][masterdev]);
                    cudaMemcpy2DAsync(&dC[lcdev][0], lddc*sizeof(float),
                                      &dC[masterdev][0], lddc*sizeof(float),
                                      m*sizeof(float), n,
