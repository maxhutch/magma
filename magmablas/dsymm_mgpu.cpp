/*
    -- MAGMA (version 1.5.0-beta3) --
       ...
       @generated from zhemm_mgpu.cpp normal z -> d, Fri Jul 18 17:34:13 2014
       @author Mark Gates
       @author Azzam Haidar
    ...
    magma_int_t gnode[MagmaMaxGPUs][MagmaMaxGPUs+2], magma_int_t nbcmplx )
{
    ...
    if(blockoffset>0){
        ...
        magmablas_dsymmetrize_tiles(  MagmaLower,  fstblksiz,  dA(stdev, offset, myblkoffst*nb+blockoffset),  ldda,  1,  ngpu*nb,  nb  );         
    }
    ...
    for( magma_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_setdevice( dev );
        magmablasSetKernelStream( streams[ dev ][ 0 ] );
        magmablas_dsymmetrize_tiles(  MagmaLower,  nb,  dA(dev, newoffset+devperm*nb, myblkoffst*nb),  ldda,  myblk,  ngpu*nb,  nb  );
        if(remm%nb>0){
            ...
            if(devperm==devlstblk)
                magmablas_dsymmetrize(  MagmaLower,  remm % nb,  dA(dev, newoffset+nbblk*nb, myblkoffst*nb+ myblk*nb),  ldda );  // last partial tile
        }
    }
...
/*
    ...
    magma_setdevice( 0 );
    magmablasSetKernelStream( streams[ dev ][ 0 ] );
    //magma_dgetmatrix( siz, siz, dA[0], ldda, R, siz );
    FILE *trace_file;
...
*/
        ...
        for( magma_int_t i = fstblksiz; i < m; i += nb ) {
            ...
            for( magma_int_t dev = 0; dev < ngpu; ++dev ) {
                ...
                if(myrowsize>0){
                    ...
                    magma_dgemm( MagmaTrans, MagmaNoTrans, myrowsize, n, ib,
    ...
    for( magma_int_t i = fstblksiz; i < m; i += nb ) {
        ...
        if(ngpu==1){
            ...
            // magmablasSetKernelStream( streams[ dev ][ 0 ] );
            magma_dgemm( MagmaTrans, MagmaNoTrans, i, n, ib,
            ...
            for( magma_int_t idev = 0; idev < myngpu; ++idev ) {
                ...
                if(mycolsize>0){
                    ...
                    if(dev!=masterdev){
                        ...
                        cudaMemcpy2DAsync(&dwork2[masterdev][maxgsize*dev], m*sizeof(double),
                                          &dC[dev][0], lddc*sizeof(double),
                                          m*sizeof(double), n,
            ...
            for( magma_int_t k = 0; k < nbcmplx; ++k ) {
                if(k!=cmplxid){
                    ...
                    if(gmaster!=-1){ //real is active
                        ...
                        cudaMemcpy2DAsync(&dwork2[gmaster][maxgsize*masterdev], m*sizeof(double),
                                          &dC[masterdev][0], lddc*sizeof(double),
                                          m*sizeof(double), n,
        ...
        if(masterdev!=-1){ 
            ...
            for( magma_int_t l = 0; l < myngpu; ++l ) {
                ...
                if((lcdev!=masterdev)&&(lccolsize>0)){
                    ...
                    magma_queue_wait_event(streams[ masterdev ][ 0 ], redevents[masterdev][masterdev]);
                    cudaMemcpy2DAsync(&dC[lcdev][0], lddc*sizeof(double),
                                      &dC[masterdev][0], lddc*sizeof(double),
                                      m*sizeof(double), n,
