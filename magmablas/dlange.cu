/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated d Tue Aug 13 16:45:12 2013
...
magma_dlange_generic(const double *A, double *C, int M, int N, int lda , int N_mod_4) 
{
...
extern "C" void
magmablas_dlange_64_64_16_4(const double *A, double *C, 
                            int M, int N, int lda,int tree_depth)
{
        ...
        if( M %64 == 0  && N %64 == 0 ){ 
                magma_dlange_special<<< grid, threads, 0, magma_stream >>> ( A, C , M , N , lda);
        }
        else{
                int N_mod_4 = N % 4 ;
                N = N - N_mod_4 ;  
                magma_dlange_generic<<< grid, threads, 0, magma_stream >>> ( A, C , M , N , lda , N_mod_4);
        }
}
