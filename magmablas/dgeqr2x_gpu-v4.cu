/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zgeqr2x_gpu-v4.cu normal z -> d, Fri Sep 11 18:29:22 2015
    ...
    for (magma_int_t b=0; b < k; b += BS) {
        for (i = b; i < min(k, b+BS); ++i) {
            ...
            if (i-b > 0) {
                ...
                if ( i-1 > 0 ) {
                    magma_dgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_stream >>>
                        ( m-i+1, dA(i-1,0), ldda, dA(i-1, i-1), work, dtau+i-1);
                    magma_dtrmv_kernel2<<< i-1, i-1, 0, magma_stream >>>
                ...
                if ( m-b > 0 ) {
                    ...
                    magma_dgemv_kernel2<<< blocks3, threads3, 0, magma_stream >>>
            ...
            if ( i > 0 ) {
                magma_dnrm2_adjust_kernel<<< 1, i, 0, magma_stream >>>(dnorm+i, dA(0, i));
            }
            ...
            if (i == 0) {
                ...
                magmablas_dlacpy(MagmaUpperLower, 1, 1, dtau, 1, dT(0,0), 1);
                magma_dsetmatrix_async(1, 1, &tt, 1, dA(i, i), 1, magma_stream);
            }
        }
        if ( i-1 > 0 ) {
            magma_dgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_stream >>>
