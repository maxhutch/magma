/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zgetf2_kernels.cu normal z -> s, Sat Nov 15 19:53:59 2014
*/
...

{
    ...
    else
    {
        ...
        tree_isamax_kernel_batched<<<grid, zamax>>>(length, x_array, incx, step, lda, ipiv_array, info_array, gbstep, data_pool_array, id_pool_array);

        if( num_blocks > 1)
        {
            // second level tree reduction
            dim3 grid2(1, 1, batchCount);
            tree_isamax_kernel2_batched<<<grid2, zamax>>>(num_blocks, step,  ipiv_array, info_array, gbstep, data_pool_array, id_pool_array);
        }
    }
        ...
        magma_int_t** ipiv_array, magma_int_t *info_array, magma_int_t gbstep, magma_int_t batchCount)
{
    ...
    if(length == 0 ) return 0;
        ...
        isamax_kernel_batched<<< grid, zamax, zamax * (sizeof(float) + sizeof(int)), magma_stream >>>
    ...
    {  
        dim3 grid(1, 1, batchCount);
        int chunk = (length-1)/zamax + 1;
        isamax_kernel_batched<<< grid, zamax, zamax * (sizeof(float) + sizeof(magma_int_t)), magma_stream >>>
                      (length, chunk, x_array, incx, step, lda, ipiv_array, info_array, gbstep);
...
*/
    ...
    dim3 grid(1,1, batchCount);
    sswap_kernel_batched<<< grid, n, 0, magma_stream >>>(n, x_array, incx, step, ipiv_array);
    return 0;
}
...
*/
    ...
    sscal_sger_kernel_batched<<< grid, min(m, MAX_NTHREADS), shared_size, magma_stream>>>(m, n, step, dA_array, lda, info_array, gbstep);
    return 0;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
...
*/
    ...
    dim3 grid(1, 1, batchCount);
    sgetf2trsm_kernel_batched<<< grid, max(n,ib), shared_size, magma_stream>>>(ib, n, dA_array, step, lda);
}
