/*
    -- MAGMA (version 1.5.0-beta1) --
       ...
       @generated from zgetf2.cu normal z -> s, Fri Apr 25 15:05:23 2014
*/
    ...
    magma_int_t* info )
{
    *info = 0;
    ...
    for( j=0; j < min_mn; j++ ) {
        ...
        jp = j - 1 + cublasIsamax(m-j, A(j,j), 1);
        ...
        //if ( A(jp, j) != 0.0) {
            ...
            // TODO: replace with pre-existing BLAS-standard sswap routine,
            // e.g., magmablas_sswap or cublasSswap
            if (jp != j) {
                magma_sswap(n, A, j, jp, lda);
                //magmablas_sswap( n, A(j,0), lda, A(jp,0), lda );
                //cublasSswap( n, A(j,0), lda, A(jp,0), lda );
            }
            
            // Compute elements J+1:M of J-th column.
            if (j < m) {
                magma_sscal_sger(m-j, n-j, A(j, j), lda);
            }
        //}
        //else if (*info == 0) {
        //    *info = j;
...
void magma_sswap(magma_int_t n, float *x, magma_int_t i, magma_int_t j, magma_int_t incx)
{
/*
