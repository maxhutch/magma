/*
    -- MAGMA (version 1.5.0-beta1) --
       ...
       @generated from zlanhe.cu normal z -> s, Fri Apr 25 15:05:22 2014
    ...
    int n_full_block, int n_mod_bs )
{
    ...
    if ( blockIdx.x == n_full_block ) {
        ...
        *************************************************************************/
        ...
        for(int j=0; j < inf_bs; j += 4) {
            if ( tx < n_mod_bs )
                la[ty+j][tx] = A[j*lda];      //MAGMA_S_MUL( MAGMA_S_ONE,  A[j*lda] );  // huh? just A[j*lda]?
            else
                la[ty+j][tx] = MAGMA_S_ZERO;  //MAGMA_S_MUL( MAGMA_S_ZERO, A[j*lda] );  // huh? just 0?
        }
        __syncthreads();
    ...
    if ( blockIdx.x == n_full_block ) {
        ...
        *************************************************************************/
        ...
        /*---------------------------------------------
            ...
            see slansy_inf_kernel_generic_l code above
        ...
        if ( ty == 0 ) {
            for( int j = 0; j < n_mod_bs; j++) {
                res += fabsf( A1[ j + lda * blockIdx.x * inf_bs ] );
            }
        }
        __syncthreads();
...
extern "C" void
slansy_inf(
    magma_uplo_t uplo, int n, const float *A, int lda, float *dwork )
{
    ...
    if ( n % inf_bs == 0 ) {
        if ( uplo == MagmaLower) {
            slansy_inf_kernel_special_l<<< grid, threads, 0, magma_stream >>>
                ( n, A, lda, dwork );
        }
        else {
            slansy_inf_kernel_special_u<<< grid, threads, 0, magma_stream >>>
                ( n, A, lda, dwork);
        }
    }
    else {
        ...
        if ( uplo == MagmaLower) {
            slansy_inf_kernel_generic_l<<< grid, threads, 0, magma_stream >>>
                ( n, A, lda, dwork, n_full_block, n_mod_bs );
        }
        else {
            slansy_inf_kernel_generic_u<<< grid, threads, 0, magma_stream >>>
                ( n, A, lda, dwork, n_full_block, n_mod_bs );
        }
    }
}
    ...
    magma_uplo_t uplo, int n, const float *A, int lda, float *dwork )
{
    ...
    if ( uplo == MagmaLower ) {
        slansy_max_kernel_l<<< grid, threads, 0, magma_stream >>>
            ( n, A, lda, dwork );
    }
    else {
        slansy_max_kernel_u<<< grid, threads, 0, magma_stream >>>
            ( n, A, lda, dwork );
    }
}
