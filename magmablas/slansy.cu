/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated s Wed Aug 14 12:16:38 2013
...
__global__ void
l_slansy_special (int n, const float* A, int lda,  float *y){
  int tx = threadIdx.x ; 
  int ty = threadIdx.y ; 
...
__global__ void
l_slansy_generic(int n, const float* A, int lda,  float *y, int m_full_block, 
                 int m_mod_32)
{ 
  int tx = threadIdx.x ; 
  int ty = threadIdx.y ; 
...
__global__ void
u_slansy_generic (int n, const float* A, int lda, float *y, int m_full_block, int m_mod_32){
...
__global__ void
u_slansy_special (int n, const float* A, int lda, float *y ){
  int tx = threadIdx.x ; 
  int ty = threadIdx.y ; 
  int ind = blockIdx.x*  dgemv_bs + tx ;
  float res = 0.;
...
extern "C" void mslansy (char uplo, int m, const float *A, int lda,  float *Y  )
{
/*
Note:
...
*/
    ...
    if( m % dgemv_bs == 0 ) {
            if( uplo == 'L' || uplo == 'l'){        
                    l_slansy_special <<< grid, threads, 0, magma_stream >>> (m, A, lda, Y);
            }
            else{
                    u_slansy_special <<< grid, threads, 0, magma_stream >>> (m, A, lda,  Y);
            } 
            ...
            int  m_mod_32 = m%32 ;  
            if( uplo == 'L' || uplo == 'l'){
                    l_slansy_generic <<< grid, threads, 0, magma_stream >>> (m, A, lda, Y, m_full_block, m_mod_32);
            }        
            else{
                    u_slansy_generic <<< grid, threads, 0, magma_stream >>> (m, A, lda, Y, m_full_block, m_mod_32);
            }        
    }
}
...
__global__ void
l_slansy_max (int m, const float* A, int lda,  float *y){
    int tx  = threadIdx.x ;
    int ind =  blockIdx.x * slansy_bs + tx ;
    float res = 0., res1;

    int break_d = blockIdx.x* slansy_bs;

    if (ind < m)
    {
       A += ind;

       for(int i=0; i<break_d; i += slansy_bs ){
          #pragma unroll 8
          for(int j=0; j< slansy_bs; j++){
             res1 = fabsf(A[j*lda]);
             res = fmax(res,res1);
          }
...
__global__ void
u_slansy_max (int m, const float* A, int lda,  float *y){
    int ind =  blockIdx.x * slansy_bs + threadIdx.x ;
    float res = 0.;

    A += ind;
    if (ind < m){
      for(int j=m-1; j>= ind; j--)
         res = fmax(res, fabsf(A[j*lda]));
...
extern "C" void slansy_max (char uplo, int m, const float *A, int lda, float *y){
    int blocks;
    if (m % slansy_bs==0)
        blocks = m/ slansy_bs;
    else
        blocks = m/ slansy_bs + 1;

    dim3 grid(blocks, 1, 1);
    dim3 threads(slansy_bs, 1, 1);

    if( uplo == 'L' || uplo == 'l'){
      l_slansy_max <<< grid, threads, 0, magma_stream >>> (m, A, lda, y);
    }
    else{
      u_slansy_max <<< grid, threads, 0, magma_stream >>> (m, A, lda, y);
    }
}
