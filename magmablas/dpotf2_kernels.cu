        ...
        int localstep, int gbstep, magma_int_t *info_array, const int batchCount)
{
    ...
    dpotf2_smlpout_fixwidth_device(m, dA_array[batchid]+localstep, dA_array[batchid]+localstep+localstep*lda, lda, localstep, gbstep, &(info_array[batchid]));
}
        ...
        int localstep, int gbstep, magma_int_t *info_array, const int batchCount)
{
    const int batchid = blockIdx.z * blockDim.y + threadIdx.y;
    if (batchid >= batchCount) return;
    dpotf2_smlpout_anywidth_device(m, n, dA_array[batchid]+localstep, dA_array[batchid]+localstep+localstep*lda, lda, localstep, gbstep, &(info_array[batchid]));
}
/////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for (magma_int_t j = 0; j < n; j += POTF2_NB) {
        ...
        if (ib == POTF2_NB)
        {
            dpotf2_smlpout_fixwidth_kernel_batched<<<dimGrid, threads, shared_mem_size, queue >>>(rows, dA_array, lda, j, gbstep, info_array, batchCount);
        } else {
            dpotf2_smlpout_anywidth_kernel_batched<<<dimGrid, threads, shared_mem_size, queue >>>(rows, ib, dA_array, lda, j, gbstep, info_array, batchCount);
        }
    }
