/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from testing/testing_zungbr.cpp normal z -> d, Fri Jan 22 21:42:48 2016
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      for( int ivect = 0; ivect < 2; ++ivect ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tauq,   double, min_mn );
            TESTING_MALLOC_CPU( taup,   double, min_mn );
            ...
            if (info != 0) {
                printf("magma_dorgbr returned error %d: %s.\n",
                       (int) info, magma_strerror( info ));
            }
