/*
    -- MAGMA (version 2.1.0) --
       ...
       @generated from testing/testing_zher2k_batched.cpp, normal z -> c, Tue Aug 30 09:39:16 2016
       @author Mark Gates
       @author Azzam Haidar
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i=0; i < batchCount; i++){
                magma_cmake_hermitian( N, h_C + i * ldc * N, ldc );
            }
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++)
                {
                    ...
                    Cnorm = safe_lapackf77_clanhe( "fro", lapack_uplo_const(opts.uplo), &N, h_C + s*C_batchSize, &ldc, work );

                    // ----- magma error
                    blasf77_caxpy( &C_batchSize, &c_neg_one, h_C + s*C_batchSize, &ione, h_Cmagma + s*C_batchSize, &ione );
                    float err = safe_lapackf77_clanhe( "fro", lapack_uplo_const(opts.uplo), &N, h_Cmagma + s*C_batchSize, &ldc, work ) / Cnorm;
