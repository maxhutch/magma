/*
   ...
   @generated from testing_zgetri_batched.cpp normal z -> d, Sat Nov 15 19:54:18 2014
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            dset_pointer(dinvA_array, d_invA, ldda, 0, 0, ldda * N, batchCount);
            set_ipointer(dipiv_array, d_ipiv, 1, 0, 0, min(M,N), batchCount);

            gpu_time = magma_sync_wtime(0);
            info1 = magma_dgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount);
            info2 = magma_dgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount);
