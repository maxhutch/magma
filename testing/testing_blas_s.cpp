/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from testing/testing_blas_z.cpp normal z -> s, Fri Jan 22 21:42:34 2016
       @author Mark Gates
...
int main( int argc, char** argv )
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        TESTING_MALLOC_PIN( A,   float, size );
        TESTING_MALLOC_PIN( B,   float, size );
        TESTING_MALLOC_PIN( C,   float, size );
        TESTING_MALLOC_PIN( C2,  float, size );
        TESTING_MALLOC_PIN( LU,  float, size );
        
        TESTING_MALLOC_DEV( dA,  float, size );
        TESTING_MALLOC_DEV( dB,  float, size );
        TESTING_MALLOC_DEV( dC1, float, size );
        TESTING_MALLOC_DEV( dC2, float, size );
