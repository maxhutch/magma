/*
    -- MAGMA (version 2.0.0-beta2) --
       ...
       @generated from testing/testing_ztrtri.cpp normal z -> s, Wed Jan  6 17:59:48 2016
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (info != 0)
                printf("magma_strtri returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                /* =====================================================================
                   ...
                   =================================================================== */
                norm  = lapackf77_slantr("f", lapack_uplo_const(opts.uplo), MagmaNonUnitStr, &N, &N, h_A, &N, work);
                blasf77_saxpy(&n2, &c_neg_one, h_A, &ione, h_R, &ione);
                error = lapackf77_slantr("f", lapack_uplo_const(opts.uplo), MagmaNonUnitStr, &N, &N, h_R, &N, work) / norm;
                bool okay = (error < tol);
                status += ! okay;
