/*
    -- MAGMA (version 2.1.0) --
       ...
       @generated from testing/testing_zher2k_vbatched.cpp, normal z -> c, Tue Aug 30 09:39:19 2016
       @author Mark Gates
       @author Azzam Haidar
...
int main( int argc, char** argv)
{
    ...
    float          magma_error, magma_err, Cnorm, work[1];
    magma_int_t N, K;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i=0; i < batchCount; i++)
            {
               magma_cmake_hermitian( h_N[i], h_C_tmp, h_ldc[i] ); 
            ...
            h_A_array[0] = d_A; h_B_array[0] = d_B; h_C_array[0] = d_C;
            ...
            if ( opts.lapack ) {
                ...
                h_A_array[0] = h_A; h_B_array[0] = h_B; h_C_array[0] = h_C; 
                for(int i = 1; i < batchCount; i++){
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++)
                {
                    ...
                    Cnorm = safe_lapackf77_clanhe( "fro", lapack_uplo_const(opts.uplo), &h_N[s], h_C_tmp, &h_ldc[s], work );

                    blasf77_caxpy( &C_batchSize, &c_neg_one, h_C_tmp, &ione, h_Cmagma_tmp, &ione );
                    magma_err = safe_lapackf77_clanhe( "fro", lapack_uplo_const(opts.uplo), &h_N[s], h_Cmagma_tmp, &h_ldc[s], work ) / Cnorm;
