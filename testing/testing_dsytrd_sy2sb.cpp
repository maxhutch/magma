/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from testing_zhetrd_he2hb.cpp normal z -> d, Fri Sep 11 18:29:39 2015
...
int main( int argc, char** argv)
{
    ...
    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_get_dsytrd_sy2sb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //TESTING_MALLOC_DEV( dT1, double, (2*min(N,N) + roundup( N, 32 ))*NB );
            ...
            /*
            magma_dsytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_dsytrd_bsy2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_dsytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_dsytrd_bsy2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
            ...
            if (ngpu == 1) {
                ...
                magma_dsyevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                magma_dmalloc_cpu( &work2, lwork2 );
                magma_dmalloc_cpu( &rwork2, N );
                magma_dmalloc_cpu( &D2, N );
                magma_dmalloc_cpu( &AINIT, N*lda );
                ...
                */
                ...
                // TODO free this memory !
                magma_dmalloc_cpu( &WORKAJETER, (2* N * N + N)  );
