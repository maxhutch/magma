...
int main( int argc, char** argv) 
{
    ...
    for( i = 0; i < ntest; ++i ) {
        ...
        {
            float *d_R, *dtau, *d_work;

            /* allocate gpu workspaces */
            TESTING_DEVALLOC( d_R,    float, lda*N );
            TESTING_DEVALLOC( dtau,   float, min_mn);
            TESTING_DEVALLOC( d_work, float, lwork );

            /* copy A to gpu */
            magma_ssetmatrix( M, N, h_R, lda, d_R, lda );

            /* call gpu-interface */
            gpu_time = magma_wtime();
            #if defined(PRECISION_z) || defined(PRECISION_c)
            magma_sgeqp3_gpu(M, N, d_R, lda, jpvt, dtau, d_work, lwork, drwork, &info);
            #else
            magma_sgeqp3_gpu(M, N, d_R, lda, jpvt, dtau, d_work, lwork, &info);
            #endif
            cudaDeviceSynchronize();
            gpu_time = magma_wtime() - gpu_time;

            /* copy outputs to cpu */
            magma_sgetmatrix( M, N, d_R, lda, h_R, lda );
        ...
        if ( checkres ) {
            ...
            error = lapackf77_sqpt01( &M, &N, &minmn, h_A, h_R, &lda, 
