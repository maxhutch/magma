/*
    ...
    @generated d Wed Aug 14 12:18:11 2013
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if(opts.warmup){
                ...
                magma_dsygvdx_2stage(itype, jobz, range, uplo,
            ...
            if ( checkres ) {
                ...
                result[0] = 1.;
                result[0] /= lapackf77_dlansy("1",&uplo, &N, h_A, &N, rwork);
                result[0] /= lapackf77_dlange("1",&N , &m1, h_R, &N, rwork);

                if (itype == 1){
                    blasf77_dsymm("L", &uplo, &N, &m1, &c_one, h_A, &N, h_R, &N, &c_zero, h_work, &N);
                    for(int i=0; i<m1; ++i)
                        blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_dsymm("L", &uplo, &N, &m1, &c_neg_one, h_B, &N, h_R, &N, &c_one, h_work, &N);
                    result[0] *= lapackf77_dlange("1", &N, &m1, h_work, &N, rwork)/N;
                }
                else if (itype == 2){
                    blasf77_dsymm("L", &uplo, &N, &m1, &c_one, h_B, &N, h_R, &N, &c_zero, h_work, &N);
                    for(int i=0; i<m1; ++i)
                        blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_dsymm("L", &uplo, &N, &m1, &c_one, h_A, &N, h_work, &N, &c_neg_one, h_R, &N);
                    ...
                    for(int i=0; i<m1; ++i)
                        blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_dsymm("L", &uplo, &N, &m1, &c_one, h_B, &N, h_work, &N, &c_neg_one, h_R, &N);
                    result[0] *= lapackf77_dlange("1", &N, &m1, h_R, &N, rwork)/N;
                }
                ...
                magma_int_t m2 = m1;
                lapackf77_dsygvd(&itype, "N", &uplo, &N,
                              h_R, &N, h_S, &N, w2,
                              h_work, &lwork,
#if defined(PRECISION_z) || defined(PRECISION_c)
                              rwork, &lrwork,
