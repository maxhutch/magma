/*
    -- MAGMA (version 2.1.0) --
       ...
       @generated from testing/testing_zher2k_vbatched.cpp, normal z -> d, Tue Aug 30 09:39:19 2016
       @author Mark Gates
       @author Azzam Haidar
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i=0; i < batchCount; i++)
            {
               magma_dmake_symmetric( h_N[i], h_C_tmp, h_ldc[i] ); 
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++)
                {
                    ...
                    Cnorm = safe_lapackf77_dlansy( "fro", lapack_uplo_const(opts.uplo), &h_N[s], h_C_tmp, &h_ldc[s], work );

                    blasf77_daxpy( &C_batchSize, &c_neg_one, h_C_tmp, &ione, h_Cmagma_tmp, &ione );
                    magma_err = safe_lapackf77_dlansy( "fro", lapack_uplo_const(opts.uplo), &h_N[s], h_Cmagma_tmp, &h_ldc[s], work ) / Cnorm;
