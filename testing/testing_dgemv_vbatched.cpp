/*
    -- MAGMA (version 2.1.0) --
       ...
       @generated from testing/testing_zgemv_vbatched.cpp, normal z -> d, Tue Aug 30 09:39:19 2016
       @author Mark Gates
       @author Azzam Haidar
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                double* hY_tmp = h_Y;
                double* h_Ymagma_tmp = h_Ymagma;
                for (int s=0; s < batchCount; s++){
                     blasf77_daxpy( &Yn[s], &c_neg_one, hY_tmp, &h_incy[s], h_Ymagma_tmp, &h_incy[s] );
                     Ynorm = lapackf77_dlange( "F", &ione, &Yn[s], hY_tmp, &h_incy[s], work );
                     magma_err = lapackf77_dlange( "F", &ione, &Yn[s], h_Ymagma_tmp, &h_incy[s], work ) / Ynorm;
