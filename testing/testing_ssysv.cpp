/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from testing_zhesv.cpp normal z -> s, Sat Nov 15 19:54:18 2014
...
void init_matrix( int m, int n, float *h_A, magma_int_t lda )
{
    ...
    for (int i=0; i<n; i++) h_A[i+i*lda] = MAGMA_S_MAKE(MAGMA_S_REAL(h_A[i+i*lda]), 0.0);
}
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_ssysv((upper ? MagmaUpperStr: MagmaLowerStr), &N, &opts.nrhs, 
            ...
            magma_ssysv( uplo, N, opts.nrhs, h_A, lda, ipiv, h_X, ldb, &info);
