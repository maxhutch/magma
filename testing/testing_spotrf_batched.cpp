/*
   ...
   @generated from testing_zpotrf_batched.cpp normal z -> s, Fri Sep 11 18:29:39 2015
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            {
                if (cpu_info[i] != 0 ) {
                    printf("magma_spotrf_batched matrix %d returned diag error %d\n", i, (int)cpu_info[i] );
                    status = -1;
                }
            }
            if (info != 0) {
                //printf("magma_spotrf_batched returned argument error %d: %s.\n", (int) info, magma_strerror( info ));
                status = -1;
            }                
            if (status == -1)
                goto cleanup;
            ...
            if ( opts.lapack ) {
                ...
                /* =====================================================================
                   ...
                   =================================================================== */
                magma_sgetmatrix( N, columns, d_A, ldda, h_R, lda );
                magma_int_t NN = lda*N;
                const char* uplo = lapack_uplo_const(opts.uplo);
                error = 0;
                for (int i=0; i < batchCount; i++)
                {
                    float err = lapackf77_slansy("f", uplo, &N, h_A + i * lda*N, &lda, work);
                    blasf77_saxpy(&NN, &c_neg_one, h_A + i * lda*N, &ione, h_R + i  * lda*N, &ione);
                    err = lapackf77_slansy("f", uplo, &N, h_R + i * lda*N, &lda, work) / err;
