/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from testing_ztrsm_batched.cpp normal z -> d, Sat Nov 15 19:54:18 2014
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            dset_pointer(dwork_array, dwork, lddb, 0, 0, dwork_batchSize, batchCount);
            dset_pointer(dinvA_array, dinvA, ((Ak+TRI_NB-1)/TRI_NB)*TRI_NB, 0, 0, dinvA_batchSize, batchCount);

            memset(h_Bmagma, 0, batchCount*ldb*N*sizeof(double));
            cudaMemset(dwork, 0, dwork_batchSize * batchCount*sizeof(double));
            ...
            dset_pointer(d_A_array, d_A, ldda, 0, 0, ldda*Ak, batchCount);
            dset_pointer(d_B_array, d_B, lddb, 0, 0, lddb*N, batchCount);
            dset_pointer(dwork_array, dwork, lddb, 0, 0, lddb*N, batchCount);

            magma_time = magma_sync_wtime( NULL );
#if 1
            magmablas_dtrsm_outofplace_batched(opts.side, opts.uplo, opts.transA, opts.diag, 1,
                    M, N,
                    alpha,
                    d_A_array,    ldda, // dA
                    d_B_array,    lddb, // dB
            ...
            magmablas_dtrsm_batched( opts.side, opts.uplo, opts.transA, opts.diag, 
                             M, N,
                             alpha, 
                             d_A_array, ldda,
                             d_B_array, lddb, 
            ...
            cublasDtrsmBatched( opts.handle, cublas_side_const(opts.side), cublas_uplo_const(opts.uplo),
                         cublas_trans_const(opts.transA), cublas_diag_const(opts.diag),
                         M, N, 
                         &alpha, (const double**)d_A_array, ldda,
