/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from testing_zlacpy.cpp normal z -> d, Sat Nov 15 19:54:18 2014
       @author Mark Gates
*/
    ...
    for( int iuplo = 0; iuplo < 3; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //magmablas_dlacpy( uplo[iuplo], M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magmablas_dlacpy( uplo[iuplo], M, N, d_A, ldda, d_B, lddb );
            ...
            if ( opts.verbose ) {
                ...
                printf( "B= " );  magma_dprint(     M, N, h_B, lda );
                printf( "dA=" );  magma_dprint_gpu( M, N, d_A, ldda );
                printf( "dB=" );  magma_dprint_gpu( M, N, d_B, ldda );
            }
