/*
   ...
   @generated from testing_zgetri_batched.cpp normal z -> c, Fri Sep 11 18:29:39 2015
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {    
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_csetmatrix( N, columns, h_R, lda, d_A, ldda );

            /* ====================================================================
               Performs operation using MAGMA
               =================================================================== */
            cset_pointer(dA_array, d_A, ldda, 0, 0, ldda * N, batchCount, opts.queue);
            cset_pointer(dinvA_array, d_invA, ldda, 0, 0, ldda * N, batchCount, opts.queue);
            ...
            if ( opts.check ) {
                ...
                magma_cgetmatrix( N, N*batchCount, d_invA, ldda, h_Ainv, lda );
                error = 0;
                for (magma_int_t i=0; i < batchCount; i++)
                {
