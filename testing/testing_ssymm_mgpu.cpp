/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from testing/testing_zhemm_mgpu.cpp normal z -> s, Fri Jan 22 21:42:34 2016
      ...
      for( int offset = 0; offset < N; offset += min(N,nb) ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( d = 0; d < opts.ngpu; ++d ) {
                ...
                TESTING_MALLOC_DEV( dB[d],    float, lddb*N      );
                TESTING_MALLOC_DEV( dC[d],    float, lddc*N      );
            ...
            for( d = 0; d < opts.ngpu; ++d ) {
                ...
                magma_ssetmatrix( M, N, hC, lda, dC[d], ldda );
            }
            ...
            if ( opts.check && iter == 0 ) {
                ...
                magma_ssetmatrix( M, N, hB, lda, dB[0], ldda );
                magma_ssetmatrix( M, N, hC, lda, dwork[0], ldda );
            ...
            if ( opts.check ) {
                ...
                for (magma_int_t dev=0; dev < opts.ngpu; ++dev) {
                    magma_setdevice( dev );
                    magma_sgetmatrix( M, N, dC[dev], ldda, hR, lda );
