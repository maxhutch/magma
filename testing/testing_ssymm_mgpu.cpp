...
int main( int argc, char** argv)
{
    ...
    TESTING_MALLOC( hA, float, lda*m );
    TESTING_MALLOC( hX, float, lda*n );
    ...
    for( int d = 0; d < ngpu; ++d ) {
        ...
        cudaSetDevice( d );
        TESTING_DEVALLOC( dA[d], float, ldda*mlocal );
        TESTING_DEVALLOC( dX[d], float, ldda*n      );
        TESTING_DEVALLOC( dB[d], float, ldda*n      );
        TESTING_DEVALLOC( dwork[d], float, ldda*n*3  );
        TESTING_HOSTALLOC( hwork[d], float, lda*n );
    ...
    TESTING_HOSTALLOC( hwork[ngpu], float, lda*n );



    if ( checkres ) {
    cudaSetDevice( 0 );
    TESTING_DEVALLOC( dA2, float, ldda*m );
    }
...
//            printf("\n\n\n\n\n");
    ...
    for( int j = 0; j < count; ++j ) {
        ...
        lda  = m;
        ldda = ((m + 31)/32)*32;
        gflops = FLOPS_SSYMM( MagmaLeft, (float)msiz, (float)n ) / 1e9;

        size = lda*m;
        lapackf77_slarnv( &ione, iseed, &size, hA );
        // make diagonal real
        for( int i = 0; i < m; ++i ) {
            hA[i + i*lda] = MAGMA_S_MAKE( MAGMA_S_REAL( hA[i+i*lda] ), 0. );
        }
        size = lda*n;
        ...
        /* ====================================================================
           ...
           =================================================================== */
        magma_ssetmatrix_1D_col_bcyclic( m, m, hA, lda, dA, ldda, ngpu, nb );
        for( int d = 0; d < ngpu; ++d ) {
            cudaSetDevice( d );
        ...
        */
        ...
        if ( checkres ) {
            // store ||A||*||X||
            errorbis  = lapackf77_slange("fro", &msiz, &msiz, hA+offst*lda+offst, &lda, work );
            errorbis *= lapackf77_slange("fro", &msiz, &n, hX, &lda, work );
            ...
            blasf77_ssymm( "Left", "Lower", &msiz, &n,
            ...
            */
            ...
            for(magma_int_t dev=0; dev<ngpu; ++dev){
                ...
                error = lapackf77_slange("fro", &msiz, &n, hR, &lda, work) / errorbis;
