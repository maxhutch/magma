/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from testing/testing_zhemm_mgpu.cpp normal z -> c, Fri Jan 22 21:42:34 2016
      ...
      for( int offset = 0; offset < N; offset += min(N,nb) ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( d = 0; d < opts.ngpu; ++d ) {
                ...
                TESTING_MALLOC_DEV( dB[d],    magmaFloatComplex, lddb*N      );
                TESTING_MALLOC_DEV( dC[d],    magmaFloatComplex, lddc*N      );
                TESTING_MALLOC_DEV( dwork[d], magmaFloatComplex, dworksiz    );
            }
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_csetmatrix_1D_col_bcyclic( M, M, hA, lda, dA, ldda, opts.ngpu, nb );
            for( d = 0; d < opts.ngpu; ++d ) {
                magma_setdevice( d );
                ...
                magma_csetmatrix( M, N, hC, lda, dC[d], ldda );
            }
            ...
            if ( opts.check && iter == 0 ) {
                ...
                magma_csetmatrix( M, N, hB, lda, dB[0], ldda );
                magma_csetmatrix( M, N, hC, lda, dwork[0], ldda );
            ...
            if ( opts.check ) {
                ...
                for (magma_int_t dev=0; dev < opts.ngpu; ++dev) {
                    magma_setdevice( dev );
                    magma_cgetmatrix( M, N, dC[dev], ldda, hR, lda );
