/*
    -- MAGMA (version 2.0.2) --
       ...
       @generated from testing/testing_zhemm_mgpu.cpp normal z -> c, Mon May  2 23:31:06 2016
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex alpha     = MAGMA_C_MAKE( 3.456, 5.678 );
    magmaFloatComplex beta      = MAGMA_C_MAKE( 1.234, 2.456 );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int offset = 0; offset < N; offset += min(N,nb) ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gflops = FLOPS_CHEMM( MagmaLeft, (float)msize, (float)N ) / 1e9;
            
            magma_int_t dworksiz = lddc*N + (M*N)*opts.ngpu;
            
            TESTING_MALLOC_CPU( hA, magmaFloatComplex, lda*M );
            TESTING_MALLOC_CPU( hB, magmaFloatComplex, ldb*N );
            TESTING_MALLOC_CPU( hC, magmaFloatComplex, ldc*N );
            
            TESTING_MALLOC_PIN( hR, magmaFloatComplex, ldc*N );

            for( dev = 0; dev < opts.ngpu; ++dev ) {
                magma_int_t mlocal = ((M / nb) / opts.ngpu + 1) * nb;
                magma_setdevice( dev );
                TESTING_MALLOC_DEV( dA[dev],    magmaFloatComplex, ldda*mlocal );
                TESTING_MALLOC_DEV( dB[dev],    magmaFloatComplex, lddb*N      );
                TESTING_MALLOC_DEV( dC[dev],    magmaFloatComplex, lddc*N      );
                TESTING_MALLOC_DEV( dwork[dev], magmaFloatComplex, dworksiz    );
            }
            ...
            lapackf77_clacpy( "Full", &M, &N, hC, &ldc, hR, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA
               =================================================================== */
            magma_csetmatrix_1D_col_bcyclic( M, M, hA, lda, dA, ldda, opts.ngpu, nb, queues0 );
            for( dev = 0; dev < opts.ngpu; ++dev ) {
                magma_setdevice( dev );
                magma_csetmatrix( M, N, hB, lda, dB[dev], ldda, opts.queue );
                // since when offset != 0, the GPU that does beta*C may not be 0,
                // send initial hC to all GPUs.
                magma_csetmatrix( M, N, hC, lda, dC[dev], ldda, opts.queue );
            }
            ...
            #ifdef TRACING
            char buf[80];
            snprintf( buf, sizeof(buf), "chemm-m%d-n%d-nb%d-ngpu%d-run%d.svg",
            ...
            if ( opts.check && iter == 0 ) {
                magma_setdevice( 0 );
                magma_csetmatrix( M, M, hA, lda, dA2, ldda, opts.queue );
                magma_csetmatrix( M, N, hB, lda, dB[0], ldda, opts.queue );
                magma_csetmatrix( M, N, hC, lda, dwork[0], ldda, opts.queue );
            ...
            if ( opts.check ) {
                // store ||A||*||B||
                Anorm  = lapackf77_clange("fro", &msize, &msize, hA + offset + offset*lda, &lda, work );
                Anorm *= lapackf77_clange("fro", &msize, &N, hB, &lda, work );
                
                //printf( "A =" ); magma_cprint( M, M, hA, lda );
                //printf( "B =" ); magma_cprint( M, N, hB, lda );
                //printf( "C =" ); magma_cprint( M, N, hC, lda );
                ...
                for (dev=0; dev < opts.ngpu; ++dev) {
                    magma_setdevice( dev );
                    magma_cgetmatrix( M, N, dC[dev], ldda, hR, lda, opts.queue );
                    
                    // compute relative error ||R||/||A||*||B||, where R := C_magma - C_lapack = R - C
                    size = ldc*N;
                    blasf77_caxpy( &size, &c_neg_one, hC, &ione, hR, &ione );
                    error = lapackf77_clange("fro", &msize, &N, hR, &lda, work) / Anorm;
