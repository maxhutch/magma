/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated c Wed Aug 14 12:18:01 2013
...
int main( int argc, char** argv)
{
    ...
    cuFloatComplex calpha    = MAGMA_C_MAKE( 3.456, 5.678 );
    cuFloatComplex cbeta     = MAGMA_C_MAKE( 1.234, 2.456 );
    ...
    cuFloatComplex *hA, *hX, *hB, *hR;
    cuFloatComplex *dA[MagmaMaxGPUs], *dX[MagmaMaxGPUs], *dB[MagmaMaxGPUs], *dwork[MagmaMaxGPUs], *hwork[MagmaMaxGPUs+1];
    cuFloatComplex *dA2;
    ...
    TESTING_MALLOC( hA, cuFloatComplex, lda*m );
    TESTING_MALLOC( hX, cuFloatComplex, lda*n );
    TESTING_MALLOC( hB, cuFloatComplex, lda*n );
    TESTING_HOSTALLOC( hR, cuFloatComplex, lda*n );
    ...
    for( int d = 0; d < ngpu; ++d ) {
        ...
        cudaSetDevice( d );
        TESTING_DEVALLOC( dA[d], cuFloatComplex, ldda*mlocal );
        TESTING_DEVALLOC( dX[d], cuFloatComplex, ldda*n      );
        TESTING_DEVALLOC( dB[d], cuFloatComplex, ldda*n      );
        TESTING_DEVALLOC( dwork[d], cuFloatComplex, ldda*n*3  );
        TESTING_HOSTALLOC( hwork[d], cuFloatComplex, lda*n );
    ...
    TESTING_HOSTALLOC( hwork[ngpu], cuFloatComplex, lda*n );



    if ( checkres ) {
    cudaSetDevice( 0 );
    TESTING_DEVALLOC( dA2, cuFloatComplex, ldda*m );
    }
...
//            printf("\n\n\n\n\n");
    ...
    for( int j = 0; j < count; ++j ) {
        ...
        // make diagonal real
        for( int i = 0; i < m; ++i ) {
            hA[i + i*lda] = MAGMA_C_MAKE( MAGMA_C_REAL( hA[i+i*lda] ), 0. );
        }
        size = lda*n;
        ...
        //memset(hR, 0, lda*n*sizeof(cuFloatComplex));
