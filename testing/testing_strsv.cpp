/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from testing/testing_ztrsv.cpp, normal z -> s, Sun Nov 20 20:20:33 2016
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_sgetrf( &N, &N, hA, &lda, ipiv, &info );
            for( j = 0; j < N; ++j ) {
                for( i = 0; i < j; ++i ) {
                    *hA(i,j) = *hA(j,i);
                }
            ...
            // ||b - Ax|| / (||A||*||x||)
            // error for CUBLAS
            normA = lapackf77_slange( "F", &N, &N, hA, &lda, work );
