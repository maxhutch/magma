...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC( LU,      float, lda*Ak );
            TESTING_MALLOC( LUT,  float, lda*Ak );
            TESTING_MALLOC( h_B,  float, ldb*N  );
            TESTING_MALLOC( h_B1,  float, ldb*N );
            TESTING_MALLOC( h_X1,  float, ldb*N );
            TESTING_MALLOC( h_X2,  float, ldb*N );
            TESTING_MALLOC( h_Bcublas, float, ldb*N  );
            TESTING_MALLOC( h_Bmagma, float, ldb*N  );
