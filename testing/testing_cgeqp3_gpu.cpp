/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated c Tue Aug 13 16:46:09 2013
...
int main( int argc, char** argv) 
{
    ...
    float *drwork, *rwork;
    TESTING_DEVALLOC(   drwork, float, 2*N        +( N+1 )*nb);
    ...
    for( i = 0; i < ntest; ++i ) {
        ...
        {
            magmaFloatComplex *d_R, *dtau, *d_work;

            /* allocate gpu workspaces */
            TESTING_DEVALLOC( d_R,    magmaFloatComplex, lda*N );
            TESTING_DEVALLOC( dtau,   magmaFloatComplex, min_mn);
            TESTING_DEVALLOC( d_work, magmaFloatComplex, lwork );

            /* copy A to gpu */
            magma_csetmatrix( M, N, h_R, lda, d_R, lda );

            /* call gpu-interface */
            gpu_time = magma_wtime();
            #if defined(PRECISION_z) || defined(PRECISION_c)
            magma_cgeqp3_gpu(M, N, d_R, lda, jpvt, dtau, d_work, lwork, drwork, &info);
            #else
            magma_cgeqp3_gpu(M, N, d_R, lda, jpvt, dtau, d_work, lwork, &info);
            #endif
            cudaDeviceSynchronize();
            gpu_time = magma_wtime() - gpu_time;

            /* copy outputs to cpu */
            magma_cgetmatrix( M, N, d_R, lda, h_R, lda );
