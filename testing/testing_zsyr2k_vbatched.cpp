    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for(int i = 0; i < batchCount; i++)
            {
                ...
                gflops += FLOPS_ZHER2K( h_N[i], h_K[i] ) / 1e9;
            }
            ...
            #if 1
            magmablas_zsyr2k_vbatched_max_nocheck(opts.uplo, opts.transA, d_N, d_K, 
                             alpha, d_A_array, d_ldda, 
                                    d_B_array, d_lddb, 
                             beta, d_C_array, d_lddc, 
                             batchCount, max_N, max_K, opts.queue );
            #else
            magmablas_zsyr2k_vbatched(opts.uplo, opts.transA, d_N, d_K, 
                             alpha, d_A_array, d_ldda, 
                                    d_B_array, d_lddb, 
                             beta, d_C_array, d_lddc, 
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++)
                {
                    ...
                    Cnorm = lapackf77_zlansy( "fro", lapack_uplo_const(opts.uplo), &h_N[s], h_C_tmp, &h_ldc[s], work );

                    blasf77_zaxpy( &C_batchSize, &c_neg_one, h_C_tmp, &ione, h_Cmagma_tmp, &ione );
                    magma_err = lapackf77_zlansy( "fro", lapack_uplo_const(opts.uplo), &h_N[s], h_Cmagma_tmp, &h_ldc[s], work ) / Cnorm;
