...
int main( int argc, char** argv)
{
    ...
    double *hA, *hX, *hB, *hR;
    double *dA[MagmaMaxGPUs], *dX[MagmaMaxGPUs], *dB[MagmaMaxGPUs], *dwork[MagmaMaxGPUs], *hwork[MagmaMaxGPUs+1];
    double *dA2;
...
//            printf("\n\n\n\n\n");
    ...
    for( int j = 0; j < count; ++j ) {
        ...
        lda  = m;
        ldda = ((m + 31)/32)*32;
        gflops = FLOPS_DSYMM( MagmaLeft, (double)msiz, (double)n ) / 1e9;
        ...
        /* ====================================================================
           ...
           =================================================================== */
        magma_dsetmatrix_1D_col_bcyclic( m, m, hA, lda, dA, ldda, ngpu, nb );
        for( int d = 0; d < ngpu; ++d ) {
            cudaSetDevice( d );
        ...
        */
        ...
        if ( checkres ) {
            // store ||A||*||X||
            errorbis  = lapackf77_dlange("fro", &msiz, &msiz, hA+offst*lda+offst, &lda, work );
            errorbis *= lapackf77_dlange("fro", &msiz, &n, hX, &lda, work );
            ...
            blasf77_dsymm( "Left", "Lower", &msiz, &n,
            ...
            */
            ...
            for(magma_int_t dev=0; dev<ngpu; ++dev){
                ...
                error = lapackf77_dlange("fro", &msiz, &n, hR, &lda, work) / errorbis;
