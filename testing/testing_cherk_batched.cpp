/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from testing_zherk_batched.cpp normal z -> c, Fri Sep 11 18:29:39 2015
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            {
               magma_cmake_hpd( N, h_C + i * ldc * N, ldc ); // need modification
            }
            ...
            cset_pointer(A_array, d_A, lda, 0, 0, ldda*Ak, batchCount, opts.queue);
            cset_pointer(C_array, d_C, ldc, 0, 0, lddc*N,  batchCount, opts.queue);
            ...
            if ( opts.lapack ) {
                ...
                for (int i=0; i < batchCount; i++)
                {
                    ...
                    float err = lapackf77_clanhe( "fro", lapack_uplo_const(opts.uplo), &N, h_Cmagma+i*ldc*N, &ldc, work ) / Cnorm;
