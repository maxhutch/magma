/*
   ...
   @generated from testing_zgesv_batched.cpp normal z -> s, Sat Nov 15 19:54:18 2014
 */
// includes, system
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            sset_pointer(dB_array, d_B, lddb, 0, 0, lddb*nrhs, batchCount);
            set_ipointer(dipiv_array, dipiv, 1, 0, 0, N, batchCount);

            gpu_time = magma_wtime();
            //magma_sgesv_gpu( N, nrhs, d_A, ldda, ipiv, d_B, lddb, &info );
            info = magma_sgesv_batched(N, nrhs, dA_array, ldda, dipiv_array, dB_array, lddb, dinfo_array, batchCount); 
