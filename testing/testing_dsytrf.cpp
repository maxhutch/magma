/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from testing_zhetrf.cpp normal z -> d, Sat Nov 15 19:54:18 2014
    ...
    magma_int_t *ipiv )
{
    ...
    if (nopiv) {
        ...
        lapackf77_dsytrs( (upper ? MagmaUpperStr: MagmaLowerStr), &n, &ione, A, &lda, ipiv, x, &n, &info );
    }
    if (info != 0)
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_dsytrf((upper ? MagmaUpperStr: MagmaLowerStr), &N, h_A, &lda, ipiv, work, &lwork, &info);
            ...
            if (nopiv) {
                ...
                magma_setdevice(0);
                gpu_time = magma_wtime();
                magma_dsytrf( uplo, N, h_A, lda, ipiv, &info);
                gpu_time = magma_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                ...
                gpu_time = magma_wtime();
                magma_dsytrf_nopiv_gpu( uplo, N, d_A, ldda, &info);
                gpu_time = magma_wtime() - gpu_time;
                ...
                magma_setdevice(0);
                //magma_dsytrf_hybrid( uplo, N, h_A, lda, ipiv, work, lwork, &info);
            }
