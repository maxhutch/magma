/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from testing_ztrsv_batched.cpp normal z -> d, Fri Sep 11 18:29:39 2015
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            dset_pointer(dwork_array, dwork, N, 0, 0, dwork_batchSize, batchCount, opts.queue);

            memset(h_bmagma, 0, batchCount*N*sizeof(double));
            magmablas_dlaset( MagmaFull, N, batchCount, c_zero, c_zero, dwork, N);
            ...
            magma_dsetmatrix( N,  batchCount, h_b, N, d_b, N );

            dset_pointer(d_A_array, d_A, ldda, 0, 0, ldda*Ak, batchCount, opts.queue);
            dset_pointer(d_b_array, d_b, N, 0, 0, N, batchCount, opts.queue);
            dset_pointer(dwork_array, dwork, N, 0, 0, N, batchCount, opts.queue);
            ...
            magma_dgetmatrix( N, batchCount, dwork, N, h_bmagma, N );
            ...
            magma_dgetmatrix( N, batchCount, d_b, N, h_bcublas, N );
