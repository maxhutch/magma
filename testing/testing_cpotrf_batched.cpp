/*
   ...
   @generated from testing_zpotrf_batched.cpp normal z -> c, Fri Sep 11 18:29:39 2015
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                for (int i=0; i < batchCount; i++)
                {
                    float err = lapackf77_clanhe("f", uplo, &N, h_A + i * lda*N, &lda, work);
                    blasf77_caxpy(&NN, &c_neg_one, h_A + i * lda*N, &ione, h_R + i  * lda*N, &ione);
                    err = lapackf77_clanhe("f", uplo, &N, h_R + i * lda*N, &lda, work) / err;
