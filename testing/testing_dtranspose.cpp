/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from testing/testing_ztranspose.cpp normal z -> d, Fri Jan 22 21:42:37 2016
       @author Mark Gates
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      for( int itran = 0; itran < ntrans; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_dsetmatrix( N, M, h_B, ldb, d_B(0,0), lddb );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_dsetmatrix( M, N, h_A, lda, d_A(0,0), ldda );
            magma_dsetmatrix( N, M, h_B, ldb, d_B(0,0), lddb );
            
            magmablasSetKernelStream( opts.queue );
            gpu_time = magma_sync_wtime( opts.queue );
            if ( trans[itran] == MagmaTrans ) {
                //magmablas_dtranspose( M-2, N-2, d_A(1,1), ldda, d_B(1,1), lddb );  // inset by 1 row & col
                magmablas_dtranspose( M, N, d_A(0,0), ldda, d_B(0,0), lddb );
            }
            #ifdef HAVE_CUBLAS
            else {
                //magmablas_dtranspose( M-2, N-2, d_A(1,1), ldda, d_B(1,1), lddb );  // inset by 1 row & col
            ...
            if ( M == N ) {
                ...
                if ( trans[itran] == MagmaTrans ) {
                    //magmablas_dtranspose_inplace( N-2, d_A(1,1), ldda );  // inset by 1 row & col
                    magmablas_dtranspose_inplace( N, d_A(0,0), ldda );
                }
                #ifdef HAVE_CUBLAS
                else {
                    //magmablas_dtranspose_inplace( N-2, d_A(1,1), ldda );  // inset by 1 row & col
