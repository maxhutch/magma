/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from testing_dgeev_m.cpp normal d -> s, Fri Sep 11 18:29:39 2015
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_PIN( h_work, float, lwork2 );
            ...
            if (info != 0)
                printf("magma_sgeev_m returned error %d: %s.\n",
            ...
            if ( opts.check == 2 ) {
                ...
                // this is really slow because it calls magma_sgeev_m multiple times
                float *LRE, DUM;
                ...
                magma_sgeev_m( MagmaVec, MagmaVec,
                               ...
                               h_work, lwork, &info );
                if (info != 0)
                    printf("magma_sgeev_m (case V, V) returned error %d: %s.\n",
                           (int) info, magma_strerror( info ));
                ...
                //               h_work, lwork, &info );
                //if (info != 0)
                //    printf("magma_sgeev_m (case N, N) returned error %d: %s.\n",
                //           (int) info, magma_strerror( info ));
                //
                ...
                magma_sgeev_m( MagmaNoVec, MagmaVec,
                               ...
                               h_work, lwork, &info );
                if (info != 0)
                    printf("magma_sgeev_m (case N, V) returned error %d: %s.\n",
                           (int) info, magma_strerror( info ));
                ...
                magma_sgeev_m( MagmaVec, MagmaNoVec,
                               ...
                               h_work, lwork, &info );
                if (info != 0)
                    printf("magma_sgeev_m (case V, N) returned error %d: %s.\n",
                           (int) info, magma_strerror( info ));
