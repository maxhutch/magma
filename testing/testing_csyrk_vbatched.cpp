/*
    -- MAGMA (version 2.1.0) --
       ...
       @generated from testing/testing_zsyrk_vbatched.cpp, normal z -> c, Tue Aug 30 09:39:19 2016
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex *h_A_tmp, *h_C_tmp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                // MKL (11.1.2) has bug in multi-threaded clansy; use single thread to work around
                ...
                for (int i=0; i < batchCount; i++)
                {
                    ...
                    Cnorm = lapackf77_clansy("fro", lapack_uplo_const(opts.uplo), &h_N[i], h_C_tmp, &h_ldc[i], work);
                    
                    blasf77_caxpy( &sizeC, &c_neg_one, h_C_tmp, &ione, h_Cmagma_tmp, &ione );
                    current_error = lapackf77_clansy( "fro", lapack_uplo_const(opts.uplo), &h_N[i], h_Cmagma_tmp, &h_ldc[i], work ) / Cnorm;
