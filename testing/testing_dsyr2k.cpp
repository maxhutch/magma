/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated d Tue Aug 13 16:45:48 2013
...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC( h_B,  double, ldb*Bk );
            TESTING_MALLOC( h_C,  double, ldc*N  );
            TESTING_MALLOC( h_Ccublas, double, ldc*N  );
            
            TESTING_DEVALLOC( d_A, double, ldda*Ak );
            TESTING_DEVALLOC( d_B, double, lddb*Bk );
            ...
            cublasDsyr2k( opts.uplo, opts.transA, N, K,
                         alpha, d_A, ldda,
                                d_B, lddb,
                         beta,  d_C, lddc );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_dsyr2k( &opts.uplo, &opts.transA, &N, &K,
                               &alpha, h_A, &lda,
                                       h_B, &ldb,
                               &beta,  h_C, &ldc );
