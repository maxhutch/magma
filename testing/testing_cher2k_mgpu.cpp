/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated c Tue Aug 13 16:45:56 2013
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex *dV[MagmaMaxGPUs], *dW[MagmaMaxGPUs], *dA[MagmaMaxGPUs];
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                TESTING_MALLOC( hR2, magmaFloatComplex, lda*n );
                TESTING_MALLOC( hV,  magmaFloatComplex, lda*k*2 );
                //TESTING_MALLOC( hW,  magmaFloatComplex, lda*k );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_int_t nlocal = ((n / nb) / ngpu + 1) * nb;
                    magma_setdevice( d );
                    TESTING_DEVALLOC( dA[d], magmaFloatComplex, ldda*nlocal );
                    TESTING_DEVALLOC( dV[d], magmaFloatComplex, ldda*k*2      );
                    //TESTING_DEVALLOC( dW[d], magmaFloatComplex, ldda*k      );
