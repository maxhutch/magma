/*
    -- MAGMA (version 2.0.2) --
       ...
       @generated from testing/testing_zher2k_mgpu.cpp normal z -> c, Mon May  2 23:31:07 2016
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                for( int dev = 0; dev < ngpu; ++dev ) {
                    ...
                    magma_setdevice( dev );
                    TESTING_MALLOC_DEV( dA[dev], magmaFloatComplex, ldda*nlocal );
                    TESTING_MALLOC_DEV( dV[dev], magmaFloatComplex, ldda*k*2    );
                    //TESTING_MALLOC_DEV( dW[dev], magmaFloatComplex, ldda*k      );
                ...
                /* ====================================================================
                   ...
                   =================================================================== */
                magma_csetmatrix_1D_col_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb, queues0 );
                for( int dev = 0; dev < ngpu; ++dev ) {
                    magma_setdevice( dev );
                    dW[dev] = dV[dev] + ldda*k;
                    magma_csetmatrix( n, k, hV, lda, dV[dev], ldda, opts.queue );
                    magma_csetmatrix( n, k, hW, lda, dW[dev], ldda, opts.queue );
                }
                ...
                magma_cgetmatrix_1D_col_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb, queues0 );
                ...
                if ( opts.lapack || opts.check ) {
                    ...
                    Anorm  = lapackf77_clange("f", &n_offset, &k, hV, &lda, work );
                    Anorm *= lapackf77_clange("f", &n_offset, &k, hW, &lda, work );
                    Anorm += lapackf77_clange("f", &n_offset, &n_offset, &hA[offset + offset*lda], &lda, work );
