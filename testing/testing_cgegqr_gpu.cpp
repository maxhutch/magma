...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex  c_neg_one = MAGMA_C_NEG_ONE, one = MAGMA_C_ONE, zero = MAGMA_C_ZERO;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magmablas_claset( MagmaFull, N, N, ddA, N );
            magmablas_claset( MagmaFull, N, N, d_T, N );
            ...
            // Regenerate R
            // blasf77_cgemm("t", "n", &N, &N, &M, &one, h_R, &M, h_A, &M, &zero, h_work, &N); 
            
            //magma_cprint(N, N, h_work, N);

            blasf77_ctrmm("r", "u", "n", "n", &M, &N, &one, h_rwork, &N, h_R, &M);
