/*
    -- MAGMA (version 2.0.0-beta2) --
       ...
       @generated from testing/testing_zhegvdx.cpp normal z -> d, Wed Jan  6 17:59:50 2016
...
int main( int argc, char** argv)
{
    ...
    if (opts.version == 1) {
        ...
        printf("%%   N     M   GPU Time (sec)   |ABZ-ZD|\n");
    }                                                   
    else if ( opts.version == 3) {                      
        printf("%%   N     M   GPU Time (sec)   |BAZ-ZD|\n");
    }                                     
        printf("%%======================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (opts.version == 2) {
                TESTING_MALLOC_CPU( h_Z,    double, N*lda      );
            ...
            if (opts.version == 1) {
                if (opts.ngpu == 1) {
                    ...
                    magma_dsygvdx_m( abs_ngpu, opts.itype, opts.jobz, MagmaRangeI, opts.uplo,
                                     N, h_R, lda, h_S, lda, vl, vu, il, iu, &m1, w1,
                                     h_work, lwork,
                                     #ifdef COMPLEX
                                     rwork, lrwork,
                ...
                #ifdef COMPLEX
                magma_dsygvr( opts.itype, opts.jobz, MagmaRangeI, opts.uplo,
                ...
                #ifdef COMPLEX
                magma_dsygvx( opts.itype, opts.jobz, MagmaRangeI, opts.uplo,
                              N, h_R, lda, h_S, lda, vl, vu, il, iu, abstol, &m1, w1,
                              h_Z, lda,
                              h_work, lwork,
                              #ifdef COMPLEX
            ...
            if ( opts.check && opts.jobz != MagmaNoVec ) {
                ...
                result[0] /= lapackf77_dlange("1", &N, &m1, h_R, &lda, rwork);
                
                if (opts.itype == 1) {
                    blasf77_dsymm("L", lapack_uplo_const(opts.uplo), &N, &m1, &c_one, h_A, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for (int i=0; i < m1; ++i)
                        blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_dsymm("L", lapack_uplo_const(opts.uplo), &N, &m1, &c_neg_one, h_B, &lda, h_R, &lda, &c_one, h_work, &N);
                    result[0] *= lapackf77_dlange("1", &N, &m1, h_work, &N, rwork)/N;
                }
                else if (opts.itype == 2) {
                    blasf77_dsymm("L", lapack_uplo_const(opts.uplo), &N, &m1, &c_one, h_B, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for (int i=0; i < m1; ++i)
                        blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_dsymm("L", lapack_uplo_const(opts.uplo), &N, &m1, &c_one, h_A, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    result[0] *= lapackf77_dlange("1", &N, &m1, h_R, &lda, rwork)/N;
                }
                else if (opts.itype == 3) {
                    blasf77_dsymm("L", lapack_uplo_const(opts.uplo), &N, &m1, &c_one, h_A, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for (int i=0; i < m1; ++i)
                        blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_dsymm("L", lapack_uplo_const(opts.uplo), &N, &m1, &c_one, h_B, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    result[0] *= lapackf77_dlange("1", &N, &m1, h_R, &lda, rwork)/N;
                }
                ...
                //               N, h_R, lda, h_S, lda, vl, vu, il, iu, &m2, w2,
                //               h_work, lwork,
                //               #ifdef COMPLEX
                //               rwork, lrwork,
                //               #endif
