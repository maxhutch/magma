/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from sparse/testing/testing_zdot.cpp, normal z -> d, Sun Nov 20 20:20:46 2016
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    for( magma_int_t num_vecs=1; num_vecs <= 32; num_vecs += 1 ) {
        for( magma_int_t n=500000; n < 500001; n += 10000 ) {
            ...
            TESTING_CHECK( magma_dvinit( &b, Magma_DEV, n, 1, one, queue ));
            TESTING_CHECK( magma_dvinit( &x, Magma_DEV, n, 8, one, queue ));
            TESTING_CHECK( magma_dvinit( &y, Magma_DEV, n, 8, one, queue ));
            TESTING_CHECK( magma_dvinit( &skp, Magma_DEV, 1, num_vecs, zero, queue ));
            ...
            for( int h=0; h < iters; h++) {
                for( int c = 0; c < num_vecs/2; c++ ) {
                    TESTING_CHECK( magma_dmdotc( n, 2, a.dval, b.dval, x.dval, y.dval, skp.dval, queue ));
                }
                for( int c = 0; c < num_vecs % 2; c++ ){
                    TESTING_CHECK( magma_dmdotc( n, 1, a.dval, b.dval, x.dval, y.dval, skp.dval, queue ));
                }
            }
            #ifdef ENABLE_TIMER
            ...
            #endif
            for( int h=0; h < iters; h++) {
                TESTING_CHECK( magma_dgemvmdot_shfl( n, num_vecs, a.dval, b.dval, x.dval, y.dval, skp.dval, queue ));
            }
            #ifdef ENABLE_TIMER
            mdgm2 = magma_sync_wtime( queue );
            ...
            //magma_dprint_gpu( num_vecs, 1, skp.dval, num_vecs, opts.queue );
