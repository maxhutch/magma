/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from sparse/src/zparilut.cpp, normal z -> s, Sun Nov 20 20:20:46 2016
*/
    ...
    magma_queue_t queue )
{
    ...
    float thrsL = MAGMA_S_ZERO;
    float thrsU = MAGMA_S_ZERO;
    ...
    magma_smconvert( hA, &hL, Magma_CSR, Magma_CSRL, queue );
    magma_smconvert( hL, &L, Magma_CSR, Magma_CSRLIST, queue );

    magma_smtranspose(hA, &hAT, queue );
    magma_smconvert( hAT, &hU, Magma_CSR, Magma_CSRL, queue );
    magma_smconvert( hU, &U, Magma_CSR, Magma_CSRLIST, queue );
    ...
    magma_sparilut_colmajor( U, &UR, queue );
    end = magma_sync_wtime( queue );
    //magma_free_cpu( UR.row );
    //magma_free_cpu( UR.list );


    //magma_s_mvisu( L, queue );

   // magma_smfree( &hAT, queue );
   // magma_smconvert( UR, &hAT, Magma_CSRLIST, Magma_CSR, queue );
    //magma_s_mvisu( UR, queue );

    magma_smalloc_cpu( &L_new.val, L.num_rows*100 );
    ...
    magma_smalloc_cpu( &U_new.val, U.num_rows*100 );
    ...
    magma_sparilut_sweep_list( &hA, &L, &U, queue );
    magma_sparilut_sweep_list( &hA, &L, &U, queue );
    magma_sparilut_sweep_list( &hA, &L, &U, queue );
    magma_sparilut_sweep_list( &hA, &L, &U, queue );
    magma_sparilut_sweep_list( &hA, &L, &U, queue );
    ...
    start = magma_sync_wtime( queue );
    magma_sparilut_colmajorup( U, &UR, queue );
    end = magma_sync_wtime( queue );
    ...
    for( magma_int_t iters =0; iters<precond->sweeps; iters++ ) {
        ...
        if( reorder == 1 ){
            // start = magma_sync_wtime( queue );
            // magma_sparilut_residuals_linkedlist( hA, L, U, &L_new, queue );
            // magma_sparilut_residuals_linkedlist( hA, L, U, &U_new, queue );
            // end = magma_sync_wtime( queue ); t_res=end-start;
            // printf("residuals unordered:%.2e\n", t_res);
            start = magma_sync_wtime( queue );
            magma_sparilut_residuals_list( hA, L, U, &L_new, queue );
            magma_sparilut_residuals_list( hA, L, U, &U_new, queue );
            end = magma_sync_wtime( queue ); t_res=end-start;
            // printf("residuals ordered:%.2e\n", t_res);
        } else {
            //magma_s_mvisu( U, queue );
            // magma_sparilut_reorder( &L, queue );
            // magma_sparilut_reorder( &U, queue );
            //printf("nnz:%d %d\n",L.nnz, U.nnz);
            // magma_sparilut_randlist( &L, queue );
            // magma_sparilut_randlist( &U, queue );
        ...
        start = magma_sync_wtime( queue );
        magma_sparilut_select_candidates_L( &num_rmL, rm_locL, &L_new, queue );
        magma_sparilut_select_candidates_U( &num_rmU, rm_locU, &U_new, queue );
        ...
        if( reorder > 1 ){
            ...
            // U.nnz = U.nnz + num_rmU;
            // start = magma_sync_wtime( queue );
            // magma_sparilut_sweep_linkedlist( &hA, &L, &U, queue );
        ...
        start = magma_sync_wtime( queue );
        info = magma_sparilut_set_approx_thrs( num_rmL, &L, 0, &thrsL, queue );
        if( info !=0 ){
            printf("%% error: breakdown in iteration :%5lld. fallback.\n\n", (long long) (iters+1)); fflush(stdout);
            info = 0;
            break;
        }//printf("done thrs L\n"); fflush(stdout);
        info = magma_sparilut_set_approx_thrs( num_rmU, &U, 0, &thrsU, queue );
        if( info !=0 ){
            printf("%% error: breakdown in iteration :%5lld. fallback.\n\n", (long long) (iters+1)); fflush(stdout);
            info = 0;
            break;
        ...
        // magma_sparilut_LU_approx_thrs( num_rmL+num_rmU, &L, &U, 0, &thrsL, queue );
        // thrsU=thrsL;
   // magma_int_t num_rm,
   // magma_s_matrix *L,
   // magma_s_matrix *U,
   ...
   // magma_queue_t queue )
        ...
        start = magma_sync_wtime( queue );
        magma_sparilut_rm_thrs( &thrsL, &num_rmL, &L, &L_new, rm_locL, queue );
        magma_sparilut_rm_thrs( &thrsU, &num_rmU, &U, &U_new, rm_locU, queue );
    ...
    L.nnz=L.nnz-num_rmL;
    U.nnz=U.nnz-num_rmU;
    magma_sparilut_count( L, &L.nnz, queue);
    magma_sparilut_count( U, &U.nnz, queue);
    ...
    magma_smfree( &hL, queue );
    magma_smfree( &hU, queue );
    magma_smconvert( L, &hL, Magma_CSRLIST, Magma_CSR, queue );
    magma_smconvert( U, &hU, Magma_CSRLIST, Magma_CSR, queue );
    magma_smtranspose(hU, &hAT, queue );
    ...
    CHECK( magma_smtransfer( hAT, &precond->U, Magma_CPU, Magma_DEV , queue ));
