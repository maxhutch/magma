/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from sparse/src/zparilut.cpp, normal z -> c, Sun Nov 20 20:20:46 2016
*/
    ...
    magma_queue_t queue )
{
    ...
    magmaFloatComplex thrsL = MAGMA_C_ZERO;
    magmaFloatComplex thrsU = MAGMA_C_ZERO;
    ...
    magma_cmconvert( hA, &hL, Magma_CSR, Magma_CSRL, queue );
    magma_cmconvert( hL, &L, Magma_CSR, Magma_CSRLIST, queue );

    magma_cmtranspose(hA, &hAT, queue );
    magma_cmconvert( hAT, &hU, Magma_CSR, Magma_CSRL, queue );
    magma_cmconvert( hU, &U, Magma_CSR, Magma_CSRLIST, queue );
    ...
    magma_cparilut_colmajor( U, &UR, queue );
    end = magma_sync_wtime( queue );
    //magma_free_cpu( UR.row );
    //magma_free_cpu( UR.list );


    //magma_c_mvisu( L, queue );

   // magma_cmfree( &hAT, queue );
   // magma_cmconvert( UR, &hAT, Magma_CSRLIST, Magma_CSR, queue );
    //magma_c_mvisu( UR, queue );

    magma_cmalloc_cpu( &L_new.val, L.num_rows*100 );
    ...
    magma_cmalloc_cpu( &U_new.val, U.num_rows*100 );
    ...
    magma_cparilut_sweep_list( &hA, &L, &U, queue );
    magma_cparilut_sweep_list( &hA, &L, &U, queue );
    magma_cparilut_sweep_list( &hA, &L, &U, queue );
    magma_cparilut_sweep_list( &hA, &L, &U, queue );
    magma_cparilut_sweep_list( &hA, &L, &U, queue );
    ...
    start = magma_sync_wtime( queue );
    magma_cparilut_colmajorup( U, &UR, queue );
    end = magma_sync_wtime( queue );
    ...
    for( magma_int_t iters =0; iters<precond->sweeps; iters++ ) {
        ...
        if( reorder == 1 ){
            // start = magma_sync_wtime( queue );
            // magma_cparilut_residuals_linkedlist( hA, L, U, &L_new, queue );
            // magma_cparilut_residuals_linkedlist( hA, L, U, &U_new, queue );
            // end = magma_sync_wtime( queue ); t_res=end-start;
            // printf("residuals unordered:%.2e\n", t_res);
            start = magma_sync_wtime( queue );
            magma_cparilut_residuals_list( hA, L, U, &L_new, queue );
            magma_cparilut_residuals_list( hA, L, U, &U_new, queue );
            end = magma_sync_wtime( queue ); t_res=end-start;
            // printf("residuals ordered:%.2e\n", t_res);
        } else {
            //magma_c_mvisu( U, queue );
            // magma_cparilut_reorder( &L, queue );
            // magma_cparilut_reorder( &U, queue );
            //printf("nnz:%d %d\n",L.nnz, U.nnz);
            // magma_cparilut_randlist( &L, queue );
            // magma_cparilut_randlist( &U, queue );
        ...
        start = magma_sync_wtime( queue );
        magma_cparilut_select_candidates_L( &num_rmL, rm_locL, &L_new, queue );
        magma_cparilut_select_candidates_U( &num_rmU, rm_locU, &U_new, queue );
        ...
        if( reorder > 1 ){
            ...
            // U.nnz = U.nnz + num_rmU;
            // start = magma_sync_wtime( queue );
            // magma_cparilut_sweep_linkedlist( &hA, &L, &U, queue );
        ...
        start = magma_sync_wtime( queue );
        info = magma_cparilut_set_approx_thrs( num_rmL, &L, 0, &thrsL, queue );
        if( info !=0 ){
            printf("%% error: breakdown in iteration :%5lld. fallback.\n\n", (long long) (iters+1)); fflush(stdout);
            info = 0;
            break;
        }//printf("done thrs L\n"); fflush(stdout);
        info = magma_cparilut_set_approx_thrs( num_rmU, &U, 0, &thrsU, queue );
        if( info !=0 ){
            printf("%% error: breakdown in iteration :%5lld. fallback.\n\n", (long long) (iters+1)); fflush(stdout);
            info = 0;
            break;
        ...
        // magma_cparilut_LU_approx_thrs( num_rmL+num_rmU, &L, &U, 0, &thrsL, queue );
        // thrsU=thrsL;
   // magma_int_t num_rm,
   // magma_c_matrix *L,
   // magma_c_matrix *U,
   ...
   // magma_queue_t queue )
        ...
        start = magma_sync_wtime( queue );
        magma_cparilut_rm_thrs( &thrsL, &num_rmL, &L, &L_new, rm_locL, queue );
        magma_cparilut_rm_thrs( &thrsU, &num_rmU, &U, &U_new, rm_locU, queue );
    ...
    L.nnz=L.nnz-num_rmL;
    U.nnz=U.nnz-num_rmU;
    magma_cparilut_count( L, &L.nnz, queue);
    magma_cparilut_count( U, &U.nnz, queue);
    ...
    magma_cmfree( &hL, queue );
    magma_cmfree( &hU, queue );
    magma_cmconvert( L, &hL, Magma_CSRLIST, Magma_CSR, queue );
    magma_cmconvert( U, &hU, Magma_CSRLIST, Magma_CSR, queue );
    magma_cmtranspose(hU, &hAT, queue );
    ...
    CHECK( magma_cmtransfer( hAT, &precond->U, Magma_CPU, Magma_DEV , queue ));
