/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from sparse/src/zparilut.cpp, normal z -> d, Sun Nov 20 20:20:46 2016
*/
    ...
    magma_queue_t queue )
{
    ...
    double thrsL = MAGMA_D_ZERO;
    double thrsU = MAGMA_D_ZERO;
    ...
    magma_dmconvert( hA, &hL, Magma_CSR, Magma_CSRL, queue );
    magma_dmconvert( hL, &L, Magma_CSR, Magma_CSRLIST, queue );

    magma_dmtranspose(hA, &hAT, queue );
    magma_dmconvert( hAT, &hU, Magma_CSR, Magma_CSRL, queue );
    magma_dmconvert( hU, &U, Magma_CSR, Magma_CSRLIST, queue );
    ...
    magma_dparilut_colmajor( U, &UR, queue );
    end = magma_sync_wtime( queue );
    //magma_free_cpu( UR.row );
    //magma_free_cpu( UR.list );


    //magma_d_mvisu( L, queue );

   // magma_dmfree( &hAT, queue );
   // magma_dmconvert( UR, &hAT, Magma_CSRLIST, Magma_CSR, queue );
    //magma_d_mvisu( UR, queue );

    magma_dmalloc_cpu( &L_new.val, L.num_rows*100 );
    ...
    magma_dmalloc_cpu( &U_new.val, U.num_rows*100 );
    ...
    magma_dparilut_sweep_list( &hA, &L, &U, queue );
    magma_dparilut_sweep_list( &hA, &L, &U, queue );
    magma_dparilut_sweep_list( &hA, &L, &U, queue );
    magma_dparilut_sweep_list( &hA, &L, &U, queue );
    magma_dparilut_sweep_list( &hA, &L, &U, queue );
    ...
    start = magma_sync_wtime( queue );
    magma_dparilut_colmajorup( U, &UR, queue );
    end = magma_sync_wtime( queue );
    ...
    for( magma_int_t iters =0; iters<precond->sweeps; iters++ ) {
        ...
        if( reorder == 1 ){
            // start = magma_sync_wtime( queue );
            // magma_dparilut_residuals_linkedlist( hA, L, U, &L_new, queue );
            // magma_dparilut_residuals_linkedlist( hA, L, U, &U_new, queue );
            // end = magma_sync_wtime( queue ); t_res=end-start;
            // printf("residuals unordered:%.2e\n", t_res);
            start = magma_sync_wtime( queue );
            magma_dparilut_residuals_list( hA, L, U, &L_new, queue );
            magma_dparilut_residuals_list( hA, L, U, &U_new, queue );
            end = magma_sync_wtime( queue ); t_res=end-start;
            // printf("residuals ordered:%.2e\n", t_res);
        } else {
            //magma_d_mvisu( U, queue );
            // magma_dparilut_reorder( &L, queue );
            // magma_dparilut_reorder( &U, queue );
            //printf("nnz:%d %d\n",L.nnz, U.nnz);
            // magma_dparilut_randlist( &L, queue );
            // magma_dparilut_randlist( &U, queue );
        ...
        start = magma_sync_wtime( queue );
        magma_dparilut_select_candidates_L( &num_rmL, rm_locL, &L_new, queue );
        magma_dparilut_select_candidates_U( &num_rmU, rm_locU, &U_new, queue );
        ...
        if( reorder > 1 ){
            ...
            // U.nnz = U.nnz + num_rmU;
            // start = magma_sync_wtime( queue );
            // magma_dparilut_sweep_linkedlist( &hA, &L, &U, queue );
        ...
        start = magma_sync_wtime( queue );
        info = magma_dparilut_set_approx_thrs( num_rmL, &L, 0, &thrsL, queue );
        if( info !=0 ){
            printf("%% error: breakdown in iteration :%5lld. fallback.\n\n", (long long) (iters+1)); fflush(stdout);
            info = 0;
            break;
        }//printf("done thrs L\n"); fflush(stdout);
        info = magma_dparilut_set_approx_thrs( num_rmU, &U, 0, &thrsU, queue );
        if( info !=0 ){
            printf("%% error: breakdown in iteration :%5lld. fallback.\n\n", (long long) (iters+1)); fflush(stdout);
            info = 0;
            break;
        ...
        // magma_dparilut_LU_approx_thrs( num_rmL+num_rmU, &L, &U, 0, &thrsL, queue );
        // thrsU=thrsL;
   // magma_int_t num_rm,
   // magma_d_matrix *L,
   // magma_d_matrix *U,
   ...
   // magma_queue_t queue )
        ...
        start = magma_sync_wtime( queue );
        magma_dparilut_rm_thrs( &thrsL, &num_rmL, &L, &L_new, rm_locL, queue );
        magma_dparilut_rm_thrs( &thrsU, &num_rmU, &U, &U_new, rm_locU, queue );
    ...
    L.nnz=L.nnz-num_rmL;
    U.nnz=U.nnz-num_rmU;
    magma_dparilut_count( L, &L.nnz, queue);
    magma_dparilut_count( U, &U.nnz, queue);
    ...
    magma_dmfree( &hL, queue );
    magma_dmfree( &hU, queue );
    magma_dmconvert( L, &hL, Magma_CSRLIST, Magma_CSR, queue );
    magma_dmconvert( U, &hU, Magma_CSRLIST, Magma_CSR, queue );
    magma_dmtranspose(hU, &hAT, queue );
    ...
    CHECK( magma_dmtransfer( hAT, &precond->U, Magma_CPU, Magma_DEV , queue ));
