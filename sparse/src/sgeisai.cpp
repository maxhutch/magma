/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from sparse/src/zgeisai.cpp, normal z -> s, Sun Nov 20 20:20:46 2016
*/
...
magma_sisai_l_t(
    ...
    if( precond->maxiter == 0 ){
        ...
        magma_s_spmv( MAGMA_S_ONE, precond->LDT, b, MAGMA_S_ZERO, precond->d, queue ); // d=L^(-1)b
        magma_s_spmv( MAGMA_S_ONE, precond->LDT, b, MAGMA_S_ZERO, *x, queue ); // SPAI
        for( int z=0; z<precond->maxiter; z++ ){
            magma_s_spmv( MAGMA_S_ONE, precond->LT, *x, MAGMA_S_ZERO, precond->work1, queue ); // work1=b+Lb
            magma_s_spmv( MAGMA_S_ONE, precond->LDT, precond->work1, MAGMA_S_ZERO, precond->work2, queue ); // x=x+L^(-1)work1
            magma_saxpy( b.num_rows*b.num_cols, -MAGMA_S_ONE, precond->work2.dval, 1 , x->dval, 1, queue );        // t = t + c
            magma_saxpy( b.num_rows*b.num_cols, MAGMA_S_ONE, precond->d.dval, 1 , x->dval, 1, queue );        // t = t + c
        }
    }
