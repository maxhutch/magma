/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from sparse/control/magma_zmconvert.cpp, normal z -> s, Sun Nov 20 20:20:45 2016
       @author Hartwig Anzt
*/
    ...
    magma_queue_t queue )
{
    ...
    for( i=0; i<*n; i++ ) {
        ...
        for( j=(*row)[i]; j<(*row)[i+1]; j++ ) {
            if ( (MAGMA_S_REAL((*val)[j]) != 0) || (MAGMA_S_IMAG((*val)[j]) != 0) ) {
                nnz_new++;
                nnz_this_row++;
            }
        }
    ...
    CHECK( magma_smalloc_cpu( valn, nnz_new ));
    ...
    if ( A.memory_location == Magma_CPU )
    {
        ...
        if ( old_format == Magma_CSR )
        {
            ...
            // CSR to CSRL
                ...
                B->nnz = numzeros;
                CHECK( magma_smalloc_cpu( &B->val, numzeros ));
                CHECK( magma_index_malloc_cpu( &B->row, A.num_rows+1 ));
            ...
            // CSR to CSRLIST
                ...
                CHECK( magma_smalloc_cpu( &B->val, A.nnz+A.num_rows*2 ));
            ...
            // so the rowlength is padded (SELLP) to a multiple of the alignment
                ...
                //fflush(stdout);
                CHECK( magma_smalloc_cpu( &B->val, B->row[slices] ));
            ...
            // CSR to BCSR
                ...
                CHECK( magma_smconvert(dA, &dB, Magma_CSR, Magma_BCSR, queue ) );
                CHECK( magma_smtransfer(dB, B, Magma_DEV, Magma_CPU, queue ) );
            }
            ...
            // CSR to CSR5
                ...
                CHECK( magma_smalloc_cpu( &B->calibrator, B->csr5_p ));
                for( magma_int_t i=0; i<B->csr5_p; i++) {
                    B->calibrator[i] = MAGMA_S_MAKE(0., 0.);
                }
        ...
        // anything to CSR
            ...
            // ELL/ELLPACK to CSR
                ...
                CHECK( magma_s_csr_compressor(&A.val, &row_tmp, &A.col,
            ...
            // ELL (column-major) to CSR
                ...
                // conversion
                CHECK( magma_smalloc_cpu( &val_tmp, A.num_rows*A.max_nnz_row ));
                CHECK( magma_index_malloc_cpu( &row_tmp, A.num_rows+1 ));
                ...
                //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros.
                //The CSR compressor removes these
                CHECK( magma_s_csr_compressor(&val_tmp, &row_tmp, &col_tmp,
                           &B->val, &B->row, &B->col, &B->num_rows, queue ));
          ...
          /*      CHECK( magma_smconvert( A, B, Magma_ELL, Magma_CSR, queue ));
                for( magma_int_t i=0; i < A.num_rows; i++) {
                    magma_sindexsortval(
                    B->col,
                    B->val,
            ...
            */
                ...
                //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros.
                //The CSR compressor removes these
                CHECK( magma_s_csr_compressor(&val_tmp2, &row_tmp, &col_tmp2,
                           &B->val, &B->row, &B->col, &B->num_rows, queue ));
                B->nnz = B->row[B->num_rows];
            }
            ...
            // SELLP to CSR
                ...
                // conversion
                CHECK( magma_smalloc_cpu( &val_tmp,
            ...
            // BCSR to CSR
                ...
                CHECK( magma_smtransfer(A, &dA, Magma_CPU, Magma_DEV, queue ) );
                CHECK( magma_smconvert(dA, &dB, Magma_BCSR, Magma_CSR, queue ) );
            ...
            // COO to CSR
                ...
                CHECK( magma_smtransfer(A, &dA, Magma_CPU, Magma_DEV, queue ) );
                CHECK( magma_smconvert(dA, &dB, Magma_COO, Magma_CSR, queue ) );
                magma_smfree( &dA, queue );
                CHECK( magma_smtransfer(dB, B, Magma_DEV, Magma_CPU, queue ) );
                magma_smfree( &dB, queue );
            }
        ...
        // CSR to BCSR
            ...
            CHECK( magma_smalloc( &B->dval, nnzb*size_b*size_b ));
        ...
        // CSC to CSR
            ...
            // conversion using CUSPARSE
            cusparseScsr2csc(cusparseHandle, A.num_cols, A.num_rows, A.nnz,
        ...
        // COO to CSR
            ...
            cusparseSgthr(cusparseHandle, nnz, A.dval, B->dval, P, CUSPARSE_INDEX_BASE_ZERO);
