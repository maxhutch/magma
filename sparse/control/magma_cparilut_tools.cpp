/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from sparse/control/magma_zparilut_tools.cpp, normal z -> c, Sun Nov 20 20:20:46 2016
       @author Hartwig Anzt
...
// start = magma_sync_wtime( queue );
    ...
    if( L_new->nnz > L_new->true_nnz ){
        ...
        magma_cmalloc_cpu( &L_new->val, L_new->nnz*2 );
    ...
    if( U_new->nnz > U_new->true_nnz ){
        ...
        magma_cmalloc_cpu( &U_new->val, U_new->nnz*2 );
    ...
    magma_queue_t queue )
{
    ...
    //start = magma_sync_wtime( queue );
    info = magma_cparilut_set_approx_thrs( (int)(*num_rm)*thrs1, L_new, 1, &element1, queue );
    count = 0;

    bound1 = MAGMA_C_ABS(element1);
    ...
    magma_queue_t queue )
{
    ...
    if( LU->nnz < 1024 ){
        ...
        blasf77_ccopy(&loc_nnz, LU->val, &incy, val, &incy );
        {
            #pragma omp parallel
            {
                magma_int_t id = omp_get_thread_num();
            ...
            element = element/MAGMA_C_MAKE((float)num_threads, 0.0);
        }
    } else {
