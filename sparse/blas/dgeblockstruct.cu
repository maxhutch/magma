/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from sparse/blas/zgeblockstruct.cu, normal z -> d, Sun Nov 20 20:20:41 2016
...
__global__ void 
magma_dmisai_blockstruct_row_kernel(    
    magma_int_t n, 
    magma_int_t bs, 
    magma_index_t *row,
    magma_index_t *col,
...
__global__ void 
magma_dmisai_blockstruct_fill_l_kernel(    
    magma_int_t n, 
    magma_int_t bs, 
    magma_index_t *row,
    magma_index_t *col,
    double *val )
{
    ...
    if( lrow < n ){
        if( lcol < n ){
            // val[loc] = MAGMA_D_MAKE((double)(lrow+1),(double)(1+lcol));
            // col[loc] = lcol;
            if( lcol<=lrow ){
                val[loc] = MAGMA_D_ONE;
                col[loc] = lcol;
            } else {
                val[loc] = MAGMA_D_ZERO;
                col[loc] = lcol;
            } 
        } 
        // else {
...
__global__ void 
magma_dmisai_blockstruct_fill_u_kernel(    
    magma_int_t n, 
    magma_int_t bs, 
    magma_index_t *row,
    magma_index_t *col,
    ...
    magma_queue_t queue )
{
    ...
    magma_dmisai_blockstruct_row_kernel<<< grid, block, 0, queue->cuda_stream() >>>
    ...
    if( uplotype == MagmaLower ){printf("enter here\n");
        magma_dmisai_blockstruct_fill_l_kernel<<< grid2, block2, 0, queue->cuda_stream() >>>
            ( A->num_rows, maxbs, A->drow, A->dcol, A->dval );
    } else {
        magma_dmisai_blockstruct_fill_u_kernel<<< grid2, block2, 0, queue->cuda_stream() >>>
            ( A->num_rows, maxbs, A->drow, A->dcol, A->dval );
    }
