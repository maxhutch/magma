/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from sparse/blas/zgeisai.cu, normal z -> d, Sun Nov 20 20:20:42 2016
...
__global__ void
magma_dlocations_trunc_lower_kernel(
    magma_int_t n,
    magma_index_t *row,
    magma_index_t *col,
    double *val,
...
__global__ void
magma_dlocations_trunc_upper_kernel(
    magma_int_t n,
    magma_index_t *row,
    magma_index_t *col,
    double *val,
    ...
    magma_queue_t queue )
{
    ...
    magma_dgpumemzero_z<<< grid, block, 0, queue->cuda_stream() >>>(
   ...
   // magma_dprint_gpu( 32, 32, L.dval, 32, queue );
    ...
    if( uplotype == MagmaLower ){
        magma_dlocations_lower_kernel<<< grid, block, 0, queue->cuda_stream() >>>(
                        LC.num_rows,
                        LC.drow,
                        LC.dcol,
                        LC.dval,
        ...
        magma_dlocations_upper_kernel<<< grid, block, 0, queue->cuda_stream() >>>(
                        LC.num_rows,
                        LC.drow,
                        LC.dcol,
                        LC.dval,
    ...
    magma_dfilltrisystems_kernel<<< grid2, block2, 0, queue->cuda_stream() >>>(
                        L.num_rows,
                        L.drow,
                        L.dcol,
                        L.dval,
    ...
    magma_queue_t queue )
{
    ...
    magma_dbackinsert_kernel<<< grid, block, 0, queue->cuda_stream() >>>(
            M->num_rows,
            M->drow,
            M->dcol,
            M->dval,
