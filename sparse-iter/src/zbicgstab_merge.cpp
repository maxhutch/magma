    ...
    magma_queue_t queue )
{
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter; 
        ...
        magma_zmdotc( dofs, 1, q.dval, v.dval, d1, d2, skp, queue );                     
        magma_zbicgmerge4(  1, skp, queue );
        magma_zbicgmerge2( dofs, skp, r.dval, v.dval, s.dval, queue );    // s=r-alpha*v

        magma_z_spmv( c_one, A, s, c_zero, t, queue );                 // t=As

        magma_zmdotc( dofs, 2, q.dval+4*dofs, t.dval, d1, d2, skp+6, queue );
        magma_zbicgmerge4(  2, skp, queue );

        magma_zbicgmerge_xrbeta( dofs, d1, d2, q.dval, r.dval, p.dval, 
