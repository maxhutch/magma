/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from zgmres.cpp normal z -> s, Fri May 30 10:41:41 2014
*/
#include <sys/time.h>
#include <time.h>
...
magma_sgmres( magma_s_sparse_matrix A, magma_s_vector b, magma_s_vector *x,  
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter; 
                                                    solver_par->numiter++ ){
        magma_scopy(dofs, r.val, 1, q(0), 1);       //  q[0]    = 1.0/||r||
        magma_sscal(dofs, 1./H(1,0), q(0), 1);      //  (to be fused)

        for(k=1; k<=restart; k++) {
            q_t.val = q(k-1);
            magmablasSetKernelStream(stream[0]);
            magma_s_spmv( c_one, A, q_t, c_zero, r );
                 // r = A q[k] 
        ...
        for( k=1; k<=restart; k++ ){
            ...
            for (i=1; i<=k; i++) {
                ...
                cblas_sdot_sub( i+1, &H(1,k), 1, &H(1,i), 1, &HH(k,i) );
                #else
                HH(k,i) = cblas_sdot(i+1, &H(1,k), 1, &H(1,i), 1);
                #endif
            }
            h1[k] = H(1,k)*H(1,0); 
            if (k != 1)
        ...
        magma_ssetmatrix_async(m, 1, y+1, m, dy, m, stream[0]);
        magmablasSetKernelStream(stream[0]);
