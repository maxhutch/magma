/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zidr_strms.cpp normal z -> c, Fri Sep 11 18:29:45 2015
*/
...
#elif MYDEBUG == 2
#define printD(...) printf("%% " __VA_ARGS__)
#define printMatrix(s,m,q) magma_cmatrixInfo_strms(s,m,q)
#else
#define printD(...) printf("%% " __VA_ARGS__)
#define printMatrix(s,m,q) magma_cmatrixInfo_strms(s,m,q)
#endif
...
// Comments:
// 1. queues - when receiving queue in magma_cidr function, one does not knows if queue is stream 0 (synchronous) or stream # (asynchronous). For this reason, I create the necessary queues needed and ignore the queue parameter. In the serial version of magma_cidr I set the queue parameter to the same queue as cuBLAS library. Also, queues will be used in OpenCL so we should use 0 instead of NULL for portability.
// 2. For pointer to array element, use &A[i] or A+i?
// 3. Synchronous functions:
//      magma_cvinit (does not uses queues, calls cmalloc)
//      magma_cmtransfer(calls magma_csetvector, magma_ccopyvector)
//      magma_cqr_wrapper(calls magma_get_cgeqrf_nb, magma_cgeqrf_gpu, magmablas_clacpy, magma_cgetvector, magma_ccopyvector, magma_cungqr_gpu)
//

extern "C" void
magma_cmatrixInfo_strms(
    const char *s,
    magma_c_matrix A,
    magma_queue_t queue )
{
    ...
    magma_queue_t queue )
{
    ...
    float nrmt;
    float rho;
    magmaFloatComplex *om = NULL;
    magmaFloatComplex *gamma = NULL;
    magmaFloatComplex *mkk = NULL;
    magmaFloatComplex *fk = NULL;

    // matrices and vectors
    magma_c_matrix dxs = {Magma_CSR};
    magma_c_matrix dP1 = {Magma_CSR}, dP = {Magma_CSR};
    magma_c_matrix dr = {Magma_CSR}, drs = {Magma_CSR};
    magma_c_matrix dg = {Magma_CSR}, dG = {Magma_CSR};
    magma_c_matrix du = {Magma_CSR}, dU = {Magma_CSR};
    ...
    magmaFloatComplex *skp = NULL;
    magmaFloatComplex *alpha = NULL;
    magmaFloatComplex *beta = NULL;
...
//---------------------------------------
    ...
    CHECK( magma_cvinit( &dg, Magma_DEV, dG.num_rows, 1, c_zero, queue ));
    magma_free( dg.dval );
    ...
    CHECK( magma_cvinit( &du, Magma_DEV, dU.num_rows, 1, c_zero, queue ));
    magma_free( du.dval );
    ...
    // M(k,k)
    CHECK( magma_cmalloc_pinned( &mkk, s ));

    // fk
    CHECK( magma_cmalloc_pinned( &fk, 1 ));

    // gamma 
    CHECK( magma_cmalloc_pinned( &gamma, 1 ));

    // om 
    CHECK( magma_cmalloc_pinned( &om, 1 ));
    ...
    magma_ccopy( dofr, dr.dval, 1, dv.dval, 1 );
    gpumem += dv.nnz * sizeof(magmaFloatComplex);

    // v1 = 0
    CHECK( magma_cvinit( &dv1, Magma_DEV, dr.num_rows, dr.num_cols, c_zero, queue ));
    gpumem += dv1.nnz * sizeof(magmaFloatComplex);
...
cudaProfilerStart();
    ...
    // c = f
    // Q1
    magma_ccopy( s, df.dval, 1, dc.dval, 1 );

    // solve small system and make v orthogonal to P
    // f(k:s) = M(k:s,k:s) c(k:s)
    // c(k:s) = M1(k:s,k:s) c(k:s)
    // Q1
    magma_ctrsv( MagmaLower, MagmaNoTrans, MagmaNonUnit, s, dM1.dval, dM1.ld, dc.dval, inc );
    ...
    do
    {
        ...
        for ( k = 0; k < s; ++k ) {
            ...
            // v = v - G(:,k:s) c(k:s)
            // Q1
            magmablas_cgemv( MagmaNoTrans, dG.num_rows, sk, c_n_one, dg.dval, dG.ld, &dc.dval[k], 1, c_one, dv.dval, 1 );
            ...
            // v = om * v + U(:,k:s) c(k:s)
            // Q1
            magmablas_cgemv( MagmaNoTrans, dU.num_rows, sk, c_one, du.dval, dU.ld, &dc.dval[k], 1, om[0], dv.dval, 1 );
            printMatrix("U", dv, queues[1]);

            // G(:,k) = A U(:,k)
            // G(:,k) = A v
            // Q1
            CHECK( magma_c_spmv( c_one, A, dv, c_zero, dg, queues[1] ));
...
// implicit sync Q1, Q2
                ...
                // M(k:s,k) = P(:,k:s)' G(:,k)
                // Q1
                magmablas_cgemv( MagmaConjTrans, dP.num_rows, s, c_one, dP.dval, dP.ld, dg.dval, 1, c_zero, dM.dval, 1 );
                ...
                if ( smoothing >= 0 ) {
                    ...
                    magma_caxpy( x->num_rows, beta[k-1], &dU.dval[(k-1)*dU.ld], 1, x->dval, 1 );
                ...
                if ( smoothing > 0 ) {
                    // xs = xs - gamma * (xs - x) 
                    // Q0
                    magma_cidr_smoothing_2( dxs.num_rows, dxs.num_cols, -gamma[0], x->dval, dxs.dval, queues[0] );
...
/*
                    ...
                    // Q2
                    magma_caxpy( sk, -beta[k-1], &dM.dval[(k-1)*dM.ld+k], 1, &df.dval[k], 1 );
                    printMatrix("F", df, queues[2]);

                    // c(k:s) = f(k:s)
                    // Q2
                    magma_ccopy( sk + 1, &df.dval[k-1], 1, &dc.dval[k-1], 1 );

                    // solve small system and make v orthogonal to P
                    // f(k:s) = M(k:s,k:s) c(k:s)
                    // c(k:s) = M1(k:s,k:s) c(k:s)
                    // Q2
                    magma_ctrsv( MagmaLower, MagmaNoTrans, MagmaNonUnit, sk, &dM1.dval[k*dM1.ld+k], dM1.ld, &dc.dval[k], inc );
...
*/
                ...
                // comment: merge caxpy with next dot product into single kernel
                for ( i = 0; i < k; ++i ) {
                    // alpha = P(:,i)' G(:,k) / M(i,i)
                    // Q1
                    alpha[i] = magma_cdotc( dP.num_rows, &dP.dval[i*dP.ld], 1, dg.dval, 1 ); 
// implicit sync Q1, Q2
                    alpha[i] = alpha[i] / mkk[i];
                    printD("bi-ortho: i, k, alpha ...................%d, %d, (%lg, %lg)\n", i, k, MAGMA_C_REAL(alpha[i]), MAGMA_C_IMAG(alpha[i]));
                    
                    // G(:,k) = G(:,k) - alpha * G(:,i)
                    // Q1
                    magma_caxpy( dG.num_rows, -alpha[i], &dG.dval[i*dG.ld], 1, dg.dval, 1 );
                    printMatrix("G", dG, queues[1]);
                }
                ...
                // M(k:s,k) = P(:,k:s)' G(:,k)
                // Q1
                magmablas_cgemv( MagmaConjTrans, dP.num_rows, sk, c_one, &dP.dval[k*dP.ld], dP.ld, dg.dval, 1, c_zero, &dM.dval[k*dM.ld+k], 1 );
                printMatrix("M", dM, queues[1]);

                // fk = f(k)
                // Q2
                magma_cgetvector_async( 1, &df.dval[k], 1, fk, 1, queues[2] );

                // alpha = dalpha
                // Q0
                magma_csetvector_async( k, alpha, 1, dalpha.dval, 1, queues[0] );
                ...
                // Q0
                magmablasSetKernelStream( queues[0] );
                magmablas_cgemv( MagmaNoTrans, dU.num_rows, k, c_n_one, dU.dval, dU.ld, dalpha.dval, 1, c_one, dv.dval, 1 );
            ...
            // U(:,k) = v
            // Q0
            magma_ccopyvector_async( dU.num_rows, dv.dval, 1, du.dval, 1, queues[0]);

            // mkk = M(k,k)
            // Q
            magma_cgetvector( 1, &dM.dval[k*dM.ld+k], 1, &mkk[k], 1 );
// implicit sync Q1, Q2

            // check M(k,k) == 0
            if ( MAGMA_C_EQUAL(mkk[k], MAGMA_C_ZERO) ) {
            ...
            // beta = f(k) / M(k,k)
            beta[k] = fk[0] / mkk[k];
            printD("beta: k ...................%d, (%lg, %lg)\n", k, MAGMA_C_REAL(beta[k]), MAGMA_C_IMAG(beta[k]));

            if ( smoothing < 0 ) {
                // make r orthogonal to q_i, i = 1..k
                // r = r - beta * G(:,k)
                // Q1
                magma_caxpy( dr.num_rows, -beta[k], dg.dval, 1, dr.dval, 1 );
                printMatrix("R", dr, queues[1]);
            
                // M1 = M
                // Q0
                //magma_ccopyvector_async( dofM, dM.dval, 1, dM1.dval, 1, queues[0] );
                magma_ccopyvector_async( sk, &dM.dval[k*dM.ld+k], 1, &dM1.dval[k*dM1.ld+k], 1, queues[0] );
                ...
                if ( (k + 1) < s ) {
                    ...
                    // f(k+1:s) = f(k+1:s) - beta * M(k+1:s,k)
                    // Q0
                    magma_caxpy( sk - 1, -beta[k], &dM.dval[k*dM.ld+(k+1)], 1, &df.dval[k+1], 1 );
                    ...
                    // c(k:s) = M1(k:s,k:s) c(k:s)
                    // Q0
                    magma_ctrsv( MagmaLower, MagmaNoTrans, MagmaNonUnit, sk - 1, &dM1.dval[(k+1)*dM1.ld+(k+1)], dM1.ld, &dc.dval[k+1], inc );
                ...
                // |r|
                // Q1
                nrmr = magma_scnrm2( dofr, dr.dval, inc );           
// implicit sync Q1, Q2
...
/*            
                // v = r
                // Q1
                magma_ccopyvector_async( dofr, dr.dval, 1, dv.dval, 1, queues[1] );
*/

                if ( (k + 1) == s ) {
                    // v1 = r
                    // Q0
                    magma_ccopyvector_async( dofr, dr.dval, 1, dv1.dval, 1, queues[0] );
                }
                else {
                    // v = r
                    // Q1
                ...
                if ( s == 1 || (k + 1) == s ) {
                    ...
                    // Q0
                    magmablasSetKernelStream( queues[0] );
                    magma_caxpy( x->num_rows, beta[k], du.dval, 1, x->dval, 1 );
                    printMatrix("X", *x, queues[0]);
                    magmablasSetKernelStream( queues[1] );
                }
...
*/
                ...
                // t = rs - r
                // Q1
                magma_cidr_smoothing_1( drs.num_rows, drs.num_cols, drs.dval, dr.dval, dtt.dval, queues[1] );

                // t't 
                // t'rs
                // Q1
                CHECK( magma_cmdotc( doft, 2, dtt.dval, dtt.dval, d1, d2, &dskp.dval[2], queues[1] ));

                // skp = dskp
                // Q
                magma_cgetvector( 2 , &dskp.dval[2], 1, &skp[2], 1 );
// implicit sync Q1, Q2
                ...
                // rs = rs - gamma * t
                // Q1
                magma_caxpy( drs.num_rows, -gamma[0], dtt.dval, inc, drs.dval, inc );
                printMatrix("RS", drs, queues[1]);

                // |rs|
                // Q1
                nrmr = magma_scnrm2( dofr, drs.dval, inc );           
// implicit sync Q1, Q2
...
*/
            ...
            if ( (k + 1) == s ) {
                ...
                // t'r
                // Q2
                CHECK( magma_cmdotc( doft, 2, dt.dval, dt.dval, d1, d2, dskp.dval, queues[2] ));
            }
...
magma_queue_sync( queues[0] );
        ...
        if ( smoothing < 0 && innerflag != 1 ) {
            // dbeta = beta
            // Q1
            magma_csetvector_async( s, beta, 1, dbeta.dval, 1, queues[1] );

            // x = x + beta * U(:,1:s)
            // Q1
            magmablas_cgemv( MagmaNoTrans, dU.num_rows, s, c_one, dU.dval, dU.ld, dbeta.dval, 1, c_one, x->dval, 1 );
...
//---------------------------------------
        // skp = dskp
        // Q
        magma_cgetvector( 2 , dskp.dval, 1, skp, 1 );
// implicit sync Q1, Q2

        // |t|
        nrmt = magma_ssqrt(MAGMA_C_REAL(skp[0]));
        printD("tr, norm(t): k .................... %d, (%lg, %lg), %lg\n", k, MAGMA_C_REAL(skp[1]), MAGMA_C_IMAG(skp[1]), nrmt);
        
        // rho = abs(t'r / (|t| * |r|))
        rho = fabs( MAGMA_C_REAL(skp[1]) / (nrmt * nrmr) );

        // om = t'r / (|t| * |t|)
        om[0] = skp[1] / MAGMA_C_REAL(skp[0]); 
        if ( rho < angle )
            om[0] = om[0] * (angle / rho);
//---------------------------------------
        printD("omega: k .................... %d, (%lg, %lg)\n", k, MAGMA_C_REAL(om[0]), MAGMA_C_IMAG(om[0]));
        if ( MAGMA_C_EQUAL(om[0], MAGMA_C_ZERO) ) {
            info = MAGMA_DIVERGENCE;
        ...
        // r = r - om * t
        // Q1
        magma_caxpy(dr.num_rows, -om[0], dt.dval, 1, dr.dval, 1);
        printMatrix("R", dr, queues[1]);
        ...
        // Q0
        magmablasSetKernelStream( queues[0] );
        magma_caxpy(x->num_rows, om[0], dv1.dval, 1, x->dval, 1);
        printMatrix("X", *x, queues[0]);
        magmablasSetKernelStream( queues[1] );
...
// implicit sync Q1, Q2
        ...
        if ( smoothing < 1 ) {
            // v = r
            // Q0
            magma_ccopyvector_async( dofr, dr.dval, 1, dv.dval, 1, queues[0] );
