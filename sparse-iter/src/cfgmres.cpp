/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zfgmres.cpp normal z -> c, Fri Sep 11 18:29:44 2015
*/
...
GeneratePlaneRotation(magmaFloatComplex dx, magmaFloatComplex dy, magmaFloatComplex *cs, magmaFloatComplex *sn)
{
    ...
    real_Double_t rho = sqrt(MAGMA_C_REAL(MAGMA_C_CNJG(dx)*dx + MAGMA_C_CNJG(dy)*dy));
    *cs = dx / rho;
    *sn = dy / rho;
#endif
}
...
static void ApplyPlaneRotation(magmaFloatComplex *dx, magmaFloatComplex *dy, magmaFloatComplex cs, magmaFloatComplex sn)
{
    ...
    // below the code Joss Knight from MathWorks provided me with - this works. 
    // No idea why the above code fails for complex - maybe rounding.
    magmaFloatComplex temp  =  MAGMA_C_CNJG(cs) * (*dx) +  MAGMA_C_CNJG(sn) * (*dy);
    *dy = -(sn) * (*dx) + cs * (*dy);
    *dx = temp;
#endif
}
    ...
    magma_queue_t queue )
{
    ...
    float rel_resid, resid0=1, r0=0.0, betanom = 0.0, nom;
    ...
    do
    {
        ...
        do
        {
            ...
            v_t.dval = V(i);
            CHECK( magma_c_applyprecond_left( A, v_t, &t, precond_par, queue ));
            CHECK( magma_c_applyprecond_right( A, t, &t2, precond_par, queue ));
            ...
            for (k = 0; k <= i; k++)
            {
                H(k, i) = magma_cdotc(dofs, V(k), 1, V(i+1), 1);
                temp = -H(k,i);
                // V(i+1) -= H(k, i) * V(k);
