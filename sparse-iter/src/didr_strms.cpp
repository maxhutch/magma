/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zidr_strms.cpp normal z -> d, Fri Sep 11 18:29:44 2015
*/
...
#elif MYDEBUG == 2
#define printD(...) printf("%% " __VA_ARGS__)
#define printMatrix(s,m,q) magma_dmatrixInfo_strms(s,m,q)
#else
#define printD(...) printf("%% " __VA_ARGS__)
#define printMatrix(s,m,q) magma_dmatrixInfo_strms(s,m,q)
#endif
...
// Comments:
// 1. queues - when receiving queue in magma_didr function, one does not knows if queue is stream 0 (synchronous) or stream # (asynchronous). For this reason, I create the necessary queues needed and ignore the queue parameter. In the serial version of magma_didr I set the queue parameter to the same queue as cuBLAS library. Also, queues will be used in OpenCL so we should use 0 instead of NULL for portability.
// 2. For pointer to array element, use &A[i] or A+i?
// 3. Synchronous functions:
//      magma_dvinit (does not uses queues, calls dmalloc)
//      magma_dmtransfer(calls magma_dsetvector, magma_dcopyvector)
//      magma_dqr_wrapper(calls magma_get_dgeqrf_nb, magma_dgeqrf_gpu, magmablas_dlacpy, magma_dgetvector, magma_dcopyvector, magma_dorgqr_gpu)
//

extern "C" void
magma_dmatrixInfo_strms(
    const char *s,
    magma_d_matrix A,
    magma_queue_t queue )
{
    ...
    magma_queue_t queue )
{
    ...
    magma_d_matrix dg = {Magma_CSR}, dG = {Magma_CSR};
    magma_d_matrix du = {Magma_CSR}, dU = {Magma_CSR};
...
//---------------------------------------
    ...
    CHECK( magma_dvinit( &dg, Magma_DEV, dG.num_rows, 1, c_zero, queue ));
    magma_free( dg.dval );
    ...
    CHECK( magma_dvinit( &du, Magma_DEV, dU.num_rows, 1, c_zero, queue ));
    magma_free( du.dval );
    ...
    // M(k,k)
    CHECK( magma_dmalloc_pinned( &mkk, s ));

    // fk
    CHECK( magma_dmalloc_pinned( &fk, 1 ));

    // gamma 
    CHECK( magma_dmalloc_pinned( &gamma, 1 ));

    // om 
    CHECK( magma_dmalloc_pinned( &om, 1 ));
    ...
    magma_dcopy( dofr, dr.dval, 1, dv.dval, 1 );
    gpumem += dv.nnz * sizeof(double);

    // v1 = 0
    CHECK( magma_dvinit( &dv1, Magma_DEV, dr.num_rows, dr.num_cols, c_zero, queue ));
    gpumem += dv1.nnz * sizeof(double);
...
cudaProfilerStart();
    ...
    // c = f
    // Q1
    magma_dcopy( s, df.dval, 1, dc.dval, 1 );

    // solve small system and make v orthogonal to P
    // f(k:s) = M(k:s,k:s) c(k:s)
    // c(k:s) = M1(k:s,k:s) c(k:s)
    // Q1
    magma_dtrsv( MagmaLower, MagmaNoTrans, MagmaNonUnit, s, dM1.dval, dM1.ld, dc.dval, inc );
    ...
    do
    {
        ...
        for ( k = 0; k < s; ++k ) {
            ...
            // v = v - G(:,k:s) c(k:s)
            // Q1
            magmablas_dgemv( MagmaNoTrans, dG.num_rows, sk, c_n_one, dg.dval, dG.ld, &dc.dval[k], 1, c_one, dv.dval, 1 );
            ...
            // v = om * v + U(:,k:s) c(k:s)
            // Q1
            magmablas_dgemv( MagmaNoTrans, dU.num_rows, sk, c_one, du.dval, dU.ld, &dc.dval[k], 1, om[0], dv.dval, 1 );
            printMatrix("U", dv, queues[1]);

            // G(:,k) = A U(:,k)
            // G(:,k) = A v
            // Q1
            CHECK( magma_d_spmv( c_one, A, dv, c_zero, dg, queues[1] ));
...
// implicit sync Q1, Q2
                ...
                // M(k:s,k) = P(:,k:s)' G(:,k)
                // Q1
                magmablas_dgemv( MagmaConjTrans, dP.num_rows, s, c_one, dP.dval, dP.ld, dg.dval, 1, c_zero, dM.dval, 1 );
                ...
                if ( smoothing >= 0 ) {
                    ...
                    magma_daxpy( x->num_rows, beta[k-1], &dU.dval[(k-1)*dU.ld], 1, x->dval, 1 );
                ...
                if ( smoothing > 0 ) {
                    // xs = xs - gamma * (xs - x) 
                    // Q0
                    magma_didr_smoothing_2( dxs.num_rows, dxs.num_cols, -gamma[0], x->dval, dxs.dval, queues[0] );
...
/*
                    ...
                    // Q2
                    magma_daxpy( sk, -beta[k-1], &dM.dval[(k-1)*dM.ld+k], 1, &df.dval[k], 1 );
                    printMatrix("F", df, queues[2]);

                    // c(k:s) = f(k:s)
                    // Q2
                    magma_dcopy( sk + 1, &df.dval[k-1], 1, &dc.dval[k-1], 1 );

                    // solve small system and make v orthogonal to P
                    // f(k:s) = M(k:s,k:s) c(k:s)
                    // c(k:s) = M1(k:s,k:s) c(k:s)
                    // Q2
                    magma_dtrsv( MagmaLower, MagmaNoTrans, MagmaNonUnit, sk, &dM1.dval[k*dM1.ld+k], dM1.ld, &dc.dval[k], inc );
...
*/
                ...
                // comment: merge daxpy with next dot product into single kernel
                for ( i = 0; i < k; ++i ) {
                    // alpha = P(:,i)' G(:,k) / M(i,i)
                    // Q1
                    alpha[i] = magma_ddot( dP.num_rows, &dP.dval[i*dP.ld], 1, dg.dval, 1 ); 
// implicit sync Q1, Q2
                    alpha[i] = alpha[i] / mkk[i];
                    printD("bi-ortho: i, k, alpha ...................%d, %d, (%lg, %lg)\n", i, k, MAGMA_D_REAL(alpha[i]), MAGMA_D_IMAG(alpha[i]));
                    
                    // G(:,k) = G(:,k) - alpha * G(:,i)
                    // Q1
                    magma_daxpy( dG.num_rows, -alpha[i], &dG.dval[i*dG.ld], 1, dg.dval, 1 );
                    printMatrix("G", dG, queues[1]);
                }
                ...
                // M(k:s,k) = P(:,k:s)' G(:,k)
                // Q1
                magmablas_dgemv( MagmaConjTrans, dP.num_rows, sk, c_one, &dP.dval[k*dP.ld], dP.ld, dg.dval, 1, c_zero, &dM.dval[k*dM.ld+k], 1 );
                printMatrix("M", dM, queues[1]);

                // fk = f(k)
                // Q2
                magma_dgetvector_async( 1, &df.dval[k], 1, fk, 1, queues[2] );

                // alpha = dalpha
                // Q0
                magma_dsetvector_async( k, alpha, 1, dalpha.dval, 1, queues[0] );
                ...
                // Q0
                magmablasSetKernelStream( queues[0] );
                magmablas_dgemv( MagmaNoTrans, dU.num_rows, k, c_n_one, dU.dval, dU.ld, dalpha.dval, 1, c_one, dv.dval, 1 );
            ...
            // U(:,k) = v
            // Q0
            magma_dcopyvector_async( dU.num_rows, dv.dval, 1, du.dval, 1, queues[0]);

            // mkk = M(k,k)
            // Q
            magma_dgetvector( 1, &dM.dval[k*dM.ld+k], 1, &mkk[k], 1 );
// implicit sync Q1, Q2

            // check M(k,k) == 0
            if ( MAGMA_D_EQUAL(mkk[k], MAGMA_D_ZERO) ) {
            ...
            // beta = f(k) / M(k,k)
            beta[k] = fk[0] / mkk[k];
            printD("beta: k ...................%d, (%lg, %lg)\n", k, MAGMA_D_REAL(beta[k]), MAGMA_D_IMAG(beta[k]));

            if ( smoothing < 0 ) {
                // make r orthogonal to q_i, i = 1..k
                // r = r - beta * G(:,k)
                // Q1
                magma_daxpy( dr.num_rows, -beta[k], dg.dval, 1, dr.dval, 1 );
                printMatrix("R", dr, queues[1]);
            
                // M1 = M
                // Q0
                //magma_dcopyvector_async( dofM, dM.dval, 1, dM1.dval, 1, queues[0] );
                magma_dcopyvector_async( sk, &dM.dval[k*dM.ld+k], 1, &dM1.dval[k*dM1.ld+k], 1, queues[0] );
                ...
                if ( (k + 1) < s ) {
                    ...
                    // f(k+1:s) = f(k+1:s) - beta * M(k+1:s,k)
                    // Q0
                    magma_daxpy( sk - 1, -beta[k], &dM.dval[k*dM.ld+(k+1)], 1, &df.dval[k+1], 1 );
                    ...
                    // c(k:s) = M1(k:s,k:s) c(k:s)
                    // Q0
                    magma_dtrsv( MagmaLower, MagmaNoTrans, MagmaNonUnit, sk - 1, &dM1.dval[(k+1)*dM1.ld+(k+1)], dM1.ld, &dc.dval[k+1], inc );
                ...
                // |r|
                // Q1
                nrmr = magma_dnrm2( dofr, dr.dval, inc );           
// implicit sync Q1, Q2
...
/*            
                // v = r
                // Q1
                magma_dcopyvector_async( dofr, dr.dval, 1, dv.dval, 1, queues[1] );
*/

                if ( (k + 1) == s ) {
                    // v1 = r
                    // Q0
                    magma_dcopyvector_async( dofr, dr.dval, 1, dv1.dval, 1, queues[0] );
                }
                else {
                    // v = r
                    // Q1
                ...
                if ( s == 1 || (k + 1) == s ) {
                    ...
                    // Q0
                    magmablasSetKernelStream( queues[0] );
                    magma_daxpy( x->num_rows, beta[k], du.dval, 1, x->dval, 1 );
                    printMatrix("X", *x, queues[0]);
                    magmablasSetKernelStream( queues[1] );
                }
...
*/
                ...
                // t = rs - r
                // Q1
                magma_didr_smoothing_1( drs.num_rows, drs.num_cols, drs.dval, dr.dval, dtt.dval, queues[1] );

                // t't 
                // t'rs
                // Q1
                CHECK( magma_dmdotc( doft, 2, dtt.dval, dtt.dval, d1, d2, &dskp.dval[2], queues[1] ));

                // skp = dskp
                // Q
                magma_dgetvector( 2 , &dskp.dval[2], 1, &skp[2], 1 );
// implicit sync Q1, Q2
                ...
                // rs = rs - gamma * t
                // Q1
                magma_daxpy( drs.num_rows, -gamma[0], dtt.dval, inc, drs.dval, inc );
                printMatrix("RS", drs, queues[1]);

                // |rs|
                // Q1
                nrmr = magma_dnrm2( dofr, drs.dval, inc );           
// implicit sync Q1, Q2
...
*/
            ...
            if ( (k + 1) == s ) {
                ...
                // t'r
                // Q2
                CHECK( magma_dmdotc( doft, 2, dt.dval, dt.dval, d1, d2, dskp.dval, queues[2] ));
            }
...
magma_queue_sync( queues[0] );
        ...
        if ( smoothing < 0 && innerflag != 1 ) {
            // dbeta = beta
            // Q1
            magma_dsetvector_async( s, beta, 1, dbeta.dval, 1, queues[1] );

            // x = x + beta * U(:,1:s)
            // Q1
            magmablas_dgemv( MagmaNoTrans, dU.num_rows, s, c_one, dU.dval, dU.ld, dbeta.dval, 1, c_one, x->dval, 1 );
...
//---------------------------------------
        // skp = dskp
        // Q
        magma_dgetvector( 2 , dskp.dval, 1, skp, 1 );
// implicit sync Q1, Q2

        // |t|
        nrmt = magma_dsqrt(MAGMA_D_REAL(skp[0]));
        printD("tr, norm(t): k .................... %d, (%lg, %lg), %lg\n", k, MAGMA_D_REAL(skp[1]), MAGMA_D_IMAG(skp[1]), nrmt);
        
        // rho = abs(t'r / (|t| * |r|))
        rho = fabs( MAGMA_D_REAL(skp[1]) / (nrmt * nrmr) );

        // om = t'r / (|t| * |t|)
        om[0] = skp[1] / MAGMA_D_REAL(skp[0]); 
        if ( rho < angle )
            om[0] = om[0] * (angle / rho);
//---------------------------------------
        printD("omega: k .................... %d, (%lg, %lg)\n", k, MAGMA_D_REAL(om[0]), MAGMA_D_IMAG(om[0]));
        if ( MAGMA_D_EQUAL(om[0], MAGMA_D_ZERO) ) {
            info = MAGMA_DIVERGENCE;
        ...
        // r = r - om * t
        // Q1
        magma_daxpy(dr.num_rows, -om[0], dt.dval, 1, dr.dval, 1);
        printMatrix("R", dr, queues[1]);
        ...
        // Q0
        magmablasSetKernelStream( queues[0] );
        magma_daxpy(x->num_rows, om[0], dv1.dval, 1, x->dval, 1);
        printMatrix("X", *x, queues[0]);
        magmablasSetKernelStream( queues[1] );
...
// implicit sync Q1, Q2
        ...
        if ( smoothing < 1 ) {
            // v = r
            // Q0
            magma_dcopyvector_async( dofr, dr.dval, 1, dv.dval, 1, queues[0] );
