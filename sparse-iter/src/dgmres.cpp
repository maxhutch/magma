/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zgmres.cpp normal z -> d, Sat Nov 15 19:54:22 2014
*/
#include <sys/time.h>
#include <time.h>
    ...
    magma_queue_t queue )
{
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter; 
        ...
        magma_dscal(dofs, 1./H(k,k-1), q(k-1), 1);    //  (to be fused)
            ...
            q_t.dval = q(k-1);
            //magmablasSetKernelStream(stream[0]);
            magma_d_spmv( c_one, A, q_t, c_zero, r, queue ); //  r = A q[k] 
    //            if (solver_par->ortho == Magma_MGS ) {
        ...
        magma_dgemv(MagmaNoTrans, dofs, m, c_one, q(0), dofs, dy, 1, 
                                                    c_one, x->dval, 1); 
