/*
    -- MAGMA (version 1.5.0) --
       ...
       @generated from zgmres.cpp normal z -> c, Tue Sep  2 12:38:35 2014
*/
#include <sys/time.h>
#include <time.h>
...
magma_cgmres( magma_c_sparse_matrix A, magma_c_vector b, magma_c_vector *x,  
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter; 
        ...
        magma_ccopy(dofs, r.val, 1, q(k-1), 1);       //  q[0]    = 1.0/||r||
        magma_cscal(dofs, 1./H(k,k-1), q(k-1), 1);    //  (to be fused)

            q_t.val = q(k-1);
            magma_c_spmv( c_one, A, q_t, c_zero, r ); //  r = A q[k] 
    //            if (solver_par->ortho == Magma_MGS ) {
                ...
                H(k+1,k) = MAGMA_C_MAKE( magma_scnrm2(dofs, r.val, 1), 0. ); // H(k+1,k) = ||r|| 
