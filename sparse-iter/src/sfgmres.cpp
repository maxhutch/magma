/*
    -- MAGMA (version 1.6.1) --
       ...
       @generated from zfgmres.cpp normal z -> s, Fri Jan 30 19:00:30 2015
*/
#include <sys/time.h>
#include <time.h>
...
GeneratePlaneRotation(float dx, float dy, float *cs, float *sn)
{
    if (dy == MAGMA_S_ZERO) {
        *cs = MAGMA_S_ONE;
        *sn = MAGMA_S_ZERO;
    } else if (abs(MAGMA_S_REAL(dy)) > abs(MAGMA_S_REAL(dx))) {
        float temp = dx / dy;
        *sn = MAGMA_S_ONE / MAGMA_S_MAKE(sqrt( MAGMA_S_REAL( MAGMA_S_ONE + temp*temp)), 0.0 ) ;
        *cs = temp * *sn;
    } else {
        float temp = dy / dx;
        *cs = MAGMA_S_ONE /MAGMA_S_MAKE(sqrt( MAGMA_S_REAL( MAGMA_S_ONE + temp*temp )), 0.0 );
        *sn = temp * *cs;
    }
}
...
magma_sfgmres(
    ...
    magma_s_vector v_t, w_t, t, t2, V, W;
    ...
    magma_s_vinit( &t, Magma_DEV, dofs, MAGMA_S_ZERO, queue );
    magma_s_vinit( &t2, Magma_DEV, dofs, MAGMA_S_ZERO, queue );
    ...
    stat_cpu += magma_smalloc_pinned( &H, (dim+1)*dim );
    stat_cpu += magma_smalloc_pinned( &s,  dim+1 );
    stat_cpu += magma_smalloc_pinned( &cs, dim );
    stat_cpu += magma_smalloc_pinned( &sn, dim );
    ...
    magma_s_vinit( &V, Magma_DEV, dofs*(dim+1), MAGMA_S_ZERO, queue );
    magma_s_vinit( &W, Magma_DEV, dofs*dim, MAGMA_S_ZERO, queue );
    
    magma_sscal( dofs, MAGMA_S_ZERO, x->dval, 1 );              //  x = 0
    ...
    do
    {
        ...
        magma_s_spmv( MAGMA_S_ONE, A, *x, MAGMA_S_ZERO, t, queue );
        ...
        do
        {
            ...
            v_t.dval = V(i);
            magma_s_applyprecond_left( A, v_t, &t, precond_par, queue );   
            magma_s_applyprecond_right( A, t, &t2, precond_par, queue );    
            magma_scopy( dofs, t2.dval, 1, W(i), 1 );  

            // A.mult(n, 1, W(i), n, V(i+1), n);
            w_t.val = W(i);
            magma_s_spmv( MAGMA_S_ONE, A, w_t, MAGMA_S_ZERO, t, queue );
            magma_scopy( dofs, t.dval, 1, V(i+1), 1 );  
            
            for (k = 0; k <= i; k++)
            {
                H(k, i) = magma_sdot(dofs, V(i+1), 1, V(k), 1);            
                temp = -H(k,i);
                // V(i+1) -= H(k, i) * V(k);
            ...
            betanom = ABS(MAGMA_S_REAL( s[i+1] ) );
    ...
    magma_s_vfree( &W, queue);
    magma_s_vfree( &t, queue);
    magma_s_vfree( &t2, queue);
