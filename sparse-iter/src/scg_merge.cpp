/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from zcg_merge.cpp normal z -> s, Fri May 30 10:41:41 2014
*/
...
magma_int_t
magma_scg_merge( magma_s_sparse_matrix A, magma_s_vector b, magma_s_vector *x,  
    ...
    magma_s_vinit( &d, Magma_DEV, dofs, c_zero );
    magma_s_vinit( &z, Magma_DEV, dofs, c_zero );
    
    float *d1, *d2, *skp;
    magma_smalloc( &d1, dofs*(1) );
    magma_smalloc( &d2, dofs*(1) );
    // array for the parameters
    magma_smalloc( &skp, 6 );       // skp = [alpha|beta|gamma|rho|tmp1|tmp2]
    ...
    magma_scopy( dofs, b.val, 1, d.val, 1 );                    // d = b
    nom0 = betanom = magma_snrm2( dofs, r.val, 1 );               
    nom = nom0 * nom0;                                           // nom = r' * r
    magma_s_spmv( c_one, A, d, c_zero, z );                      // z = A d
    den = MAGMA_S_REAL( magma_sdot(dofs, d.val, 1, z.val, 1) ); // den = d'* z
    solver_par->init_res = nom0;
    
    // array on host for the parameters
    magma_smalloc_cpu( &skp_h, 6 );

    alpha = rho = gamma = tmp1 = c_one; 
    beta =  magma_sdot(dofs, r.val, 1, r.val, 1);
    skp_h[0]=alpha; 
    skp_h[1]=beta; 
    skp_h[2]=gamma; 
    skp_h[3]=rho; 
    ...
    cudaMemcpy( skp, skp_h, 6*sizeof( float ), 
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter; 
        ...
        magma_scgmerge_spmv1(  A, d1, d2, d.val, z.val, skp ); 
            
        // updates x, r, computes scalars and updates d
        magma_scgmerge_xrbeta( dofs, d1, d2, x->val, r.val, d.val, z.val, skp ); 

        // check stopping criterion (asynchronous copy)
        cublasGetVectorAsync(1 , sizeof( float ), skp+1, 1, 
                                                    skp_h+1, 1, stream[1] );
