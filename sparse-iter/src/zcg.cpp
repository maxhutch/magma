    ...
    magma_queue_t queue )
{
    ...
    den = MAGMA_Z_REAL( magma_zdotc( dofs, p.dval, 1, q.dval, 1, queue ) ); // den = p dot q
    ...
    do
    {
        ...
        betanom = magma_dznrm2( dofs, r.dval, 1, queue );             // betanom = || r ||
        ...
        den = MAGMA_Z_REAL(magma_zdotc( dofs, p.dval, 1, q.dval, 1, queue) );
                // den = p dot q
        nom = betanomsq;
    }
