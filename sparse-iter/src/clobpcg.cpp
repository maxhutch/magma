/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zlobpcg.cpp normal z -> c, Sat Nov 15 19:54:22 2014
*/
...
magma_clobpcg(
    magma_c_sparse_matrix A, magma_c_solver_par *solver_par,
    magma_queue_t queue )
{
...
#define magma_c_bspmv_tuned(m, n, alpha, A, X, beta, AX, queue)       {        \
            ...
            magma_c_spmv(alpha, A, x, beta, ax, queue );                           \
}
...
//**************************************************************
    ...
    stat += magma_cmalloc_pinned( &hwork   ,        lwork );
        if( stat != 0){
        magma_free_pinned( hwork );
        return MAGMA_ERR_HOST_ALLOC;
    }
    stat += magma_cmalloc(        &blockAX   ,        m*n );
    stat += magma_cmalloc(        &blockAR   ,        m*n );
    stat += magma_cmalloc(        &blockAP   ,        m*n );
    stat += magma_cmalloc(        &blockR    ,        m*n );
    stat += magma_cmalloc(        &blockP    ,        m*n );
    stat += magma_cmalloc(        &blockW    ,        m*n );
    stat += magma_cmalloc(        &dwork     ,        m*n );            
    stat += magma_smalloc(        &eval_gpu  ,        3*n );
    if( stat != 0){
        magma_free_pinned( hwork );
        magma_free( blockAX  );
        magma_free( blockAR  );
...
/*
  ...
  */        
            ...
            magma_clobpcg_res( m, n, eval_gpu, blockX, blockR, eval_gpu, queue );
...
// re-orthogonalization
            ...
            if (!restart) {
                ...
                magma_ccompactActive(m, n, blockP,  m, activeMask, queue );
                magma_ccompactActive(m, n, blockAP, m, activeMask, queue );
...
//                magma_ctrsm( MagmaRight, MagmaUpper, MagmaNoTrans, MagmaNonUnit, 
  //                           m, cBlockSize, c_one, dwork, cBlockSize, blockAP, m);
            ...
            */
            ...
            magma_clobpcg_maxpy( m, n, blockP, blockX, queue );    
            ...
            magma_clobpcg_maxpy( m, n, blockAP, blockAX, queue );    

            condestGhistory[iterationNumber+1]=condestG;

            magma_sgetmatrix(1, 1, residualNorms(0, iterationNumber), 1,  &tmp, 1);
