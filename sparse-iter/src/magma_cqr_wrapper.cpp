/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from magma_zqr_wrapper.cpp normal z -> c, Fri Sep 11 18:29:44 2015
*/
    ...
    magma_queue_t queue )
{
    ...
    if ( A.memory_location == Magma_DEV ) {
        ...
        magma_csetvector( lda * n, A.val, inc, dA, inc );
    }
    ...
    if ( R != NULL ) {
        if ( A.memory_location == Magma_DEV ) {
            ...
            magmablas_clacpy( MagmaUpper, k, n, dA, lda, R->dval, lda );
        } else {
            CHECK( magma_cvinit( &dR1, Magma_DEV, lda, n, c_zero, queue ) );
            magmablas_clacpy( MagmaUpper, k, n, dA, lda, dR1.dval, lda );
            CHECK( magma_cvinit( R, Magma_CPU, lda, n, c_zero, queue ) );
            magma_cgetvector( lda * n, dR1.dval, inc, R->val, inc );
        }
    }
    ...
    if ( Q != NULL ) {
        ...
        if ( A.memory_location == Magma_DEV ) {
            ...
            magma_ccopyvector( lda * n, dA, inc, Q->dval, inc );
        } else {
            CHECK( magma_cvinit( Q, Magma_CPU, lda, n, c_zero, queue ) );
            magma_cgetvector( lda * n, dA, inc, Q->val, inc );
        }
    }
