/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from zbcsrlu.cpp normal z -> c, Fri May 30 10:41:42 2014
*/
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    magma_c_sparse_matrix A                   descriptor for matrix A
    magma_c_vector b                          RHS b vector
...
magma_cbcsrlu( magma_c_sparse_matrix A, magma_c_vector b, 
    ...
    if( A.memory_location == Magma_DEV){
        ...
        magma_c_mtransfer( A, &B, Magma_DEV, Magma_CPU ); 
        magma_cbcsrlu( B,  b, x, solver_par );
        magma_c_mfree(&B);
        return MAGMA_SUCCESS;
    }
    else{

    magma_c_sparse_matrix B, C, D;
    ...
    magma_c_mconvert( A, &B, Magma_CSR, Magma_BCSR);
    magma_c_mtransfer( B, &C, Magma_CPU, Magma_DEV);
    ...
    magma_cbcsrlutrf( C, &D, ipiv, solver_par->version );
    magma_device_sync(); t_lu+=(magma_wtime()-t_lu1);


    // triangular solves
    magma_device_sync(); t_lusv1=magma_wtime();
    magma_cbcsrlusv( D, b, x, solver_par, ipiv );
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    magma_c_sparse_matrix A           input matrix A (on DEV)
    magma_c_sparse_matrix *M          output matrix containing LU decomposition
    magma_int_t *ipiv                 pivot vector

    ========================================================================  */

magma_int_t
magma_cbcsrlutrf( magma_c_sparse_matrix A, magma_c_sparse_matrix *M, 
                                       magma_int_t *ipiv, magma_int_t version ){


    // some useful variables
    magmaFloatComplex one = MAGMA_C_ONE;
    magmaFloatComplex m_one = MAGMA_C_NEG_ONE;
    ...
    magma_cmalloc( &M->val, size_b*size_b*(M->numblocks) );
    magma_indexmalloc( &M->row, r_blocks+1 );
    magma_indexmalloc( &M->col, M->numblocks );

    // Prepare A 
    magmaFloatComplex **hA, **dA,  **hB, **dB, **hBL, **dBL, **hC, **dC;
    int rowt=0, rowt2=0;
    ...
    magmaFloatComplex **AIs, **AIIs, **dAIs, **dAIIs, **BIs, **dBIs;
    ...
    cublasSetVector(  A.numblocks, sizeof(magmaFloatComplex *), 
                                                        hA, 1, dA, 1 );
    cublasSetVector(  A.numblocks, sizeof(magmaFloatComplex *), 
                                                        hB, 1, dB, 1 );
    cublasSetVector(  (M->numblocks-A.numblocks), 
                        sizeof(magmaFloatComplex *), hC, 1, dC, 1 );

    magma_cbcsrvalcpy(  size_b, A.numblocks, (M->numblocks-A.numblocks), 
                                                        dA, dB, dC );
    ...
    ldwork = size_b * magma_get_cgetri_nb( size_b );
    magma_cmalloc( &dwork, ldwork );
    ...
    for( k=0; k<r_blocks; k++){
        ...
        cublasSetVector( klblocks, sizeof(magmaFloatComplex *), hBL, 1, 
                                                                    dBL, 1 );
        ...
        cublasSetVector( num_block_rows, sizeof(magmaFloatComplex *), hA, 1, 
                                                        dA, 1 );
        cublasSetVector( kblocks, sizeof(magmaFloatComplex *), hB, 1, dB, 1 );
        ...
        cublasSetVector( kblocks*num_block_rows, sizeof(magmaFloatComplex *), 
                                                                hC, 1, dC, 1 );
        ...
        // AIs and BIs for the batched GEMMs later
            ...
            cublasSetVector( kblocks*num_block_rows, 
                        sizeof(magmaFloatComplex *), AIs, 1, dAIs, 1 );
            cublasSetVector( kblocks*num_block_rows, 
                        sizeof(magmaFloatComplex *), BIs, 1, dBIs, 1 );
        }  
        ...
        cublasSetVector( max(num_block_rows, kblocks), 
                sizeof(magmaFloatComplex *), AIIs, 1, dAIIs, 1 );
        
        magma_cgetrf_gpu( size_b, size_b, M(k,k), ldda, ipiv+k*size_b, &info );


        // Swap elements on the right before update
        cublasSetVector( size_b, sizeof( int ), ipiv+k*size_b, 1, ipiv_d, 1 ); 
        magma_cbcsrlupivloc( size_b, kblocks, dB, ipiv_d );


        // update blocks right
        cublasCtrsmBatched( handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, 
        ...
        magma_cbcsrlupivloc( size_b, klblocks, dBL, ipiv_d );



        // update blocks below
        cublasCtrsmBatched( handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER, 
                            CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, size_b, size_b, 
                            &one, dAIIs, size_b, dA, size_b, num_block_rows );
        
        if( version==1 ){ 
            magmablasSetKernelStream( stream[0] );
            magma_cbcsrluegemm( size_b, num_block_rows, kblocks, dA, dB, dC ); 
        }
        ...
        if( version==0 ){ 
            ...
            magmablasSetKernelStream( stream[0] );
            cublasCgemmBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N, size_b, size_b, 
                              size_b,&m_one, (const magmaFloatComplex **) dAIs,  
                              size_b, (const magmaFloatComplex **) dBIs, 
...
magma_int_t
magma_cbcsrlusv( magma_c_sparse_matrix A, magma_c_vector b, magma_c_vector *x,  
           magma_c_solver_par *solver_par, magma_int_t *ipiv ){
    ...
    // set x = b
    cudaMemcpy( x->val, b.val, A.num_rows*sizeof( magmaFloatComplex ), 
                                            cudaMemcpyDeviceToDevice );

    // First pivot the RHS
    magma_cbcsrswp( r_blocks, size_b, ipiv, x->val );


    // forward solve
    magma_cbcsrtrsv( MagmaLower, r_blocks, c_blocks, size_b, 
                     A.val, A.blockinfo, x->val );

    // backward solve
    magma_cbcsrtrsv( MagmaUpper, r_blocks, c_blocks, size_b, 
                     A.val, A.blockinfo, x->val );
