/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zidr.cpp normal z -> c, Fri Sep 11 18:29:44 2015
*/
    ...
    magma_queue_t queue )
{
    ...
    // |b|
    nrmb = magma_scnrm2( b.num_rows, b.dval, 1 );

    // check for |b| == 0
    if ( nrmb == 0.0 ) {
        magma_cscal( x->num_rows * x->num_cols, MAGMA_C_ZERO, x->dval, 1 );
...
//---------------------------------------
    ...
    if ( dP1.num_cols > 1 ) {
        ...
        // P = P1 / |P1|
        nrm = magma_scnrm2( dP1.num_rows * dP1.num_cols, dP1.dval, 1 );
        nrm = 1.0 / nrm;
        magma_csscal( dP1.num_rows * dP1.num_cols, nrm, dP1.dval, 1 );
...
//---------------------------------------
    ...
    do
    {
        ...
        for ( k = 0; k < s; ++k ) {
            ...
            magma_ccopyvector( sk, &df.dval[k], 1, &dc.dval[k], 1 );
            magma_ccopyvector( dM.num_rows * dM.num_cols, dM.dval, 1, dM1.dval, 1 );
            magma_ctrsv( MagmaLower, MagmaNoTrans, MagmaNonUnit, sk, &dM1.dval[k*dM1.ld+k], dM1.ld, &dc.dval[k], 1 );

            // v1 = r - G(:,k:s) c(k:s)
            magma_ccopyvector( dr.num_rows * dr.num_cols, dr.dval, 1, dv1.dval, 1 );
            ...
            magma_ccopyvector( dU.num_rows, dv1.dval, 1, &dU.dval[k*dU.ld], 1 );

            // compute new G
            // G(:,k) = A U(:,k)
            CHECK( magma_c_spmv( c_one, A, dv1, c_zero, dv, queue ));
            magma_ccopyvector( dG.num_rows, dv.dval, 1, &dG.dval[k*dG.ld], 1 );

            // bi-orthogonalize the new basis vectors
            for ( i = 0; i < k; ++i ) {
                // alpha = P(:,i)' G(:,k) / M(i,i)
                alpha = magma_cdotc( dP.num_rows, &dP.dval[i*dP.ld], 1, &dG.dval[k*dG.ld], 1 );
                magma_cgetvector( 1, &dM.dval[i*dM.ld+i], 1, &mkk, 1 );
                alpha = alpha / mkk;

                // G(:,k) = G(:,k) - alpha * G(:,i)
                magma_caxpy( dG.num_rows, -alpha, &dG.dval[i*dG.ld], 1, &dG.dval[k*dG.ld], 1 );

                // U(:,k) = U(:,k) - alpha * U(:,i)
                magma_caxpy( dU.num_rows, -alpha, &dU.dval[i*dU.ld], 1, &dU.dval[k*dU.ld], 1 );
            }
            ...
            if ( smoothing == 0 ) {
                // |r|
                nrmr = magma_scnrm2( b.num_rows * b.num_cols, dr.dval, 1 );
            }
            else if ( smoothing == 1 ) {
                // x = x + beta * U(:,k)
...
//---------------------------------------
                // t = rs - r
                magma_ccopyvector( drs.num_rows * drs.num_cols, drs.dval, 1, dt.dval, 1 );
                magma_caxpy( dt.num_rows, c_n_one, dr.dval, 1, dt.dval, 1 );

                // gamma = (t' * rs) / (|t| * |t|)
                nrmt = magma_scnrm2( dt.num_rows * dt.num_cols, dt.dval, 1 );
                gamma = magma_cdotc( dt.num_rows, dt.dval, 1, drs.dval, 1 );
                gamma = gamma / (nrmt * nrmt);

                // rs = rs - gamma * t
                magma_caxpy( drs.num_rows, -gamma, dt.dval, 1, drs.dval, 1 );

                // xs = xs - gamma * (xs - x) 
                magma_ccopyvector( dxs.num_rows * dxs.num_cols, dxs.dval, 1, dt.dval, 1 );
                magma_caxpy( dt.num_rows, c_n_one, x->dval, 1, dt.dval, 1 );
                magma_caxpy( dxs.num_rows, -gamma, dt.dval, 1, dxs.dval, 1 );

                // |rs|
                nrmr = magma_scnrm2( b.num_rows * b.num_cols, drs.dval, 1 );           
            }
//---------------------------------------
        ...
        // v = r
        magma_ccopyvector( dr.num_rows * dr.num_cols, dr.dval, 1, dv.dval, 1 );
...
//---------------------------------------
        ...
        // tr = t' r
        tr = magma_cdotc( dr.num_rows, dt.dval, 1, dr.dval, 1 );
...
//---------------------------------------
    ...
    if ( smoothing == 1 ) {
        magma_ccopyvector( dr.num_rows * dr.num_cols, drs.dval, 1, dr.dval, 1 );
        magma_ccopyvector( x->num_rows * x->num_cols, dxs.dval, 1, x->dval, 1 );
    }
