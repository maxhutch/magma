/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zlobpcg.cpp normal z -> d, Sat Nov 15 19:54:22 2014
*/
...
magma_dlobpcg(
    magma_d_sparse_matrix A, magma_d_solver_par *solver_par,
    magma_queue_t queue )
{
...
#define magma_d_bspmv_tuned(m, n, alpha, A, X, beta, AX, queue)       {        \
            ...
            magma_d_spmv(alpha, A, x, beta, ax, queue );                           \
}
...
//**************************************************************
    ...
    stat += magma_dmalloc_pinned( &hwork   ,        lwork );
        if( stat != 0){
        magma_free_pinned( hwork );
        return MAGMA_ERR_HOST_ALLOC;
    }
    stat += magma_dmalloc(        &blockAX   ,        m*n );
    stat += magma_dmalloc(        &blockAR   ,        m*n );
    stat += magma_dmalloc(        &blockAP   ,        m*n );
    stat += magma_dmalloc(        &blockR    ,        m*n );
    stat += magma_dmalloc(        &blockP    ,        m*n );
    stat += magma_dmalloc(        &blockW    ,        m*n );
    stat += magma_dmalloc(        &dwork     ,        m*n );            
...
/*
  ...
  */        
            ...
            magma_dlobpcg_res( m, n, eval_gpu, blockX, blockR, eval_gpu, queue );
...
// re-orthogonalization
            ...
            if (!restart) {
                ...
                magma_dcompactActive(m, n, blockP,  m, activeMask, queue );
                magma_dcompactActive(m, n, blockAP, m, activeMask, queue );
...
//                magma_dtrsm( MagmaRight, MagmaUpper, MagmaNoTrans, MagmaNonUnit, 
  //                           m, cBlockSize, c_one, dwork, cBlockSize, blockAP, m);
            ...
            */
            ...
            magma_dlobpcg_maxpy( m, n, blockP, blockX, queue );    
            ...
            magma_dlobpcg_maxpy( m, n, blockAP, blockAX, queue );    
