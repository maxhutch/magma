/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zmdot.cu normal z -> d, Fri Sep 11 18:29:42 2015
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    if (k>1) {
        magma_dblockdot_kernel<<<Gs, Bs, Ms, queue>>>( Gs.x, n, k, v, r, d1 );
    }
    else {
        magma_ddot_kernel<<<Gs, Bs, Ms, queue>>>( Gs.x, n, v, r, d1 );
    }
/*
...
*/
    ...
    if ( k>1) {
        while( Gs.x > 1 ) {
            ...
            magma_dblockreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2, queue >>> 
                        ( Gs.x, n, k, aux1, aux2 );
        ...
        while( Gs.x > 1 ) {
            Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x;
            if ( Gs_next.x == 1 ) Gs_next.x = 2;
            magma_dreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2, queue >>> 
                        ( Gs.x, n, aux1, aux2 );
            Gs_next.x = Gs_next.x /2;
            Gs.x = Gs_next.x;
            b = 1 - b;
    ...
    for( int j=0; j<k; j++) {
            magma_dcopyvector_async( 1, aux1+j*n, 1, skp+j, 1, queue );
    }
