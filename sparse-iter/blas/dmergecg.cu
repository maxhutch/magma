/*
    -- MAGMA (version 1.5.0) --
       ...
       @generated from zmergecg.cu normal z -> d, Tue Sep  2 12:38:33 2014
       @author Hartwig Anzt
...
__global__ void 
magma_dcgreduce_kernel_spmv1( int Gs,
                           int n, 
                           double *vtmp,
                           double *vtmp2 ){
...
__global__ void 
magma_dcgmerge_spmvellpackt_kernel(  
                 int n,
...
magma_dcgmerge_spmvellpack_kernel(  
    ...
    if(i < n ){
        ...
        for ( int k = 0; k < num_cols_per_row ; k ++){
            ...
            double val = d_val [ num_cols_per_row * i + k ];
            if( val != 0)
                dot += val * d[ col ];
        }
        z[ i ] =  dot;
 ...
 temp[ Idx ] = MAGMA_D_MAKE( 0.0, 0.0);
    ...
    if(i < num_rows ){
        ...
        for ( int n = 0; n < border; n ++){
            ...
            double val = d_val[offset+ blocksize * n + Idx];
            if( val != 0){
                  dot=dot+val*d[col];
            }
        }
...
__global__ void 
magma_dcgmerge_spmvsellpt_kernel_8( int num_rows, 
                     int blocksize,
                     int T,
                     double *d_val, 
                     magma_index_t *d_colind,
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void 
magma_dcgmerge_spmvsellpt_kernel_16( int num_rows, 
                     int blocksize,
                     int T,
                     double *d_val, 
                     magma_index_t *d_colind,
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void 
magma_dcgmerge_spmvsellpt_kernel_32( int num_rows, 
                     int blocksize,
                     int T,
                     double *d_val, 
                     magma_index_t *d_colind,
...
magma_dcgmerge_spmv1(  
    ...
    if( A.storage_type == Magma_CSR )
        magma_dcgmerge_spmvcsr_kernel<<<Gs, Bs, Ms, magma_stream >>>
        ( A.num_rows, A.val, A.row, A.col, d_d, d_z, d1 );
    else if( A.storage_type == Magma_ELLPACK )
        magma_dcgmerge_spmvellpack_kernel<<<Gs, Bs, Ms, magma_stream >>>
        ( A.num_rows, A.max_nnz_row, A.val, A.col, d_d, d_z, d1 );
    else if( A.storage_type == Magma_ELL )
        magma_dcgmerge_spmvellpackt_kernel<<<Gs, Bs, Ms, magma_stream >>>
        ( A.num_rows, A.max_nnz_row, A.val, A.col, d_d, d_z, d1 );
    else if( A.storage_type == Magma_SELLC || A.storage_type == Magma_SELLP ){
        if( A.blocksize==256){
            magma_dcgmerge_spmvsellc_kernel<<<Gs, Bs, Ms, magma_stream >>>
        ...
        magma_dcgmerge_spmvellpackrt_kernel2<<<Gs, Bs, Ms, magma_stream >>>
    ...
    while( Gs.x > 1 ){
        ...
        magma_dcgreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
...
magma_dcgmerge_xrbeta(  
    ...
    double *aux1 = d1, *aux2 = d2;
    int b = 1;        
    magma_dcgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_dcg_d_kernel<<<Gs3, Bs3, 0>>>( n, skp, d_r, d_d );  
