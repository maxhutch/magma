/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from magma_zmcsrcompressor_gpu.cu normal z -> s, Sat Nov 15 19:54:21 2014
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    if ( A->memory_location == Magma_DEV && A->storage_type == Magma_CSR ) {
        ...
        magma_s_sparse_matrix dA, CSRA;
        magma_storage_t A_storage = A->storage_type;
        magma_location_t A_location = A->memory_location;
        magma_s_mconvert( *A, &CSRA, A->storage_type, Magma_CSR, queue );
        magma_s_mtransfer( *A, &dA, A->memory_location, Magma_DEV, queue );

        magma_smcsrcompressor_gpu( &dA, queue );

        magma_s_mfree( &dA, queue );
        magma_s_mfree( A, queue );
        magma_s_mtransfer( dA, &CSRA, Magma_DEV, A_location, queue );
        magma_s_mconvert( CSRA, A, Magma_CSR, A_storage, queue );
        magma_s_mfree( &dA, queue );
