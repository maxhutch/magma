/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from testing_zpreconditioner.cpp normal z -> c, Fri Sep 11 18:29:47 2015
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        //preconditioner
        CHECK( magma_c_precondsetup( B_d, b, &zopts.precond_par, queue ) );
        ...
        info = magma_c_applyprecond_left( B_d, b, &x1, &zopts.precond_par, queue ); 
        ...
        tempo1 = magma_sync_wtime( queue );
        info = magma_c_applyprecond_right( B_d, b, &x2, &zopts.precond_par, queue ); 
        tempo2 = magma_sync_wtime( queue );
        if( info != 0 ){
            printf("error: preconditioner returned: %s (%d).\n",
                magma_strerror( info ), int(info) );
        ...
        info = magma_c_applyprecond_left( B_d, b, &t, &zopts.precond_par, queue ); 
        info = magma_c_applyprecond_right( B_d, t, &x, &zopts.precond_par, queue ); 
