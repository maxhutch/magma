/*
    -- MAGMA (version 1.5.0) --
       ...
       @generated from testing_zspmv.cpp normal z -> c, Tue Sep  2 12:38:36 2014
       @author Hartwig Anzt
*/
...
int main( int argc, char** argv)
{
    ...
    while(  i < argc ){
        ...
        magma_c_vector hx, hy, dx, dy;

        // init CPU vectors
        magma_c_vinit( &hx, Magma_CPU, hA.num_rows, one );
        magma_c_vinit( &hy, Magma_CPU, hA.num_rows, zero );

        // init DEV vectors
        magma_c_vinit( &dx, Magma_DEV, hA.num_rows, one );
        magma_c_vinit( &dy, Magma_DEV, hA.num_rows, zero );
...
/*
        ...
        start = magma_wtime(); 
        for (j=0; j<10; j++ )
        mkl_ccsrmv( "N", &hA.num_rows, &hA.num_cols, 
...
*/
        ...
        // copy matrix to GPU
        magma_c_mtransfer( hA, &dA, Magma_CPU, Magma_DEV);
        // SpMV on GPU (CSR)
        magma_device_sync(); start = magma_wtime(); 
        for (j=0; j<10; j++)
            magma_c_spmv( one, dA, dx, zero, dy);
        ...
        magma_c_mconvert(  hA, &hA_ELL, Magma_CSR, Magma_ELL);
        magma_c_mtransfer( hA_ELL, &dA_ELL, Magma_CPU, Magma_DEV);
        magma_c_mfree(&hA_ELL);
        // SpMV on GPU (ELL)
        magma_device_sync(); start = magma_wtime(); 
        for (j=0; j<10; j++)
            magma_c_spmv( one, dA_ELL, dx, zero, dy);
        magma_device_sync(); end = magma_wtime(); 
        printf( " > MAGMA: %.2e seconds %.2e GFLOP/s    (standard ELL).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_c_mfree(&dA_ELL);


        // convert to SELLP and copy to GPU
        magma_c_mconvert(  hA, &hA_SELLP, Magma_CSR, Magma_SELLP);
        magma_c_mtransfer( hA_SELLP, &dA_SELLP, Magma_CPU, Magma_DEV);
        magma_c_mfree(&hA_SELLP);
        // SpMV on GPU (SELLP)
        magma_device_sync(); start = magma_wtime(); 
        for (j=0; j<10; j++)
            magma_c_spmv( one, dA_SELLP, dx, zero, dy);
        magma_device_sync(); end = magma_wtime(); 
        printf( " > MAGMA: %.2e seconds %.2e GFLOP/s    (SELLP).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_c_mfree(&dA_SELLP);
