/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from sparse-iter/testing/testing_zspmv.cpp normal z -> s, Fri Jan 22 21:42:52 2016
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        // init CPU vectors
        CHECK( magma_svinit( &hx, Magma_CPU, hA.num_rows, 1, c_zero, queue ));
        CHECK( magma_svinit( &hy, Magma_CPU, hA.num_rows, 1, c_zero, queue ));

        // init DEV vectors
        CHECK( magma_svinit( &dx, Magma_DEV, hA.num_rows, 1, c_one, queue ));
        CHECK( magma_svinit( &dy, Magma_DEV, hA.num_rows, 1, c_zero, queue ));
        ...
        CHECK( magma_smconvert(  hA, &hA_ELL, Magma_CSR, Magma_ELL, queue ));
        CHECK( magma_smtransfer( hA_ELL, &dA_ELL, Magma_CPU, Magma_DEV, queue ));
        ...
        start = magma_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_s_spmv( c_one, dA_ELL, dx, c_zero, dy, queue ));
        end = magma_sync_wtime( queue );
