/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from sparse-iter/testing/testing_zsolver_rhs.cpp normal z -> s, Fri Jan 22 21:42:52 2016
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        if ( strcmp("LAPLACE2D", argv[i]) == 0 && i+1 < argc ) {   // Laplace test
            ...
            CHECK( magma_svinit( &b_h, Magma_CPU, A.num_cols, 1, one, queue ));
        } else {                        // file-matrix test
            CHECK( magma_s_csr_mtx( &A,  argv[i], queue ));
            CHECK( magma_svread( &b_h, A.num_cols, argv[i+1], queue ));
            i++;
        }
        ...
        //CHECK( magma_svinit( &b_h, Magma_CPU, A.num_cols, 1, MAGMA_S_ONE, queue ));
