/*
    -- MAGMA (version 1.5.0) --
       ...
       @generated from testing_zdot.cpp normal z -> d, Tue Sep  2 12:38:36 2014
       @author Hartwig Anzt
*/
...
int main( int argc, char** argv)
{
    ...
    for( magma_int_t n=10000; n<100000001; n=n+10000 ){
            ...
            magma_d_vinit( &a, Magma_DEV, n*num_vecs, one );
            magma_d_vinit( &b, Magma_DEV, num_vecs, one );
            int min_ten = min(num_vecs, 15);
            magma_d_vinit( &x, Magma_DEV, min_ten*n, one );
            magma_d_vinit( &y, Magma_DEV, min_ten*n, one );
            magma_d_vinit( &skp, Magma_DEV, num_vecs, zero );

            // warm up
            magma_dgemvmdot( n, num_vecs, a.val, b.val, x.val, y.val, skp.val );
            ...
            for( int h=0; h<iters; h++){
                for( int l=0; l<num_vecs; l++)
                    alpha = magma_ddot(n, a.val, 1, b.val, 1);
            }
            #ifdef ENABLE_TIMER
            ...
            #endif
            for( int h=0; h<iters; h++){
                magma_dgemv(MagmaTrans, n, num_vecs, one, a.val, n, b.val, 1, zero, skp.val, 1);
                //h++;
            }
            #ifdef ENABLE_TIMER
            ...
            #endif
            for( int h=0; h<iters; h++){
                magmablas_dgemv(MagmaTrans, n, num_vecs, one, a.val, n, b.val, 1, zero, skp.val, 1);
                //h++;
            }
            #ifdef ENABLE_TIMER
            ...
            #endif
            for( int h=0; h<iters; h++){
                //magma_dmdotc( n, num_vecs, a.val, b.val, x.val, y.val, skp.val );
                magma_dmdotc( n, 2, a.val, b.val, x.val, y.val, skp.val );
                magma_dmdotc( n, 2, a.val, b.val, x.val, y.val, skp.val );
                magma_dmdotc( n, 1, a.val, b.val, x.val, y.val, skp.val );
                //h++;
            }
            #ifdef ENABLE_TIMER
            ...
            //magma_dprint_gpu(num_vecs,1,skp.val,num_vecs);
