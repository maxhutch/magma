/*
    -- MAGMA (version 2.1.0) --
       ...
       @generated from sparse-iter/testing/testing_zmcompressor.cpp, normal z -> c, Tue Aug 30 09:39:20 2016
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        start = magma_sync_wtime( queue );
        for (int j=0; j < 10; j++) {
            TESTING_CHECK( magma_cmcsrcompressor_gpu( &B_d, queue ));
        }
        end = magma_sync_wtime( queue );
        printf( " > MAGMA GPU: %.2e seconds.\n", (end-start)/10 );


        TESTING_CHECK( magma_cmtransfer( B_d, &B, Magma_DEV, Magma_CPU, queue ));
