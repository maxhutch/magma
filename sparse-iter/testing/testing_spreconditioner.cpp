/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from testing_zpreconditioner.cpp normal z -> s, Fri Sep 11 18:29:47 2015
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        CHECK( magma_svinit( &t, Magma_DEV, A.num_cols, 1, zero, queue ));
        CHECK( magma_svinit( &x1, Magma_DEV, A.num_cols, 1, zero, queue ));
        CHECK( magma_svinit( &x2, Magma_DEV, A.num_cols, 1, zero, queue ));
                        
        //preconditioner
        CHECK( magma_s_precondsetup( B_d, b, &zopts.precond_par, queue ) );
        
        float residual;
        CHECK( magma_sresidual( B_d, b, x, &residual, queue ));
        zopts.solver_par.init_res = residual;
        printf("data = [\n");
        
        printf("%%runtime left preconditioner:\n");
        tempo1 = magma_sync_wtime( queue );
        info = magma_s_applyprecond_left( B_d, b, &x1, &zopts.precond_par, queue ); 
        tempo2 = magma_sync_wtime( queue );
        if( info != 0 ){
            printf("error: preconditioner returned: %s (%d).\n",
                magma_strerror( info ), int(info) );
        }
        CHECK( magma_sresidual( B_d, b, x1, &residual, queue ));
        printf("%.8e  %.8e\n", tempo2-tempo1, residual );
        
        printf("%%runtime right preconditioner:\n");
        tempo1 = magma_sync_wtime( queue );
        info = magma_s_applyprecond_right( B_d, b, &x2, &zopts.precond_par, queue ); 
        tempo2 = magma_sync_wtime( queue );
        if( info != 0 ){
            printf("error: preconditioner returned: %s (%d).\n",
                magma_strerror( info ), int(info) );
        }
        CHECK( magma_sresidual( B_d, b, x2, &residual, queue ));
        printf("%.8e  %.8e\n", tempo2-tempo1, residual );
        
        
        printf("];\n");
        
        info = magma_s_applyprecond_left( B_d, b, &t, &zopts.precond_par, queue ); 
        info = magma_s_applyprecond_right( B_d, t, &x, &zopts.precond_par, queue ); 
