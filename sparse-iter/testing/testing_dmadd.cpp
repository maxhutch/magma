/*
    -- MAGMA (version 2.1.0) --
       ...
       @generated from sparse-iter/testing/testing_zmadd.cpp, normal z -> d, Tue Aug 30 09:39:21 2016
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    TESTING_CHECK( magma_dmtransfer( A, &A_d, Magma_CPU, Magma_DEV, queue ));
    TESTING_CHECK( magma_dmtransfer( B, &B_d, Magma_CPU, Magma_DEV, queue ));

    TESTING_CHECK( magma_dcuspaxpy( &one, A_d, &one, B_d, &C_d, queue ));

    magma_dmfree(&B_d, queue );

    TESTING_CHECK( magma_dcuspaxpy( &mone, A_d, &one, C_d, &B_d, queue ));
    
    TESTING_CHECK( magma_dmtransfer( B_d, &B2, Magma_DEV, Magma_CPU, queue ));
