/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from testing_zspmm.cpp normal z -> c, Sat Nov 15 19:54:24 2014
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    while(  i < argc ) {
        ...
        // init CPU vectors
        magma_c_vinit( &hx, Magma_CPU, m*n, c_one, queue );
        magma_c_vinit( &hy, Magma_CPU, m*n, c_zero, queue );

        // init DEV vectors
        magma_c_vinit( &dx, Magma_DEV, m*n, c_one, queue );
        magma_c_vinit( &dy, Magma_DEV, m*n, c_zero, queue );
        ...
        magma_c_vinit( &dy, Magma_DEV, dx.num_rows, c_zero, queue );
        // SpMV on GPU (ELL)
        start = magma_sync_wtime( queue ); 
        for (j=0; j<10; j++)
