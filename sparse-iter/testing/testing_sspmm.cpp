/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from testing_zspmm.cpp normal z -> s, Sat Nov 15 19:54:24 2014
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    magma_s_sparse_matrix hA, hA_SELLP, hA_ELL, dA, dA_SELLP, dA_ELL;
    ...
    while(  i < argc ) {
        ...
        if ( strcmp("LAPLACE2D", argv[i]) == 0 && i+1 < argc ) {   // Laplace test
            ...
            magma_sm_5stencil(  laplace_size, &hA, queue );
        } else {                        // file-matrix test
            magma_s_csr_mtx( &hA,  argv[i], queue );
        }
        ...
        magma_s_vector hx, hy, dx, dy, hrefvec, hcheck;
        ...
        // init CPU vectors
        magma_s_vinit( &hx, Magma_CPU, m*n, c_one, queue );
        magma_s_vinit( &hy, Magma_CPU, m*n, c_zero, queue );

        // init DEV vectors
        magma_s_vinit( &dx, Magma_DEV, m*n, c_one, queue );
        magma_s_vinit( &dy, Magma_DEV, m*n, c_zero, queue );
        ...
        // copy matrix to GPU
        magma_s_mtransfer( hA, &dA, Magma_CPU, Magma_DEV, queue );
        // SpMV on GPU (CSR)
        start = magma_sync_wtime( queue ); 
        for (j=0; j<10; j++)
            magma_s_spmv( c_one, dA, dx, c_zero, dy, queue );
        end = magma_sync_wtime( queue ); 
        printf( " > MAGMA: %.2e seconds %.2e GFLOP/s    (standard CSR).\n",
                                        (end-start)/10, FLOPS*10.*n/(end-start) );

        magma_s_vtransfer( dy, &hrefvec , Magma_DEV, Magma_CPU, queue );
        magma_s_mfree(&dA, queue );

        // convert to ELL and copy to GPU
        magma_s_mconvert(  hA, &hA_ELL, Magma_CSR, Magma_ELL, queue );
        magma_s_mtransfer( hA_ELL, &dA_ELL, Magma_CPU, Magma_DEV, queue );
        magma_s_mfree(&hA_ELL, queue );
        magma_s_vfree( &dy, queue );
        magma_s_vinit( &dy, Magma_DEV, dx.num_rows, c_zero, queue );
        // SpMV on GPU (ELL)
        start = magma_sync_wtime( queue ); 
        for (j=0; j<10; j++)
            magma_s_spmv( c_one, dA_ELL, dx, c_zero, dy, queue );
        end = magma_sync_wtime( queue ); 
        printf( " > MAGMA: %.2e seconds %.2e GFLOP/s    (standard ELL).\n",
                                        (end-start)/10, FLOPS*10.*n/(end-start) );

        magma_s_vtransfer( dy, &hcheck , Magma_DEV, Magma_CPU, queue );
        res = 0.0;
        ...
        magma_s_vfree( &hcheck, queue );

        magma_s_mfree(&dA_ELL, queue );


        // convert to SELLP and copy to GPU
        magma_s_mconvert(  hA, &hA_SELLP, Magma_CSR, Magma_SELLP, queue );
        magma_s_mtransfer( hA_SELLP, &dA_SELLP, Magma_CPU, Magma_DEV, queue );
        magma_s_mfree(&hA_SELLP, queue );
        magma_s_vfree( &dy, queue );
        magma_s_vinit( &dy, Magma_DEV, dx.num_rows, c_zero, queue );
        // SpMV on GPU (SELLP)
        start = magma_sync_wtime( queue ); 
        for (j=0; j<10; j++)
            magma_s_spmv( c_one, dA_SELLP, dx, c_zero, dy, queue );
        end = magma_sync_wtime( queue ); 
        ...
        magma_s_vfree(&hx, queue );
        magma_s_vfree(&hy, queue );
        magma_s_vfree(&hrefvec, queue );
        // free GPU memory
