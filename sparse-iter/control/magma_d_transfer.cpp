/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from magma_z_transfer.cpp normal z -> d, Fri May 30 10:41:46 2014
       @author Hartwig Anzt
*/
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    magma_d_sparse_matrix *B             copy of A      
...
magma_d_mtransfer( magma_d_sparse_matrix A, 
    ...
    if( src == Magma_CPU && dst == Magma_DEV ){
        ...
        if( A.storage_type == Magma_CSR || A.storage_type == Magma_CSC 
            ...
            // memory allocation
            stat = cublasAlloc( A.nnz, sizeof( double ), 
                                                        ( void** )&B->val );
            if( ( int )stat != 0 ) 
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            cublasSetVector( A.nnz , sizeof( double ), 
                                                    A.val, 1, B->val, 1 );
   ...
   /*     //CSRCSC-type 
        if( A.storage_type == Magma_CSRCSCL || 
            ...
            if( ( int )stat != 0 ) 
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
        //    stat = cublasAlloc( A.num_rows, sizeof( double ), 
        //                                                ( void** )&B->diag );
         //   if( ( int )stat != 0 ) 
         //   {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = cublasAlloc(  (A.num_rows+1) , sizeof( magma_index_t ), 
            ...
            cublasSetVector( A.nnz , sizeof( double ), 
                                                    A.val, 1, B->val, 1 );
         //   cublasSetVector( A.num_rows , sizeof( double ), 
           //                                         A.diag, 1, B->diag, 1 );
            cublasSetVector( (A.num_rows+1) , sizeof( magma_index_t ), 
                                                    A.row, 1, B->row, 1 );
            cublasSetVector( A.nnz , sizeof( magma_index_t ), 
 ...
 /*       //CSRCSC-type
       ...
       //     cublasGetVector( A.num_rows, 
           //        sizeof( double ), A.diag, 1, B->diag, 1 );
            cublasGetVector( (A.num_rows+1), 
                    sizeof( magma_index_t ), A.row, 1, B->row, 1 );            
            cublasGetVector( A.nnz, 
                    sizeof( magma_index_t ), A.col, 1, B->col, 1 );
        ...
        if( A.storage_type == Magma_ELLRT ){
            ...
            magma_indexmalloc_cpu( &B->col, rowlength*A.num_rows );
            // data transfer
            cublasGetVector( A.num_rows*rowlength, sizeof( double ), 
                                    A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_SELLC || A.storage_type == Magma_SELLP ){
            ...
            magma_indexmalloc_cpu( &B->row, A.numblocks+1 );
            // data transfer
            cublasGetVector( A.nnz, sizeof( double ), 
                                    A.val, 1, B->val, 1 );
    ...
    if( src == Magma_DEV && dst == Magma_DEV ){
        ...
        if( A.storage_type == Magma_CSR || A.storage_type == Magma_CSC 
            ...
            cudaMemcpy( B->val, A.val, 
                A.nnz*sizeof( double ), cudaMemcpyDeviceToDevice );
  ...
  /*      //CSRCSC-type
       ...
       //     cudaMemcpy( B->diag, A.diag,A.num_rows*sizeof( double ), 
             //       cudaMemcpyDeviceToDevice );
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    magma_d_vector x              vector x    
    magma_d_vector *y             copy of x      
