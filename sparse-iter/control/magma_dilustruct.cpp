/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from magma_zilustruct.cpp normal z -> d, Fri May 30 10:41:46 2014
       @author Hartwig Anzt
*/
...
extern "C"
magma_int_t 
magma_dilustruct( magma_d_sparse_matrix *A, magma_int_t levels ){
    ...
    if( A->memory_location == Magma_CPU && A->storage_type == Magma_CSR ){
        ...
        magma_d_sparse_matrix B, L, U, L_d, U_d, LU_d;

        magma_d_mconvert( *A, &B, Magma_CSR, Magma_CSR );

        for( i=0; i<levels; i++ ){

            magma_d_mconvert( *A, &L, Magma_CSR, Magma_CSRL );
            magma_d_mconvert( *A, &U, Magma_CSR, Magma_CSRU );
            ...
            magma_d_mtransfer(LU_d, A, Magma_DEV, Magma_CPU);
            magma_d_mfree( &L_d );
            magma_d_mfree( &U_d );
            magma_d_mfree( &LU_d );
        ...
        for( i=0; i<A->nnz; i++ )
            A->val[i] = MAGMA_D_MAKE( (double)(0.0/A->nnz), 0.0 );
        ...
        magma_d_mconvert( hA, &CSRCOOA, hA.storage_type, Magma_CSR );

        magma_dilustruct( &CSRCOOA, levels );

        magma_d_mfree( &hA );
        magma_d_mfree( A );
        magma_d_mconvert( CSRCOOA, &hA, Magma_CSR, A_storage );
