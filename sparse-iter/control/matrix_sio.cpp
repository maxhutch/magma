/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from matrix_zio.cpp normal z -> s, Fri May 30 10:41:46 2014
       @author Hartwig Anzt
*/
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    float **val             value array of CSR output 
...
magma_int_t read_s_csr_from_mtx(    magma_storage_t *type, 
  ...
  coo_val = (float *) malloc(*nnz*sizeof(float));
  assert( coo_val != NULL);
  ...
  printf("detected symmetric case\n");
    ...
    magma_index_t* new_col = 
        (magma_index_t *) malloc(true_nonzeros*sizeof(magma_index_t)) ; 
    float* new_val = 
      (float *) malloc(true_nonzeros*sizeof(float)) ; 
...
extern "C"
magma_int_t write_s_csrtomtx( magma_s_sparse_matrix B, const char *filename){

    write_s_csr_mtx( B.num_rows, B.num_cols, B.nnz, &B.val, &B.row, &B.col, 
...
magma_int_t write_s_csr_mtx(    magma_int_t n_row, 
  ...
  if( MajorType == MagmaColMajor){
    ...
    s_transpose_csr( n_row, n_col, nnz, *val, *row, *col, 
    ...
    {    
      ...
      for(j=0; j<rowtemp2-rowtemp1; j++)  
        file << ((new_col)[rowtemp1+j]+1) <<" "<< rowindex <<" "<< 
                        MAGMA_S_REAL((new_val)[rowtemp1+j]) << std::endl;
      rowindex++;
    }
    printf(" done\n");
    ...
    for(i=0; i<n_col; i++)
    {
      ...
      for(j=0; j<rowtemp2-rowtemp1; j++)  
        file << rowindex <<" "<< ((*col)[rowtemp1+j]+1) <<" "<< 
                        MAGMA_S_REAL((*val)[rowtemp1+j]) << std::endl;
      rowindex++;
    }
    printf(" done\n");
  }
    ...
    for(i=0; i<n_col; i++)
    {
      ...
      for(j=0; j<rowtemp2-rowtemp1; j++)  
        cout<< rowindex <<" "<< (*col)[rowtemp1+j]+1 <<" "<< 
                        MAGMA_S_REAL((*val)[rowtemp1+j]) <<endl;
      rowindex++;
    }
  }
  return MAGMA_SUCCESS;
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    magma_s_sparse_matrix A              sparse matrix in Magma_CSR format
        ...
        if( A.storage_type == Magma_DENSE ){
            for( magma_index_t i=0; i<(A.num_rows); i++ ){
              for( magma_index_t j=0; j<A.num_cols; j++ )
                printf("%4.2f ", MAGMA_S_REAL( A.val[i*(A.num_cols)+j] ) );
                //cout << MAGMA_S_REAL( A.val[i*(A.num_cols)+j] ) << " " ;
              cout << endl;
            }
        }
        else if( A.storage_type == Magma_CSR ){
            magma_s_sparse_matrix C;
            magma_s_mconvert( A, &C, A.storage_type, Magma_DENSE );
            magma_s_mvisu(  C );
            magma_s_mfree(&C);
        }
        else{
            ...
            magma_s_mconvert( C, &D, Magma_CSR, Magma_DENSE );
            magma_s_mvisu(  D );
            magma_s_mfree(&C);
            magma_s_mfree(&D);
        }
...
magma_int_t magma_s_csr_mtx( magma_s_sparse_matrix *A, const char *filename ){
  ...
  coo_val = (float *) malloc(A->nnz*sizeof(float));
  assert( coo_val != NULL);
  ...
  //printf("detected symmetric case\n");
    ...
    magma_index_t* new_row;
    magma_index_t* new_col;
    magma_smalloc_cpu( &new_val, true_nonzeros );
  ...
  */
  ...
  if( csr_compressor > 0){ // run the CSR compressor to remove zeros
      ...
      magma_s_mtransfer( *A, &B, Magma_CPU, Magma_CPU ); 
      magma_s_csr_compressor(&(A->val), 
                        &(A->row),
                         &(A->col), 
     ...
     // printf(" remaining nonzeros:%d ", B.nnz); 
      ...
      magma_s_mtransfer( B, A, Magma_CPU, Magma_CPU ); 
      magma_s_mfree( &B ); 
     // printf("done.\n");
  }
  return MAGMA_SUCCESS;
