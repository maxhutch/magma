/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from magma_ztranspose.cpp normal z -> d, Fri May 30 10:41:45 2014
       @author Hartwig Anzt
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    double *val              value array of input matrix 
    ...
    double **new_val         value array of transposed matrix 
...
magma_int_t 
magma_d_mtranspose( magma_d_sparse_matrix A, magma_d_sparse_matrix *B ){

    if( A.memory_location == Magma_CPU ){
        if( A.storage_type == Magma_CSR ){
            d_transpose_csr( A.num_rows, A.num_cols, A.nnz, A.val, A.row, A.col, 
            ...
            magma_d_mtranspose( C, &D );
            magma_d_mconvert( D, B, Magma_CSR, A.storage_type );
            magma_d_mfree(&C);
            magma_d_mfree(&D);
        ...
        magma_d_mtranspose( C, &D );
        magma_d_mtransfer( D, B, Magma_CPU, A.memory_location );
        magma_d_mfree(&C);
        magma_d_mfree(&D);
        return MAGMA_SUCCESS;
    }
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    magma_d_sparse_matrix A             input matrix (CSR)
    magma_d_sparse_matrix *B            output matrix (CSR)
...
magma_int_t 
magma_d_csrtranspose( magma_d_sparse_matrix A, magma_d_sparse_matrix *B ){

    if( A.storage_type != Magma_CSR ){
        magma_d_sparse_matrix ACSR, BCSR;
        magma_d_mconvert( A, &ACSR, A.storage_type, Magma_CSR );
        magma_d_cucsrtranspose( ACSR, &BCSR );
        magma_d_mconvert( BCSR, B, Magma_CSR, A.storage_type );
...
magma_int_t 
magma_d_cucsrtranspose( magma_d_sparse_matrix A, magma_d_sparse_matrix *B ){
// for symmetric matrices: convert to csc using cusparse
    ...
    if( A.storage_type != Magma_CSR && A.storage_type != Magma_CSRCOO ){
        ...
        magma_d_mconvert( BCSR, B, Magma_CSR, B->storage_type );
        magma_d_mfree( &ACSR );
        magma_d_mfree( &BCSR );
        return MAGMA_SUCCESS;
    }
        ...
        magma_d_mtransfer( A, &A_d, A.memory_location, Magma_DEV );

        magma_d_mtransfer( A, &B_d, A.memory_location, Magma_DEV );
