/*
    -- MAGMA (version 1.5.0-beta3) --
       ...
       @generated from magma_z_mconverter.cpp normal z -> s, Fri Jul 18 17:34:30 2014
       @author Hartwig Anzt
*/
...
magma_s_csr_compressor(             float ** val, 
    ...
    magma_smalloc_cpu( valn, nnz_new );
...
magma_s_mconvert( magma_s_sparse_matrix A, 
    ...
    if( A.memory_location == Magma_CPU ){
        ...
        if( old_format == Magma_CSR && new_format == Magma_CSRL ){
            ...
            B->nnz = numzeros;
            magma_smalloc_cpu( &B->val, numzeros );
            magma_index_malloc_cpu( &B->row, A.num_rows+1 );
            ...
            for( magma_int_t i=0; i<A.num_rows; i++){
                ...
                for( magma_int_t j=A.row[i]; j<A.row[i+1]; j++){
                    if( A.col[j]<i){
                        ...
                        B->val[numzeros] = MAGMA_S_MAKE(1.0, 0.0);
                        B->col[numzeros] = A.col[j];
                        numzeros++;
                    }
                    else if( A.col[j] == i ){
   ...
   /*     // CSRL/CSRCSCL to CSR
        if( ( old_format == Magma_CSRL  ) 
            ...
            magma_s_mconvert( A, B, Magma_CSR, Magma_CSR);
            ...
            magma_s_sparse_matrix ELL_sorted, ELL;
            ...
            magma_smalloc_cpu( &ELL.val, maxrowlength*A.num_rows );
            ...
            magma_smalloc_cpu( &ELL_sorted.val, maxrowlength*A.num_rows );
            magma_index_malloc_cpu( &ELL_sorted.col, maxrowlength*A.num_rows );

            for( magma_int_t i=0; i<(maxrowlength*A.num_rows); i++){
                ELL.val[i] = MAGMA_S_MAKE(0., 0.);
                ELL.col[i] =  -1;
                ELL_count[i] =  0;
                ELL_sorted.val[i] = MAGMA_S_MAKE(0., 0.);
                ELL_sorted.col[i] =  -1;
            }
            ...
            magma_s_mconvert( ELL_sorted, B, Magma_ELLPACK, Magma_CSR );

            magma_s_mfree( &ELL_sorted );
            magma_s_mfree( &ELL );
            free( ELL_count );*/
...
*/
 ...
 /*       // CSR to CSRCSCL
        ...
        if( old_format == Magma_CSR && new_format == Magma_CSRCSCL ){
            ...
            magma_s_sparse_matrix C;
            //C.diagorder_type = Magma_UNITY;
            magma_s_mconvert( A, &C, Magma_CSR, Magma_CSRL );
            // fill in information for B
        ...
        if( old_format == Magma_CSR && new_format == Magma_CSRCSCU ){
            ...
            magma_s_sparse_matrix C2;
            magma_s_mconvert( A, &C2, Magma_CSR, Magma_CSRL);
        ...
        if( old_format == Magma_CSR && new_format == Magma_ELLPACK ){
            ...
            //printf( "Conversion to ELLPACK with %d elements per row: ",
                                                            // maxrowlength );
            //fflush(stdout);
            magma_smalloc_cpu( &B->val, maxrowlength*A.num_rows );
        ...
        if( old_format == Magma_ELLPACK && new_format == Magma_CSR ){
            ...
            magma_s_csr_compressor(&A.val, &row_tmp, &A.col, 
        ...
        if( old_format == Magma_ELL && new_format == Magma_CSR ){
            ...
            float *val_tmp;
            magma_smalloc_cpu( &val_tmp, A.num_rows*A.max_nnz_row );
            magma_index_malloc_cpu( &row_tmp, A.num_rows+1 );
            ...
            //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros. 
            //The CSR compressor removes these
            magma_s_csr_compressor(&val_tmp, &row_tmp, &col_tmp, 
                       &B->val, &B->row, &B->col, &B->num_rows ); 
        ...
        if( old_format == Magma_ELLD && new_format == Magma_CSR ){
            ...
            float *val_tmp2;
            magma_smalloc_cpu( &val_tmp2, A.num_rows*A.max_nnz_row );
            ...
            //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros. 
            //The CSR compressor removes these
            magma_s_csr_compressor(&val_tmp2, &row_tmp, &col_tmp2, 
                       &B->val, &B->row, &B->col, &B->num_rows ); 
        ...
        if( old_format == Magma_CSR && new_format == Magma_ELLDD ){         
            ...
            // conversion
            magma_int_t i;
            magma_s_sparse_matrix A_tmp;
            if( B->diagorder_type == Magma_DIAGFIRST ){
                magma_s_mconvert( A, &A_tmp, Magma_CSR, Magma_ELLD );
            }
            else{
                magma_s_mconvert( A, &A_tmp, Magma_CSR, Magma_ELLPACK );
            }
            ...
            for( i=0; i<A.num_rows; i++){
                B->val[ i*maxrowlength ] = MAGMA_S_MAKE(1.0, 0.0);
            }
            for( i=0; i<off; i++){
        ...
        if( old_format == Magma_ELLDD && new_format == Magma_CSR ){         
            ...
            magma_int_t i, j;
            magma_s_sparse_matrix A_tmp;
            magma_smalloc_cpu( &A_tmp.val, off );
            ...
            for( i=0; i<A.num_rows; i++){
                for( j=0; j<maxrowlength; j++){
                    ...
                    float val = A.val[ off + maxrowlength*i+j ];
            ...
            if( A.diagorder_type == Magma_DIAGFIRST )
                magma_s_mconvert(A_tmp, B, Magma_ELLD, Magma_CSR );
            else
                magma_s_mconvert(A_tmp, B, Magma_ELLPACK, Magma_CSR );
        ...
        if( old_format == Magma_CSR && new_format == Magma_ELLRT ){
            ...
            magma_smalloc_cpu( &B->val, rowlength*A.num_rows );
        ...
        if( old_format == Magma_CSR && 
            ...
            //fflush(stdout);
            magma_smalloc_cpu( &B->val, B->row[slices] );
        ...
        if( (old_format == Magma_SELLC || old_format == Magma_SELLP )
            ...
            magma_index_t *col_tmp;
            float *val_tmp;
            magma_smalloc_cpu( &val_tmp, A.max_nnz_row*(A.num_rows+C) );
        ...
        if( old_format == Magma_CSR && new_format == Magma_DENSE ){
            ...
            // conversion
            magma_smalloc_cpu( &B->val, A.num_rows*A.num_cols );
        ...
        if( old_format == Magma_CSR && new_format == Magma_BCSR ){
            ...
            magma_smalloc_cpu( &B->val, numblocks * size_b * size_b );
            ...
            float *transpose;
            magma_smalloc( &transpose, size_b*size_b );
            for( magma_int_t i=0; i<B->numblocks; i++ ){
                magma_ssetvector( size_b*size_b, B->val+i*size_b*size_b, 1, transpose, 1 );
                magmablas_stranspose_inplace( size_b, transpose, size_b );
                magma_sgetvector( size_b*size_b, transpose, 1, B->val+i*size_b*size_b, 1 );
            }
            /*
        ...
        if( old_format == Magma_BCSR && new_format == Magma_CSR ){
            ...
            //fflush(stdout);
            float *val_tmp;      
            magma_smalloc_cpu( &val_tmp, A.row[ r_blocks ] * size_b * size_b );
            magma_index_t *row_tmp;            
            ...
            float *transpose, *val_tmp2;
            magma_smalloc( &transpose, size_b*size_b );
            magma_smalloc_cpu( &val_tmp2, size_b*size_b*A.numblocks );
            for( magma_int_t i=0; i<A.numblocks; i++ ){
                magma_ssetvector( size_b*size_b, A.val+i*size_b*size_b, 1, transpose, 1 );
                magmablas_stranspose_inplace( size_b, transpose, size_b );
                magma_sgetvector( size_b*size_b, transpose, 1, val_tmp2+i*size_b*size_b, 1 );
            }
        ...
        if( old_format == Magma_CSR && new_format == Magma_BCSR ){
            ...
            magma_smalloc( &B->val, nnzb*size_b*size_b );
        ...
        if( old_format == Magma_CSR && new_format == Magma_COO ){
            ...
            magma_scopyvector( A.nnz, A.val, 1, B->val, 1 );
        ...
        if( old_format == Magma_COO && new_format == Magma_CSR ){
            ...
            magma_s_sparse_matrix hA, hB;
            magma_s_mtransfer( A, &hA, A.memory_location, Magma_CPU );
            magma_s_mconvert( hA, &hB, old_format, new_format );
            magma_s_mtransfer( hB, B, Magma_CPU, A.memory_location );
            magma_s_mfree( &hA );
            magma_s_mfree( &hB );   
...
magma_int_t 
magma_s_LUmergein( magma_s_sparse_matrix L, 
...
magma_s_LUmerge( magma_s_sparse_matrix L, 
    ...
    magma_s_mconvert( L, B, Magma_CSRL, Magma_CSR );
    magma_int_t i, j, k;
