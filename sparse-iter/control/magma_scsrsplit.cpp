/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from magma_zcsrsplit.cpp normal z -> s, Sat Nov 15 19:54:23 2014
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    if (  A.memory_location == Magma_CPU &&
        ...
        magma_s_mconvert( Ah, &ACSR, A.storage_type, Magma_CSR, queue );

        magma_scsrsplit( bsize, ACSR, &DCSR, &RCSR, queue );

        magma_s_mconvert( DCSR, &Dh, Magma_CSR, A.storage_type, queue );
        magma_s_mconvert( RCSR, &Rh, Magma_CSR, A.storage_type, queue );

        magma_s_mtransfer( Dh, D, Magma_CPU, A.memory_location, queue );
        magma_s_mtransfer( Rh, R, Magma_CPU, A.memory_location, queue );

        magma_s_mfree( &Ah, queue );
        magma_s_mfree( &ACSR, queue );
        magma_s_mfree( &Dh, queue );
        magma_s_mfree( &DCSR, queue );
        magma_s_mfree( &Rh, queue );
        magma_s_mfree( &RCSR, queue );
