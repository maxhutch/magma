/*
    -- MAGMA (version 1.6.1) --
       ...
       @generated from magma_zmtransfer.cpp normal z -> c, Fri Jan 30 19:00:32 2015
       @author Hartwig Anzt
*/
    ...
    magma_queue_t queue )
{
    ...
    if ( src == Magma_CPU && dst == Magma_DEV ) {
        ...
        if ( A.storage_type == Magma_ELLPACKT || A.storage_type == Magma_ELL ) {
            ...
            B->diameter = A.diameter;
            // memory allocation
            stat_dev += magma_cmalloc( &B->dval, A.num_rows * A.max_nnz_row ); 
            if( stat_dev != 0 ){ goto CLEANUP; }
        ...
        if ( A.storage_type == Magma_ELLRT ) {
            ...
            // memory allocation
            stat_dev += magma_cmalloc( &B->dval, A.num_rows * rowlength ); 
            if( stat_dev != 0 ){ goto CLEANUP; }
        ...
        if ( A.storage_type == Magma_BCSR ) {
            ...
            // memory allocation
            stat_dev += magma_cmalloc( &B->dval, size_b*size_b*A.numblocks ); 
            if( stat_dev != 0 ){ goto CLEANUP; }
    ...
    if ( src == Magma_CPU && dst == Magma_CPU ) {
        ...
        if ( A.storage_type == Magma_ELLPACKT || A.storage_type == Magma_ELL ) {
            ...
            B->diameter = A.diameter;
            // memory allocation
            stat_cpu += magma_cmalloc_cpu( &B->val, A.num_rows*A.max_nnz_row ); 
            if( stat_cpu != 0 ){ goto CLEANUP; }
    ...
    if ( src == Magma_DEV && dst == Magma_CPU ) {
        ...
        if ( A.storage_type == Magma_BCSR ) {
            ...
            magma_int_t r_blocks = ceil( (float)A.num_rows / (float)size_b );
                    // max number of blocks per column
            // memory allocation
            stat_cpu += magma_cmalloc_cpu( &B->val, A.numblocks*A.blocksize*A.blocksize ); 
            if( stat_cpu != 0 ){ goto CLEANUP; }
