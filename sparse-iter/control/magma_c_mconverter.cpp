...
magma_c_mconvert( magma_c_sparse_matrix A, 
   ...
   /*     // CSRL/CSRCSCL to CSR
        if( ( old_format == Magma_CSRL  ) 
            ...
            magma_c_sparse_matrix ELL_sorted, ELL;
            ...
            for( magma_int_t i=0; i<(maxrowlength*A.num_rows); i++){
                ...
                ELL.col[i] =  -1;
                ELL_count[i] =  0;
                ELL_sorted.val[i] = MAGMA_C_MAKE(0., 0.);
                ELL_sorted.col[i] =  -1;
            }
            ...
            magma_c_mconvert( ELL_sorted, B, Magma_ELLPACK, Magma_CSR );

            magma_c_mfree( &ELL_sorted );
            magma_c_mfree( &ELL );
            free( ELL_count );*/
...
*/
 ...
 /*       // CSR to CSRCSCL
        ...
        if( old_format == Magma_CSR && new_format == Magma_CSRCSCL ){
            ...
            magma_c_mconvert( A, &C, Magma_CSR, Magma_CSRL );
            // fill in information for B
        ...
        if( old_format == Magma_CSR && new_format == Magma_CSRCSCU ){
            ...
            magma_c_sparse_matrix C2;
            magma_c_mconvert( A, &C2, Magma_CSR, Magma_CSRL);
        ...
        if( old_format == Magma_CSR && new_format == Magma_ELLDD ){         
            ...
            if( B->diagorder_type == Magma_DIAGFIRST ){
                magma_c_mconvert( A, &A_tmp, Magma_CSR, Magma_ELLD );
            }
            else{
                magma_c_mconvert( A, &A_tmp, Magma_CSR, Magma_ELLPACK );
            }
            ...
            for( i=0; i<A.num_rows; i++){
                B->val[ i*maxrowlength ] = MAGMA_C_MAKE(1.0, 0.0);
            }
            for( i=0; i<off; i++){
        ...
        if( old_format == Magma_ELLDD && new_format == Magma_CSR ){         
            ...
            for( i=0; i<A.num_rows; i++){
                for( j=0; j<maxrowlength; j++){
                    ...
                    magmaFloatComplex val = A.val[ off + maxrowlength*i+j ];
            ...
            if( A.diagorder_type == Magma_DIAGFIRST )
                magma_c_mconvert(A_tmp, B, Magma_ELLD, Magma_CSR );
            else
                magma_c_mconvert(A_tmp, B, Magma_ELLPACK, Magma_CSR );
...
magma_c_LUmergein( magma_c_sparse_matrix L, 
    ...
    if( (L.storage_type == Magma_CSRCOO) )
    {
        for(  i=0; i<L.nnz; i++){
            ...
            magmaFloatComplex lval = L.val[ i ];
    ...
    if( U.storage_type == Magma_CSRCOO ){
        for(  i=0; i<U.nnz; i++){
            ...
            magmaFloatComplex lval = U.val[ i ];
            for( k=B->row[lrow]; k<B->row[lrow+1]; k++){
                if( B->col[ k ] == lcol ){
                    B->val[ k ] = lval;
                    break;
...
magma_c_LUmerge( magma_c_sparse_matrix L, 
    ...
    magma_c_mconvert( L, B, Magma_CSRL, Magma_CSR );
    magma_int_t i, j, k;
    ...
    if( U.storage_type == Magma_CSRU )
    {
        for(  i=0; i<U.num_rows; i++){
            for( j=U.row[i]; j<U.row[i+1]; j++){
                ...
                magmaFloatComplex lval = U.val[ j ];
