/*
    -- MAGMA (version 1.5.0-beta3) --
       ...
       @generated from magma_z_transfer.cpp normal z -> c, Fri Jul 18 17:34:30 2014
       @author Hartwig Anzt
*/
...
magma_c_mtransfer( magma_c_sparse_matrix A,
    ...
    if( src == Magma_CPU && dst == Magma_DEV ){
        ...
        if( A.storage_type == Magma_CSR || A.storage_type == Magma_CSC
            ...
            // memory allocation
            stat = magma_cmalloc( &B->val, A.nnz );
            if( stat != 0 )
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_csetvector( A.nnz, A.val, 1, B->val, 1 );
   ...
   /*     //CSRCSC-type
        if( A.storage_type == Magma_CSRCSCL ||
            ...
            if( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
         //   stat = magma_cmalloc( &B->diag, A.num_rows );
         //   if( stat != 0 )
         //   {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = magma_index_malloc( &B->row, (A.num_rows + 1) );
            if( stat != 0 )
            ...
            // data transfer
            magma_csetvector( A.nnz, A.val, 1, B->val, 1 );
         //   magma_csetvector( A.num_rows, A.diag, 1, B->diag, 1 );
            magma_index_setvector( (A.num_rows+1), A.row, 1, B->row, 1 );
            magma_index_setvector( A.nnz, A.col, 1, B->col, 1 );
        ...
        if( A.storage_type == Magma_ELLPACK ){
            ...
            B->diameter = A.diameter;
            // memory allocation
            stat = magma_cmalloc( &B->val, A.num_rows * A.max_nnz_row );
            if( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_csetvector( A.num_rows * A.max_nnz_row, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_ELLDD ){
            ...
            B->diameter = A.diameter;
            // memory allocation
            stat = magma_cmalloc( &B->val, 2*A.num_rows * A.max_nnz_row );
            if( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_csetvector( 2 * A.num_rows * A.max_nnz_row, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_ELLRT ){
            ...
            // memory allocation
            stat = magma_cmalloc( &B->val, A.num_rows * rowlength );
            if( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_csetvector( A.num_rows * rowlength, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_BCSR ){
            ...
            // memory allocation
            stat = magma_cmalloc( &B->val, size_b*size_b*A.numblocks );
            if( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            ...
            // data transfer
            magma_csetvector( size_b*size_b*A.numblocks, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_DENSE ){
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_csetvector( A.num_rows*A.num_cols, A.val, 1, B->val, 1 );
        }
    }
   ...
   //     }
    ...
    if( src == Magma_DEV && dst == Magma_CPU ){
        ...
        if( A.storage_type == Magma_CSR || A.storage_type == Magma_CSC
            ...
            magma_index_malloc_cpu( &B->col, A.nnz );
            // data transfer
            magma_cgetvector( A.nnz, A.val, 1, B->val, 1 );
 ...
 /*       //CSRCSC-type
        if( A.storage_type == Magma_CSRCSCL ||
          ...
          //  magma_cmalloc_cpu( &B->diag, A.num_rows );
            ...
            // data transfer
            magma_cgetvector( A.nnz, A.val, 1, B->val, 1 );
       //     magma_cgetvector( A.num_rows, A.diag, 1, B->diag, 1 );
            magma_index_getvector( (A.num_rows+1), A.row, 1, B->row, 1 );
            magma_index_getvector( A.nnz, A.col, 1, B->col, 1 );
        ...
        if( A.storage_type == Magma_ELLPACK ){
            ...
            magma_index_malloc_cpu( &B->col, A.num_rows*A.max_nnz_row );
            // data transfer
            magma_cgetvector( A.num_rows*A.max_nnz_row, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_ELLDD ){
            ...
            magma_index_malloc_cpu( &B->col, 2*A.num_rows*A.max_nnz_row );
            // data transfer
            magma_cgetvector( 2*A.num_rows*A.max_nnz_row, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_ELLRT ){
            ...
            magma_index_malloc_cpu( &B->col, rowlength*A.num_rows );
            // data transfer
            magma_cgetvector( A.num_rows*rowlength, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_BCSR ){
            ...
            magma_index_malloc_cpu( &B->blockinfo, r_blocks * c_blocks );
            // data transfer
            magma_cgetvector( A.numblocks * A.blocksize * A.blocksize, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_DENSE ){
            ...
            magma_cmalloc_cpu( &B->val, A.num_rows*A.num_cols );
            // data transfer
            magma_cgetvector( A.num_rows*A.num_cols, A.val, 1, B->val, 1 );
        }
    }
  ...
  /*      //CSRCSC-type
        if( A.storage_type == Magma_CSRCSCL ||
            ...
            // data transfer
            magma_ccopyvector( A.nnz, A.val, 1, B->val, 1 );
            //magma_ccopyvector( A.num_rows, A.diag, 1, B->diag, 1 );
            magma_index_copyvector( (A.num_rows+1), A.row, 1, B->row, 1 );
            magma_index_copyvector( A.nnz, A.col, 1, B->col, 1 );
        ...
        if( A.storage_type == Magma_ELLPACK ){
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_ccopyvector( A.num_rows*A.max_nnz_row, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_ELLDD ){
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_ccopyvector( 2 * A.num_rows*A.max_nnz_row, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_ELLRT ){
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_ccopyvector( A.num_rows * rowlength, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_BCSR ){
            ...
            magma_index_malloc_cpu( &B->blockinfo, r_blocks * c_blocks );
            // data transfer
            magma_ccopyvector( size_b*size_b*A.numblocks, A.val, 1, B->val, 1 );
        ...
        if( A.storage_type == Magma_DENSE ){
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_ccopyvector( A.num_rows*A.num_cols, A.val, 1, B->val, 1 );
        }
    }
...
magma_c_vtransfer( magma_c_vector x,
    ...
    if( src == Magma_CPU && dst == Magma_DEV ){
        ...
        y->nnz = x.nnz;
        // memory allocation
        stat = magma_cmalloc( &y->val, x.num_rows );
        if( stat != 0 )
