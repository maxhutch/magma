...
extern "C"
magma_int_t 
magma_silustruct( magma_s_sparse_matrix *A, magma_int_t levels ){
    ...
    if( A->memory_location == Magma_CPU && A->storage_type == Magma_CSR ){
        ...
        magma_s_sparse_matrix B, L, U, L_d, U_d, LU_d;

        magma_s_mconvert( *A, &B, Magma_CSR, Magma_CSR );

        for( i=0; i<levels; i++ ){

            magma_s_mconvert( *A, &L, Magma_CSR, Magma_CSRL );
            magma_s_mconvert( *A, &U, Magma_CSR, Magma_CSRU );
            ...
            magma_s_mtransfer(LU_d, A, Magma_DEV, Magma_CPU);
            magma_s_mfree( &L_d );
            magma_s_mfree( &U_d );
            magma_s_mfree( &LU_d );
        ...
        for( i=0; i<A->nnz; i++ )
            A->val[i] = MAGMA_S_MAKE( (float)(0.0/A->nnz), 0.0 );
        ...
        magma_s_sparse_matrix hA, CSRCOOA;
        magma_storage_t A_storage = A->storage_type;
        magma_location_t A_location = A->memory_location;
        magma_s_mtransfer( *A, &hA, A->memory_location, Magma_CPU );
        magma_s_mconvert( hA, &CSRCOOA, hA.storage_type, Magma_CSR );

        magma_silustruct( &CSRCOOA, levels );

        magma_s_mfree( &hA );
        magma_s_mfree( A );
        magma_s_mconvert( CSRCOOA, &hA, Magma_CSR, A_storage );
        magma_s_mtransfer( hA, A, Magma_CPU, A_location );
        magma_s_mfree( &hA );
        magma_s_mfree( &CSRCOOA );    
