/*
    -- MAGMA (version 1.6.1) --
       ...
       @generated from magma_zmconverter.cpp normal z -> c, Fri Jan 30 19:00:32 2015
       @author Hartwig Anzt
*/
    ...
    magma_queue_t queue )
{
    ...
    stat_cpu += magma_cmalloc_cpu( valn, nnz_new );
    ...
    if ( A.memory_location == Magma_CPU ) {
        ...
        if( old_format == Magma_CSR ){
            ...
            if ( new_format == Magma_CSR ) {
                ...
                stat_cpu += magma_cmalloc_cpu( &B->val, A.nnz );
            ...
            // CSR to CSRL
                ...
                B->nnz = numzeros;
                stat_cpu += magma_cmalloc_cpu( &B->val, numzeros );
                stat_cpu += magma_index_malloc_cpu( &B->row, A.num_rows+1 );
            ...
            // CSR to ELLPACKT (using row-major storage)  
                ...
                //printf( "Conversion to ELLPACK with %d elements per row: ",
                                                                // maxrowlength );
                //fflush(stdout);
                stat_cpu += magma_cmalloc_cpu( &B->val, maxrowlength*A.num_rows );
            ...
            // CSR to ELLRT (also ELLPACKRT)
                ...
                stat_cpu += magma_cmalloc_cpu( &B->val, rowlength*A.num_rows );
            ...
            // so the rowlength is padded (SELLP) to a multiple of the alignment
                ...
                //fflush(stdout);
                stat_cpu += magma_cmalloc_cpu( &B->val, B->row[slices] );
            ...
            // CSR to DENSE
                ...
                // conversion
                stat_cpu += magma_cmalloc_cpu( &B->val, A.num_rows*A.num_cols );
                if( stat_cpu != 0 ){ goto CLEANUP; }
            ...
            // CSR to BCSR
                ...
                stat_cpu += magma_cmalloc_cpu( &B->val, numblocks * size_b * size_b );
                ...
                stat_dev += magma_cmalloc( &transpose, size_b*size_b );
        ...
        // anything to CSR
            ...
            // ELL (column-major) to CSR
                ...
                stat_cpu += magma_cmalloc_cpu( &val_tmp, A.num_rows*A.max_nnz_row );
                stat_cpu += magma_index_malloc_cpu( &row_tmp, A.num_rows+1 );
            ...
            // ELLD (ELLPACK with diagonal element first) to CSR
                ...
                stat_cpu += magma_cmalloc_cpu( &val_tmp2, A.num_rows*A.max_nnz_row );
            ...
            // SELLP to CSR    
                ...
                magma_index_t *col_tmp;
                magmaFloatComplex *val_tmp;
                stat_cpu += magma_cmalloc_cpu( &val_tmp, A.max_nnz_row*(A.num_rows+C) );
            ...
            // DENSE to CSR
                ...
                stat_cpu += magma_cmalloc_cpu( &B->val, B->nnz);
            ...
            // BCSR to CSR
                ...
                //fflush(stdout);
                magmaFloatComplex *val_tmp;      
                stat_cpu += magma_cmalloc_cpu( &val_tmp, A.row[ r_blocks ] * size_b * size_b );
                magma_index_t *row_tmp;            
                ...
                magmaFloatComplex *transpose, *val_tmp2;
                stat_dev +=  magma_cmalloc( &transpose, size_b*size_b );
                if( stat_dev != 0 ){
                ...
                stat_cpu += magma_cmalloc_cpu( &val_tmp2, size_b*size_b*A.numblocks );
                if( stat_cpu != 0 ){
                    magma_free_cpu( val_tmp2 );
                    printf("error: memory allocation.\n");
                    return MAGMA_ERR_HOST_ALLOC;
        ...
        if ( old_format == Magma_CSR && new_format == Magma_CSR ) {
            magma_c_mtransfer( A, B, Magma_DEV, Magma_DEV, queue );
        }
        // CSR to DENSE
        if ( old_format == Magma_CSR && new_format == Magma_DENSE ) {
            ...
            stat_dev += magma_cmalloc( &B->dval, A.num_rows*A.num_cols );
            if( stat_cpu != 0 ){ goto CLEANUP; }
        ...
        // DENSE to CSR    
            ...
            stat_dev += magma_cmalloc( &B->dval, B->nnz );
        ...
        // CSR to BCSR
            ...
            stat_dev += magma_cmalloc( &B->dval, nnzb*size_b*size_b );
        ...
        // COO to CSR
            ...
            magma_c_mconvert( hA, &hB, old_format, new_format, queue );
            magma_c_mtransfer( hB, B, Magma_CPU, A.memory_location, queue );
            magma_c_mfree( &hA, queue );
            magma_c_mfree( &hB, queue );   
        }
    }
