    ...
    magma_queue_t queue )
{
    if ( A.memory_location == Magma_DEV && A.storage_type == Magma_CSR ) {
        ...
        *row = A.drow;
        ...
        magma_d_sparse_matrix A_DEV, A_CSR;
        magma_d_mconvert( A, &A_CSR, A.storage_type, Magma_CSR, queue ); 
        magma_d_mtransfer( A_CSR, &A_DEV, A.memory_location, Magma_DEV, queue ); 
        magma_dcsrget_gpu( A_DEV, m, n, row, col, val, queue );
        magma_d_mfree( &A_CSR, queue );
        magma_d_mfree( &A_DEV, queue );
    }
    return MAGMA_SUCCESS;
}
