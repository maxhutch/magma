/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from matrix_zio.cpp normal z -> d, Fri May 30 10:41:46 2014
       @author Hartwig Anzt
*/
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    double **val             value array of CSR output 
...
magma_int_t read_d_csr_from_mtx(    magma_storage_t *type, 
  ...
  printf("detected symmetric case\n");
    ...
    magma_index_t* new_col = 
        (magma_index_t *) malloc(true_nonzeros*sizeof(magma_index_t)) ; 
    double* new_val = 
      (double *) malloc(true_nonzeros*sizeof(double)) ; 
...
extern "C"
magma_int_t write_d_csrtomtx( magma_d_sparse_matrix B, const char *filename){

    write_d_csr_mtx( B.num_rows, B.num_cols, B.nnz, &B.val, &B.row, &B.col, 
...
magma_int_t write_d_csr_mtx(    magma_int_t n_row, 
  ...
  if( MajorType == MagmaColMajor){
    ...
    d_transpose_csr( n_row, n_col, nnz, *val, *row, *col, 
    ...
    {    
      ...
      for(j=0; j<rowtemp2-rowtemp1; j++)  
        file << ((new_col)[rowtemp1+j]+1) <<" "<< rowindex <<" "<< 
                        MAGMA_D_REAL((new_val)[rowtemp1+j]) << std::endl;
      rowindex++;
    }
    printf(" done\n");
    ...
    for(i=0; i<n_col; i++)
    {
      ...
      for(j=0; j<rowtemp2-rowtemp1; j++)  
        file << rowindex <<" "<< ((*col)[rowtemp1+j]+1) <<" "<< 
                        MAGMA_D_REAL((*val)[rowtemp1+j]) << std::endl;
      rowindex++;
    }
    printf(" done\n");
  }
    ...
    for(i=0; i<n_col; i++)
    {
      ...
      for(j=0; j<rowtemp2-rowtemp1; j++)  
        cout<< rowindex <<" "<< (*col)[rowtemp1+j]+1 <<" "<< 
                        MAGMA_D_REAL((*val)[rowtemp1+j]) <<endl;
      rowindex++;
    }
  }
  return MAGMA_SUCCESS;
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    magma_d_sparse_matrix A              sparse matrix in Magma_CSR format
...
magma_int_t magma_d_mvisu( magma_d_sparse_matrix A )
{
    if( A.memory_location == Magma_CPU ){
        if( A.storage_type == Magma_DENSE ){
            for( magma_index_t i=0; i<(A.num_rows); i++ ){
              for( magma_index_t j=0; j<A.num_cols; j++ )
                printf("%4.2f ", MAGMA_D_REAL( A.val[i*(A.num_cols)+j] ) );
                //cout << MAGMA_D_REAL( A.val[i*(A.num_cols)+j] ) << " " ;
              cout << endl;
            }
        }
        else if( A.storage_type == Magma_CSR ){
            magma_d_sparse_matrix C;
            magma_d_mconvert( A, &C, A.storage_type, Magma_DENSE );
            magma_d_mvisu(  C );
            magma_d_mfree(&C);
        }
        else{
            magma_d_sparse_matrix C, D;
            magma_d_mconvert( A, &C, A.storage_type, Magma_CSR );
            magma_d_mconvert( C, &D, Magma_CSR, Magma_DENSE );
            magma_d_mvisu(  D );
            magma_d_mfree(&C);
            magma_d_mfree(&D);
        }
        ...
        magma_d_sparse_matrix C;
        magma_d_mtransfer( A, &C, A.memory_location, Magma_CPU );
        magma_d_mvisu(  C );
        magma_d_mfree(&C);
    }
...
/*  -- MAGMA (version 1.5.0-beta2) --
    ...
    magma_d_sparse_matrix *A             matrix in magma sparse matrix format
    const char * filename                filname of the mtx matrix
...
magma_int_t magma_d_csr_mtx( magma_d_sparse_matrix *A, const char *filename ){
  ...
  coo_val = (double *) malloc(A->nnz*sizeof(double));
  assert( coo_val != NULL);
  ...
  //printf("detected symmetric case\n");
    ...
    magma_index_t* new_row;
    magma_index_t* new_col;
    magma_dmalloc_cpu( &new_val, true_nonzeros );
  ...
  */
  ...
  if( csr_compressor > 0){ // run the CSR compressor to remove zeros
      ...
      magma_d_mtransfer( *A, &B, Magma_CPU, Magma_CPU ); 
      magma_d_csr_compressor(&(A->val), 
                        &(A->row),
                         &(A->col), 
     ...
     // printf(" remaining nonzeros:%d ", B.nnz); 
      ...
      magma_d_mtransfer( B, A, Magma_CPU, Magma_CPU ); 
      magma_d_mfree( &B ); 
     // printf("done.\n");
  }
  return MAGMA_SUCCESS;
