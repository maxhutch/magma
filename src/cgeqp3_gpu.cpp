/*
    -- MAGMA (version 1.5.0-beta3) --
       ...
       @generated from zgeqp3_gpu.cpp normal z -> c, Fri Jul 18 17:34:17 2014
...
magma_cgeqp3_gpu( magma_int_t m, magma_int_t n,
                  ...
                  magma_int_t *info )
{
    ...
    *info = 0;
    ...
    for (j = 0; j < n; ++j) {
        if (jpvt[j] != 0) {
            if (j != nfxd) {
                blasf77_cswap(&m, dA(0, j), &ione, dA(0, nfxd), &ione);  // TODO: matrix not on CPU!
                jpvt[j]    = jpvt[nfxd];
                jpvt[nfxd] = j + 1;
            }
            else {
    ...
    if (nfxd > 0) {
        na = min(m,nfxd);
        lapackf77_cgeqrf(&m, &na, A, &lda, tau, dwork, &lwork, info);
        if (na < n) {
            n_j = n - na;
    ...
    if (nfxd < minmn) {
...
#endif
        /*for (j = nfxd; j < n; ++j) {
            rwork[j] = cblas_scnrm2(sm, dA(nfxd, j), ione);
            rwork[n + j] = rwork[j];
        }*/
        ...
        if (j < minmn) {
            ...
            lapackf77_claqp2(&m, &n_j, &j, dA(0, j), &lda, &jpvt[j],
