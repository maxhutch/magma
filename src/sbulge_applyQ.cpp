 ...
 *     @author Stan Tomov
 *
 *     @generated s Tue Aug 13 16:44:39 2013
 *
 */
...
#define TAU(m)   &(TAU[(m)])
#define T(m)     &(T[(m)])
extern "C" void magma_sbulge_applyQ(magma_int_t WANTZ, char SIDE, magma_int_t NE, magma_int_t N, magma_int_t NB, magma_int_t Vblksiz, float *E, magma_int_t LDE, float *V, float *TAU, float *T, magma_int_t *INFO, float *dV, float *dT, float *dE, magma_int_t copytype )
{
    ...
    *INFO=0;
     ...
     * */
    if(copytype>0) magma_ssetmatrix( LDV, blkcnt*Vblksiz, V, LDV, dV, LDV );
    if(copytype>1) magma_ssetmatrix( LDT, blkcnt*Vblksiz, T, LDT, dT, LDT );
    if(copytype>2) magma_ssetmatrix( N, NE, E, N, dE, N );
    float *dwork;
    magma_int_t ldwork;
    ldwork  = NE;
    LWORK   = 2*N*max(Vblksiz,64);
    if(MAGMA_SUCCESS != magma_smalloc( &dwork, LWORK )) { 
    ...
    if(WANTZ==1) 
    {
        ...
        magmablas_slaset_identity(N, N, dE, N);        
    }
        ...
        for (bg = nbGblk; bg>0; bg--)
        {
           ...
           for (m = rownbm; m>0; m--)
           {
               ...
               if((vlen>0)&&(vnb>0)){
                   if(WANTZ==1){
                      len =  N-colst;    
                      magma_slarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, vlen, len, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,colst), LDE, dwork, len);
                   }else{
                      magma_slarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, vlen, NE, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, NE);
                   }
               }           
           }
        }
        ...
        for (m = rownbm; m>0; m--)
        {
           ...
           for (n = nbgr; n>0; n--)
           {
               ...
               if((vlen>0)&&(vnb>0)){
                   ...
                   magma_slarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, vlen, N1, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, N1);
                   magmablasSetKernelStream(stream[1]);        
                   magma_slarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, vlen, N2, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,N1), LDE, &dwork[N1*Vblksiz], N2);
        ...
        for (bg =1; bg<=nbGblk; bg++)
        {
           ...
           for (m = 1; m<=rownbm; m++)
           {
               ...
               if((vlen>0)&&(vnb>0)){
                #if defined(USESTREAM)
                   magmablasSetKernelStream(stream[0]);                       
                   magma_slarfb_gpu( MagmaRight, MagmaNoTrans, MagmaForward, MagmaColumnwise, N1, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(0, fst), LDE, dwork, N1);
                   magmablasSetKernelStream(stream[1]);        
                   magma_slarfb_gpu( MagmaRight, MagmaNoTrans, MagmaForward, MagmaColumnwise, N2, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(N1, fst), LDE, &dwork[N1*Vblksiz], N2);
                #else
                   magma_slarfb_gpu( MagmaRight, MagmaNoTrans, MagmaForward, MagmaColumnwise, NE, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(0, fst), LDE, dwork, NE);
                #endif
               } 
           }
        }
