/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated d Tue Aug 13 16:44:07 2013
...
magma_dpotrf_m(magma_int_t num_gpus0, char uplo, magma_int_t n,
...
/*  -- MAGMA (version 1.4.0) --
    ...
    *info = 0;
    ...
    for (d=0; d<num_gpus; d++ ) {
        magma_setdevice(d);
        if (MAGMA_SUCCESS != magma_dmalloc( &dt[d], NB*lddla + max(2,num_gpus)*nb*ldda )) {
    ...
    if (upper) {
        ...
        for( J=0; J<n; J+=NB ) {
            ...
            magma_dhtodpo( num_gpus, &uplo, JB, n, J, J, nb, a, lda, dwork, NB, stream, &iinfo);
            ...
            // using two streams
            //magma_dpotrf2_mgpu(num_gpus, uplo, JB, n-J, J, J, nb,
            //                   dwork, NB, dt, ldda, a, lda, h, stream, event, &iinfo);
            // using three streams
            magma_dpotrf3_mgpu(num_gpus, uplo, JB, n-J, J, J, nb,
                               dwork, NB, dt, ldda, a, lda, h, stream, event, &iinfo);
            if( iinfo != 0 ) {
                *info = J+iinfo;
                break;
            ...
            magma_ddtohpo(num_gpus, &uplo, JB, n, J, J, nb, NB, a, lda, dwork, NB, stream, &iinfo);
            //magma_ddtohpo(num_gpus, &uplo, JB, n, J, J, nb, 0, a, lda, dwork, NB, stream, &iinfo);
        }
    } else {
        /* ========================================================= *
        ...
        for( J=0; J<n; J+=NB ) {
            ...
            magma_dhtodpo( num_gpus, &uplo, n, JB, J, J, nb, a, lda, dwork, lddla, stream, &iinfo);
            ...
            h = (JB+nb-1)/nb; // big diagonal of big panel will be on CPU
            // using two streams
            //magma_dpotrf2_mgpu(num_gpus, uplo, n-J, JB, J, J, nb,
            //                   dwork, lddla, dt, ldda, a, lda, h, stream, event, &iinfo);
            // using three streams
            magma_dpotrf3_mgpu(num_gpus, uplo, n-J, JB, J, J, nb,
                               dwork, lddla, dt, ldda, a, lda, h, stream, event, &iinfo);
            if( iinfo != 0 ) {
                *info = J+iinfo;
                break;
            ...
            magma_ddtohpo( num_gpus, &uplo, n, JB, J, J, nb, JB, a, lda, dwork, lddla, stream, &iinfo);
    ...
    printf(" Without memory allocation: %f / %f = %f GFlop/s\n",
           FLOPS_DPOTRF(n)/1000000, GetTimerValue(start0, end0),
           FLOPS_DPOTRF(n)/(1000000*GetTimerValue(start0, end0)));
    printf(" Performance %f / %f = %f GFlop/s\n",
           FLOPS_DPOTRF(n)/1000000, chol_time,
           FLOPS_DPOTRF(n)/(1000000*chol_time));
    #endif
    return *info;
} /* magma_dpotrf_ooc */
