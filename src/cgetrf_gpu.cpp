/*
    -- MAGMA (version 1.5.0) --
       ...
       @author Stan Tomov
       @generated from zgetrf_gpu.cpp normal z -> c, Tue Sep  2 12:38:19 2014
*/
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) {
        ...
        for( i=0; i < s; i++ ) {
            ...
            magma_cgetmatrix_async( m-i*nb, nb, dAP, cols, work, ldwork,
                                    stream[0]);
            ...
            lapackf77_cgetrf( &rows, &nb, work, &ldwork, ipiv+i*nb, &iinfo);
            if ( (*info == 0) && (iinfo > 0) )
                *info = iinfo + i*nb;

            // upload i-th panel
            magma_csetmatrix_async( m-i*nb, nb, work, ldwork, dAP, maxm,
                                    stream[0]);
