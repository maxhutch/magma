...
magma_cunmtr_gpu(char side, char uplo, char trans,
...
/*  -- MAGMA (version 1.4.1) --
    ...
    DA      (device input) COMPLEX array, dimension
                                 (LDDA,M) if SIDE = 'L'
                                 (LDDA,N) if SIDE = 'R'
    ...
    *info = 0;
    ...
    if (upper) {
        magma_cunmql2_gpu(side, trans, mi, ni, nq-1, &da[ldda], ldda, tau,
        ...
        magma_cunmqr2_gpu(side, trans, mi, ni, nq-1, &da[1], ldda, tau,
