...
class magma_dapplyQ_data {

public:

    magma_dapplyQ_data(magma_int_t threads_num_, magma_int_t n_, magma_int_t ne_, magma_int_t n_gpu_,
                       magma_int_t nb_, magma_int_t Vblksiz_, double *E_, magma_int_t lde_,
                       double *V_, magma_int_t ldv_, double *TAU_,
                       double *T_, magma_int_t ldt_, double *dE_, magma_int_t ldde_)
    :
...
private:
    ...
    magma_dapplyQ_data(magma_dapplyQ_data& data); // disable copy

};

class magma_dapplyQ_id_data {
...
public:
    ...
    magma_dapplyQ_id_data(magma_int_t id_, magma_dapplyQ_data* data_)
    : id(id_), data(data_)
    {}
...
magma_dbulge_back(magma_int_t threads, char uplo, 
...
//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    if(n_gpu < ne)
    {
        ...
        #endif
        magma_dapplyQ_data data_applyQ(threads, n, ne, n_gpu, nb, Vblksiz, Z, ldz, V, ldv, TAU, T, ldt, dZ, lddz);
        ...
        {
            arg[thread] = magma_dapplyQ_id_data(thread, &data_applyQ);
            pthread_create(&thread_id[thread], &thread_attr, magma_dapplyQ_parallel_section, &arg[thread]);
        }
        arg[0] = magma_dapplyQ_id_data(0, &data_applyQ);
        ...
        magma_dbulge_applyQ_v2('L', ne, n, nb, Vblksiz, dZ, lddz, V, ldv, T, ldt, info);
        magma_device_sync();
    }
...
static void *magma_dapplyQ_parallel_section(void *arg)
{
    ...
    if(my_core_id==0)
    {
        ...
        magma_dbulge_applyQ_v2('L', n_gpu, n, nb, Vblksiz, dE, ldde, V, ldv, T, ldt, &info);
        magma_device_sync();
