/*
    -- MAGMA (version 2.0.0-beta2) --
       ...
       @generated from src/zungqr_2stage_gpu.cpp normal z -> c, Wed Jan  6 17:59:33 2016
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magmablas_claset( MagmaFull, m-kk, n-kk, c_zero, c_one,  dA(kk,kk), ldda );
    }
    else {
        ki = 0;
        kk = 0;
    ...
    if (kk < n) {
        ...
        i__2 = n - kk;
        i__3 = k - kk;
        //magma_cgetmatrix(i__1, i__2, dA(kk, kk), ldda, panel, i__1);
        //lapackf77_cungqr(&i__1, &i__2, &i__3, panel, &i__1, &tau[kk],
        //                 work, &lwork, &iinfo);
        //
        //magma_csetmatrix(i__1, i__2, panel, i__1, dA(kk, kk), ldda);
        ...
        //magmablas_claset(MagmaFull, kk-nb,     nb, c_zero, c_zero, dA(0,kk-nb),     ldda);
        //magmablas_claset(MagmaFull, m-(kk-nb), nb, c_zero, c_one,  dA(kk-nb,kk-nb), ldda);
    }
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= nb; i -= nb) {
            ...
            i__2 = m - i;
            //magma_cgetmatrix_async( i__2, ib, dA(i,i), ldda, panel, i__2, stream[0] );  // verify
            if (i + ib < n) {
                /* Apply H to A(i:m,i+ib:n) from the left */
                i__3 = n - i;

                magmablas_claset( MagmaFull, i,   ib, c_zero, c_zero, dA(0,i), ldda );
                magmablas_claset( MagmaFull, m-i, ib, c_zero, c_one,  dA(i,i), ldda );
            ...
            //                 work, &lwork, &iinfo);
            //magma_csetmatrix_async( i__2, ib, panel, i__2, dA(i,i), ldda, stream[1] );  // verify

            /* Set rows 1:i-1 of current block to zero */
            i__2 = i + ib;
            //magmablas_claset(MagmaFull, i-ib,     ib, c_zero, c_zero, dA(0,i-ib),    ldda);
            //magmablas_claset(MagmaFull, m-(i-ib), ib, c_zero, c_one,  dA(i-ib,i-ib), ldda);
        }
    }

    magmablas_claset( MagmaFull, m, nb, c_zero, c_one, dA(0,0), ldda );
