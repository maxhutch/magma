/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from zgetrf2_mgpu.cpp normal z -> s, Fri May 30 10:40:56 2014
...
#define W(j) (w+((j)%num_gpus)*nb*ldw)
    ...
    *info = 0;
    ...
    magmablas_stranspose2( d_lAP[0], maxm, dAT(0,0,0), lddat, nb0, m );
    ...
    for( i=0; i < s; i++ ) {
        ...
        for( dd=0; dd < num_gpus; dd++ ) {
            ...
            if ( d == (i+1)%num_gpus) {
                ...
                magmablas_stranspose2(panel_local[d], ldpan[d], &d_lAP[d][(i%h)*nb*maxm], cols, rows, nb);
            ...
            if ( d == (i+1)%num_gpus ) {
                ...
                if ( nb0 > 0 ) {
                    ...
                    magmablas_stranspose2( &d_lAP[d][((i+1)%h)*nb*maxm], ldda, dAT(d,loff,i_local), lddat, nb0, m-(i+1)*nb );
    ...
    if ( nb0 > 0 ) {
        ...
        for( d=0; d < num_gpus; d++ ) {
            ...
            if ( d == id ) {
                ...
                magmablas_stranspose2( panel_local[d], lddat, &d_lAP[d][(s%h)*nb*maxm], cols, rows, nb0);
                ...
                magmablas_stranspose2( panel_local[d], nb, &d_lAP[d][(s%h)*nb*maxm], cols, rows, nb0);
                magma_strsm( MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit,
                             nb1, nb0, c_one,
                             panel_local[d],     nb,
