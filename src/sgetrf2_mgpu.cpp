/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated s Tue Aug 13 16:44:12 2013
...
#define W(j) (w+((j)%num_gpus)*nb*ldw)
    ...
    *info = 0;
    ...
    magmablas_stranspose2( d_lAP[0], maxm, inAT(0,0,0), lddat, nb0, m );
        ...
        for( dd=0; dd<num_gpus; dd++ ) {
            ...
            if( d == (i+1)%num_gpus )
            {
                ...
                if( nb0 > 0 ) {
                    ...
                    magmablas_stranspose2( &d_lAP[d][((i+1)%h)*nb*maxm], ldda, inAT(d,loff,i_local), lddat, nb0, m-(i+1)*nb );
