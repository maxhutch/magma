/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zhetrf.cpp normal z -> d, Fri Sep 11 18:29:29 2015
*/
    ...
    *info = 0;
    ...
    if ( upper ) {
        for (int k = 0; k < n; k += nb ) {
            kb = min(nb, n-k);
            magma_dsetmatrix_async( k+kb, kb, A(0,k), lda, dA(0,k), ldda, stream[0] );
        }
    } else {
        for (int k = 0; k < n; k += nb ) {
            kb = min(nb, n-k);
            magma_dsetmatrix_async( n-k, kb, A(k,k), lda, dA(k,k), ldda, stream[0] );
        }
    }
    trace_gpu_end( 0, 0 );
    ...
    if ( upper ) {
        ...
        for (int k = n-1; k >= 0; k -= kb ) {
            ...
            if ( k+1 > nb ) {
                ...
                magma_dgetmatrix( nk, nk, dA( 0, 0 ),ldda, A( 0, 0 ),lda );
        ...
        for (int k = 0; k < n; k += kb ) {
            ...
            if ( k < n-nb ) {
                ...
                magma_queue_sync( stream[0] );
                magma_dgetmatrix( nk,nk, dA(k,k),ldda, A(k,k),lda );
