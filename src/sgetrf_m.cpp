/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from zgetrf_m.cpp normal z -> s, Fri May 30 10:40:57 2014
...
magma_sgetrf_m(magma_int_t num_gpus0, magma_int_t m, magma_int_t n,
...
#define dPT(d,i,j) (dPT[d] + (i)*nb*nb + (j)*nb*maxm)
    ...
    *info = 0;
    ...
    for( I=0; I < n; I += NB ) {
        ...
        for( offset = 0; offset < min(m,I); offset += NB ) {
            ...
            for( d=0; d < num_gpus; d++ ) {
                ...
                /* transpose */
                magmablas_stranspose2( dPT(d,0,0), nb, dA[d], maxm-offset, M-offset, nbi);
            }
            ...
            for( jj=0, ib=offset/nb; jj < NBk; jj += nb, ib++ ) {
                ...
                for( d=0; d < num_gpus; d++ ) {
                    ...
                    if ( jj+nb < NBk ) {
                        ...
                        magmablas_stranspose2( dPT(d,0,(1+jj/nb)%2), nb, dA[d], rows-nb, M-ii-nb, nb);
                    }
    ...
    if ( *info >= 0 ) magma_sgetrf_piv(m, n, NB, A, lda, ipiv, info);
    return *info;
} /* magma_sgetrf_m */


extern "C" magma_int_t
magma_sgetrf_piv(magma_int_t m, magma_int_t n, magma_int_t NB,
                 float *A, magma_int_t lda, magma_int_t *ipiv, magma_int_t *info)
{
...
extern "C" magma_int_t
magma_sgetrf2_piv(magma_int_t m, magma_int_t n, magma_int_t start, magma_int_t end,
                  float *A, magma_int_t lda, magma_int_t *ipiv, magma_int_t *info)
{
