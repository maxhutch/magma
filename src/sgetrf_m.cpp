...
magma_sgetrf_m(magma_int_t num_gpus0, magma_int_t m, magma_int_t n, float *a, magma_int_t lda,
...
//#define PROFILE
    ...
    *info = 0;
    ...
    for( I=0; I<n; I+=NB ) {
        ...
        for( offset = 0; offset<min(m,I); offset+=NB )
        {
            ...
            for( d=0; d<num_gpus; d++ ) {
                ...
                /* transpose */
                magmablas_stranspose2( inPT(d,0,0), nb, dA[d], maxm-offset, M-offset, nbi);
            }
            ...
            for( jj=0, ib=offset/nb; jj<NBk; jj+=nb, ib++ )
            {
                ...
                for( d=0; d<num_gpus; d++ ) {
                    ...
                    if( jj+nb < NBk ) {
                        ...
                        magmablas_stranspose2( inPT(d,0,(1+jj/nb)%2), nb, dA[d], rows-nb, M-ii-nb, nb);
                    }
    ...
    flops = FLOPS_SGETRF( m, n ) / 1000000;
    printf(" NB=%d nb=%d\n",NB,nb);
