/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from src/zgetrf_m.cpp normal z -> s, Fri Jan 22 21:41:33 2016
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        for( I=0; I < n; I += NB ) {
            ...
            timer_start( time );
            magmablas_ssetmatrix_transpose_mgpu(ngpu, queues, A(0,I), lda,
            ...
            timer_start( time );
            magmablas_sgetmatrix_transpose_mgpu(ngpu, queues, dAT, ldn_local, A(0,I), lda, dA, maxm, M, N, nb);
            for( d=0; d < ngpu; d++ ) {
                magma_setdevice(d);
                magma_queue_sync( queues[d][0] );
                magma_queue_sync( queues[d][1] );
