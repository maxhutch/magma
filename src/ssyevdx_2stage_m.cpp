    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    work[0]  = MAGMA_S_MAKE( lwmin * one_eps, 0.);  // round up
...
*/
    ...
    timer_stop( time );
    timer_printf( "  N= %10d  nb= %5d time ssytrd_sy2sb= %6.2f\n", (int)n, (int)nb, time );
    timer_start( time );
#else
    ...
    if (ver == 30) {
        magma_ssytrd_sy2sb_mgpu_spec(uplo, n, nb, A, lda, TAU1, Wstg1, lwstg1, dA, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    } else {
        magma_ssytrd_sy2sb_mgpu(uplo, n, nb, A, lda, TAU1, Wstg1, lwstg1, dA, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    }
    ...
    timer_stop( time );
    timer_printf( "  N= %10d  nb= %5d time ssytrd_convert = %6.2f\n", (int)n, (int)nb, time );
    timer_start( time );
    ...
    timer_printf( "  N= %10d  nb= %5d time ssytrd_sb2st= %6.2f\n", (int)n, (int)nb, time );
    timer_printf( "  N= %10d  nb= %5d time ssytrd= %6.2f\n", (int)n, (int)nb, time_total );
    ...
    if (! wantz) {
        ...
        timer_stop( time );
        timer_printf( "  N= %10d  nb= %5d time sstedx_m = %6.2f\n", (int)n, (int)nb, time );
        ...
        timer_stop( time );
        timer_printf( "  N= %10d  nb= %5d time sbulge_back_m = %6.2f\n", (int)n, (int)nb, time );
        timer_start( time );
        ...
        if (MAGMA_SUCCESS != magma_smalloc( &dAsgpu, n*lddasgpu )) {
            *info = MAGMA_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_ssetmatrix( n, n, A, lda, dAsgpu, lddasgpu );

        magma_sormqr_gpu_2stages(MagmaLeft, MagmaNoTrans, n-nb, *m, n-nb, dAsgpu+nb, lddasgpu,
        ...
        timer_stop( time );
        timer_printf( "  N= %10d  nb= %5d time sormqr_m + copy = %6.2f\n", (int)n, (int)nb, time );
#ifdef SINGLEGPU
        magma_free(dT1sgpu);
        magma_free(dZ);
        magma_free(dAsgpu);
