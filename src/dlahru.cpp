/*
    -- MAGMA (version 1.5.0-beta1) --
       ...
       @generated from zlahru.cpp normal z -> d, Fri Apr 25 15:05:51 2014
       @author Stan Tomov
       @author Mark Gates
*/
...
/**
    ...
    @param[in,out]
    dA      DOUBLE_PRECISION array on the GPU, dimension
    ...
    @param[in]
    dY      (workspace) DOUBLE_PRECISION array on the GPU, dimension
            (N, NB). On entry the (N-K)-by-NB Y = A V. It is used internally
            as workspace, so its value is changed on exit.
    
    @param[in]
    dV      (workspace) DOUBLE_PRECISION array on the GPU, dimension
