/*
    -- MAGMA (version 1.5.0-beta1) --
       ...
       @generated from zgetrf_mgpu.cpp normal z -> s, Fri Apr 25 15:05:36 2014
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        for (i=0; i < num_gpus; i++) {
            ...
            magmablas_stranspose2( d_lAT[i], lddat, d_lA[i], ldda, m, n_local[i] );
        }
        for (i=0; i < num_gpus; i++) {
            magma_setdevice(i);
        ...
        /* cpu workspace */
        lddwork = maxm;
        if (MAGMA_SUCCESS != magma_smalloc_pinned( &work, lddwork*nb*num_gpus )) {
            for (i=0; i < num_gpus; i++ ) {
                magma_setdevice(i);
        ...
        magma_sgetrf2_mgpu(num_gpus, m, n, nb, 0, d_lAT, lddat, ipiv, d_panel, work, maxm,
                           streaml, info);
        ...
        for( d=0; d < num_gpus; d++ ) {
            ...
            /* save on output */
            magmablas_stranspose2( d_lA[d], ldda, d_lAT[d], lddat, n_local[d], m );
            magma_device_sync();
            magma_free( d_lAT[d]   );
