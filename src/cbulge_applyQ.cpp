 ...
 *     @author Stan Tomov
 *
 *     @generated from src/zbulge_applyQ.cpp normal z -> c, Fri Jan 22 21:41:45 2016
 *
 */
...
extern "C" void magma_cbulge_applyQ(
    magma_int_t WANTZ, magma_side_t SIDE, magma_int_t NE, magma_int_t N, magma_int_t NB,
    magma_int_t Vblksiz, magmaFloatComplex *E, magma_int_t LDE,
    magmaFloatComplex *V, magmaFloatComplex *TAU, magmaFloatComplex *T,
    magma_int_t *INFO, magmaFloatComplex *dV, magmaFloatComplex *dT,
    magmaFloatComplex *dE, magma_int_t copytype )
{
    //%===========================
    //%   local variables
    //%===========================
    ...
    *INFO=0;
    ...
    if (WANTZ == 1) {
        ...
        magmablas_claset( MagmaFull, N, N, c_zero, c_one, dE, N );
    }
            ...
            for (m = rownbm; m > 0; m--) {
                ...
                for (n = nbgr; n > 0; n--) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        ...
                        magma_clarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, vlen, N1, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, N1);
                        magmablasSetKernelStream(stream[1]);
                        magma_clarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, vlen, N2, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,N1), LDE, &dwork[N1*Vblksiz], N2);
                        #else
                        magma_clarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, vlen, NE, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, NE);
                        #endif
                    }
