...
magma_clarft_sm32x32_batched(magma_int_t n, magma_int_t k, magmaFloatComplex **v_array, magma_int_t ldv,
                    magmaFloatComplex **tau_array, magmaFloatComplex **T_array, magma_int_t ldt, magma_int_t batchCount, cublasHandle_t myhandle, magma_queue_t queue)
{
...
#define use_gemm_larft_sm32
    ...
    //magmablas_cgemm_batched( MagmaConjTrans, MagmaNoTrans, k, k, n, MAGMA_C_ONE, v_array, ldv, v_array, ldv, MAGMA_C_ZERO, T_array, ldt, batchCount, queue);
    cublasCgemmBatched(myhandle, CUBLAS_OP_C, CUBLAS_OP_N, k, k, n,
                             &one, (const magmaFloatComplex**) v_array, ldv,
                                    (const magmaFloatComplex**) v_array, ldv,
                             &zero,  T_array, ldt, batchCount);

    magmablas_claset_batched(MagmaLower, k, k, MAGMA_C_ZERO, MAGMA_C_ZERO, T_array, ldt, batchCount, queue);
#else
    #if 1
    for(int i=0; i<k; i++)
    {
...
#ifdef RFT_MAG_GEM
    magmablas_cgemm_batched( MagmaConjTrans, MagmaNoTrans, 
            k, k, n, 
            one,  v_array, ldv, 
                  v_array, ldv, 
    ...
    for(j=0; j<k; j+=nb)
    {
        ...
        if(prev_n>0 && mycol>0){
            ...
            cublasCgemmBatched(myhandle, CUBLAS_OP_N, CUBLAS_OP_N, 
                    prev_n, mycol, prev_n,
                    &one, (const magmaFloatComplex**) T_array, ldt,
                          (const magmaFloatComplex**) dW1_displ, ldtstep,
