/*
    -- MAGMA (version 1.5.0-beta3) --
       ...
       @generated from zhetrd_mgpu.cpp normal z -> d, Fri Jul 18 17:34:18 2014
...
//#define PROFILE_SY2RK
    ...
    for( did=0; did < num_gpus; did++ ) {
        ...
        if ( MAGMA_SUCCESS != magma_dmalloc(&dA[did],     ln*ldda+3*lddwork*nb) ||
             MAGMA_SUCCESS != magma_dmalloc(&dx[did],     num_streams*n) ||
             MAGMA_SUCCESS != magma_dmalloc(&dy[did],     num_streams*n) ||
    ...
    if ( MAGMA_SUCCESS != magma_dmalloc_pinned( &hwork, num_streams*num_gpus*n ) ) {
    ...
    if (upper) {
        ...
        for (i = nb*((n-1)/nb); i >= nx; i -= nb) {
            ...
            magma_dsyr2k_mgpu(num_gpus, MagmaUpper, MagmaNoTrans, nb, i, ib,
...
#endif
            magma_dsyr2k_mgpu(num_gpus, MagmaLower, MagmaNoTrans, nb, n-i-ib, ib,
...
extern "C" magma_int_t
magma_dhtodhe(magma_int_t num_gpus, magma_uplo_t uplo, magma_int_t n, magma_int_t nb,
              double *A, magma_int_t lda,
