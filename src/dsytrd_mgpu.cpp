/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated d Tue Aug 13 16:44:36 2013
...
magma_dsytrd_mgpu(magma_int_t num_gpus, magma_int_t k, char uplo, magma_int_t n,
...
/*  -- MAGMA (version 1.4.0) --
    ...
    magma_int_t ln, ldda;
    magma_int_t nb = magma_get_dsytrd_nb(n), ib;
    ...
    double *dx[MagmaMaxGPUs], *dy[MagmaMaxGPUs], *hwork;
...
//#define PROFILE_SY2RK
    ...
    for( did=0; did<num_gpus; did++ ) {
        magma_setdevice(did);
        if ( MAGMA_SUCCESS != magma_dmalloc(&da[did],     ln*ldda+3*lddwork*nb) ||
             MAGMA_SUCCESS != magma_dmalloc(&dx[did],     k*n     ) ||
             MAGMA_SUCCESS != magma_dmalloc(&dy[did],     k*n     ) ||
             MAGMA_SUCCESS != magma_dmalloc(&dwork2[did], ldwork2 ) ) {
            for( i=0; i<did; i++ ) {
                magma_setdevice(i);
                magma_free(da[i]);
                magma_free(dx[i]);
    ...
    magma_setdevice(0);
    if( MAGMA_SUCCESS != magma_dmalloc_pinned( &hwork, k*num_gpus*n ) ) {
    ...
    if (upper) {
        ...
        if (1<=n-nx) {
            magma_dhtodhe(num_gpus, &uplo, n, nb, a, lda, da, ldda, stream, &iinfo );
        }
        ...
        for (i = nb*((n-1)/nb); i >= nx; i -= nb) {
            ...
            magma_dlatrd_mgpu(num_gpus, uplo, n, i+ib, ib, nb,
                              A(0, 0), lda, e, tau,
                              work, ldwork,
                              da, ldda, 0,
                              dwork, i+ib,
            ...
            magma_dsyr2k_mgpu(num_gpus, 'U', 'N', nb, i, ib,
            ...
            if (i-nb >= nx ) {
                ...
                magma_dgetmatrix_async( (i-nb)+ib, ib,
                                        dA(did, 0, ii), ldda,
                                         A(0, i-nb),    lda,
                                        stream[did][0] );
            }
            ...
            /* Copy superdiagonal elements back into A, and diagonal
               elements into D */
            for (j = i; j < i+ib; ++j) {
                if( j > 0 ) { MAGMA_D_SET2REAL( *A(j-1, j), e[j - 1] ); }
        ...
        if( nx > 0 ) {
            ...
            lapackf77_dsytd2(uplo_, &nx, A(0, 0), &lda, d, e, tau, &iinfo);
        }
    }
    else {
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            mv_time +=
            magma_dlatrd_mgpu(num_gpus, uplo, n, n-i, ib, nb,
                              A(i, i), lda, &e[i],
                              &tau[i], work, ldwork,
                              da, ldda, i,
                              dwork,  (n-i),
...
#endif
            magma_dsyr2k_mgpu(num_gpus, 'L', 'N', nb, n-i-ib, ib,
            ...
            /* Copy subdiagonal elements back into A, and diagonal
               elements into D */
            for (j = i; j < i+ib; ++j) {
                if( j+1 < n ) { MAGMA_D_SET2REAL( *A(j+1, j), e[j] ); }
                d[j] = MAGMA_D_REAL( *A(j, j) );
            }
        } /* for i=... */
...
extern "C" magma_int_t
magma_dhtodhe(magma_int_t num_gpus, char *uplo, magma_int_t n, magma_int_t nb,
              double *a, magma_int_t lda,
...
magma_dsyr2k_mgpu(
    ...
    double alpha,
    double **db, magma_int_t lddb, magma_int_t offset_b,
    double beta,
...
#define dC(id, i, j)  (dc[(id)]+(j)*lddc + (i))
    ...
    if( lapackf77_lsame( uplo_, "U" ) ) {
        for( i=nb; i<n; i+=nb ) {
            ...
            ib = min(nb, n-i);
            ii = nb*((i+offset)/(nb*num_gpus));
            magma_dgemm(MagmaNoTrans, MagmaTrans, i, ib, k,
        ...
        for( i=0; i<n-nb; i+=nb ) {
            ...
            magma_dgemm(MagmaNoTrans, MagmaTrans, n1, ib, k,
...
magma_dsytrd_mgpu(magma_int_t num_gpus, magma_int_t k, char uplo, magma_int_t n,
                  ...
                  magma_int_t *info)
{
    printf("magma_dsytrd_mgpu is not supported on this GPU. Exit.\n");
    exit(1);
}
