/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated d Tue Aug 13 16:44:24 2013
...
magma_dlaqps_gpu(magma_int_t m, magma_int_t n, magma_int_t offset,
...
#define  F(i, j) (F  + (i) + (j)*(ldf ))
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
            ...
            magmablas_dswap( i__1, F(pvt, 0), ldf, F(k, 0), ldf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
            //vn1[pvt] = vn1[k];
...
//#define RIGHT_UPDATE
        ...
        magma_dlarfg_gpu(m-rk, A(rk, k), A(rk + 1, k), &tau[k], &vn1[k], &Aks[k]);

        //Akk = *A(rk, k);
        //*A(rk, k) = c_one;
        //magma_dgetvector( 1, &Aks[k],  1, &Akk,     1 );

        /* needed to avoid the race condition */
        if (k == 0) magma_dsetvector(  1,    &c_one,       1, A(rk, k), 1 );
        else        magma_dcopymatrix( 1, 1, A(offset, 0), 1, A(rk, k), 1 );
        ...
        if (k < n-1) {
            ...
            magma_dgemv( MagmaTrans, m-rk, n-k-1,
                         tauk,   A( rk,  k+1 ), lda,
                                 A( rk,  k   ), 1,
                         c_zero, F( k+1, k   ), 1 );
            ...
            //magma_dgemv( MagmaTrans, i__1 - i__5, i__2 - i__3,
            ...
            //blasf77_dgemv( MagmaTransStr, &i__1, &i__3,
            //               &tau[k], A(rk,  k+1), &lda,
            //                        A(rk,  k  ), &ione,
            //               &c_zero, F(k+1, k  ), &ione );
            
            //magma_queue_sync( stream );
            //blasf77_dgemv( MagmaTransStr, &i__5, &i__4,
        ...
        if (k > 0) {
            ...
            i__1 = m - offset - nb;
            i__2 = k;
            magma_dgemv( MagmaTrans, i__1, i__2,
            ...
            i__1 = m - rk;
            i__2 = k;
            //blasf77_dgemv( MagmaTransStr, &i__1, &i__2,
            //               &z__1,   A(rk, 0), &lda,
            //                        A(rk, k), &ione,
            //               &c_zero, auxv, &ione );
        ...
        if (k < n-1) {
            i__1 = n - k - 1;
            i__2 = k + 1;
            //blasf77_dgemm( MagmaNoTransStr, MagmaTransStr, &ione, &i__1, &i__2,
            //               &c_neg_one, A(rk, 0  ), &lda,
            //                           F(k+1,0  ), &ldf,
            //               &c_one,     A(rk, k+1), &lda );
#ifdef RIGHT_UPDATE
            /* right-looking update of rows,                     */
            magma_dgemm( MagmaNoTrans, MagmaTrans, nb-k, i__1, ione,
        ...
        if (rk < min(m, n+offset)-1 ){
            magmablas_dnrm2_row_check_adjust(n-k-1, tol3z, &vn1[k+1], &vn2[k+1], A(rk,k+1), lda, lsticcs);
        ...
        //magma_dswap( 1, &Aks[k], 1, A(rk, k), 1 );
        
        ++k;
    }
    magma_dcopymatrix( 1, k, Aks, 1, A(offset, 0), lda+1 );
    ...
    *kb = k + 1;
    ...
    /*while( lsticc > 0 ) {
        ...
        i__1 = m - rk - 1;
        if (lsticc <= nb)
            vn1[lsticc] = cblas_dnrm2(i__1, A(rk + 1, lsticc), ione);
        else {
            // Where is the data, CPU or GPU ?
            double r1, r2;
            
            r1 = cblas_dnrm2(nb-k, A(rk + 1, lsticc), ione);
