/*
    -- MAGMA (version 2.0.2) --
       ...
       @generated from src/zlarft_batched.cpp normal z -> s, Mon May  2 23:30:28 2016
*/
    ...
    for (j=0; j < k; j += nb)
    {
        ...
        if (prev_n > 0 && mycol > 0) {
            ...
            for (i=0; i < prev_n; i += nb)
            {
                ...
                if (DEBUG == 3) {
                    printf("        doing recstrmv on the rectangular portion of size %d %d of T(%d,%d)\n",
        ...
        if (stair_T == 0) {
            if (DEBUG == 3) {
                printf("doing strmv on the triangular portion of size %d %d of T(%d,%d)\n",
