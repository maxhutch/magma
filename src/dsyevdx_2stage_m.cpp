/*
    -- MAGMA (version 2.0.0-beta2) --
       ...
       @generated from src/zheevdx_2stage_m.cpp normal z -> d, Wed Jan  6 17:59:34 2016
...
*/
    ...
    timer_stop( time );
    timer_printf( "  N= %10d  nb= %5d time dsytrd_sy2sb= %6.2f\n", (int)n, (int)nb, time );
    timer_start( time );
#else
    ...
    magma_dsetmatrix_1D_col_bcyclic( n, n, A, lda, dA, ldda, ngpu, distblk );
    magma_setdevice(0);
    timer_stop( time_dist );

    timer_start( time_band );
    if (ver == 30) {
        magma_dsytrd_sy2sb_mgpu_spec(uplo, n, nb, A, lda, TAU1, Wstg1, lwstg1, dA, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    } else {
        magma_dsytrd_sy2sb_mgpu(uplo, n, nb, A, lda, TAU1, Wstg1, lwstg1, dA, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    }
    ...
    timer_stop( time );
    timer_printf( "  N= %10d  nb= %5d time dsytrd_convert = %6.2f\n", (int)n, (int)nb, time );
    timer_start( time );
    ...
    timer_printf( "  N= %10d  nb= %5d time dsytrd_sb2st= %6.2f\n", (int)n, (int)nb, time );
    timer_printf( "  N= %10d  nb= %5d time dsytrd= %6.2f\n", (int)n, (int)nb, time_total );
    ...
    if (! wantz) {
        ...
        timer_stop( time );
        timer_printf( "  N= %10d  nb= %5d time dstedx_m = %6.2f\n", (int)n, (int)nb, time );
        ...
        timer_stop( time );
        timer_printf( "  N= %10d  nb= %5d time dbulge_back_m = %6.2f\n", (int)n, (int)nb, time );
        timer_start( time );
        ...
        if (MAGMA_SUCCESS != magma_dmalloc( &dAsgpu, n*lddasgpu )) {
            *info = MAGMA_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_dsetmatrix( n, n, A, lda, dAsgpu, lddasgpu );

        magma_dormqr_gpu_2stages(MagmaLeft, MagmaNoTrans, n-nb, *m, n-nb, dAsgpu+nb, lddasgpu,
        ...
        timer_stop( time );
        timer_printf( "  N= %10d  nb= %5d time dormqr_m + copy = %6.2f\n", (int)n, (int)nb, time );
#ifdef SINGLEGPU
        magma_free(dT1sgpu);
        magma_free(dZ);
        magma_free(dAsgpu);
