...
#define dA(i, j)  (dA + (i) + (j)*ldda)   // A(i, j) means at i row, j column
    ...
    magma_malloc((void**)&dW4_displ, batchCount * sizeof(*dW4_displ));  // used in dlarfb
    ...
    magma_dmalloc(&dR,  ldr * n   * batchCount);
    magma_dmalloc(&dT,  ldt * ldt * batchCount);
    ...
    magmablas_dlaset_q(MagmaFull, ldr, n*batchCount  , MAGMA_D_ZERO, MAGMA_D_ZERO, dR, ldr, queue);
    magmablas_dlaset_q(MagmaFull, ldt, ldt*batchCount, MAGMA_D_ZERO, MAGMA_D_ZERO, dT, ldt, queue);
    ...
    magma_getvector( batchCount, sizeof(double*), dT_array, 1, cpuTarray, 1);
    ...
    for(i=0; i<min_mn;i+=nb)
    {
            ...
            magma_ddisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue);
           ...
           dgeqrf_copy_upper_batched(ib, jb, dW0_displ, ldda, dR_array, ldr, batchCount, queue);
            ...
            magmablas_dlacpy_batched(MagmaUpper, ib, ib, dR_array, ldr, dW0_displ, ldda, batchCount, queue);
