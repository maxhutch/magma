...
/**
    ...
    @param[in,out]
    d_lAT   COMPLEX_16 array of pointers on the GPU, dimension (num_gpus).
    ...
    d_lAP   COMPLEX_16 array of pointers on the GPU, dimension (num_gpus).
    ...
    @param (workspace)
    w       COMPLEX_16 array, dimension (num_gpus*nb*maxm).
    ...
    *info = 0;
    ...
    for( i=0; i < s; i++ ) {
        ...
        for( dd=0; dd < num_gpus; dd++ ) {
            ...
            if ( dd == 0 )
                magmablas_zpermute_long2( lddat, dAT(d,0,0), lddat, ipiv, nb, i*nb );
            else
                magmablas_zpermute_long3(        dAT(d,0,0), lddat, ipiv, nb, i*nb );
            trace_gpu_end( d, 1 );
            d = (d+1)%num_gpus;
        }
        ...
        for( dd=0; dd < num_gpus; dd++ ) {
            ...
            if ( d == (i+1)%num_gpus) {
                ...
                magmablas_ztranspose( rows, nb, &d_lAP[d][(i%h)*nb*maxm], cols, panel_local[d], ldpan[d] );
            ...
            if ( d == (i+1)%num_gpus ) {
                ...
                if ( nb0 > 0 ) {
                    ...
                    magmablas_ztranspose( nb0, m-(i+1)*nb, dAT(d,loff,i_local), lddat, &d_lAP[d][((i+1)%h)*nb*maxm], ldda );
