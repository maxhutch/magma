...
#define W(j) (w+((j)%num_gpus)*nb*ldw)
    ...
    *info = 0;
    ...
    magmablas_ztranspose2( d_lAP[0], maxm, dAT(0,0,0), lddat, nb0, m );
        ...
        for( dd=0; dd < num_gpus; dd++ ) {
            ...
            if ( d == (i+1)%num_gpus ) {
                ...
                if ( nb0 > 0 ) {
                    ...
                    magmablas_ztranspose2( &d_lAP[d][((i+1)%h)*nb*maxm], ldda, dAT(d,loff,i_local), lddat, nb0, m-(i+1)*nb );
    ...
    if ( nb0 > 0 ) {
        ...
        for( d=0; d < num_gpus; d++ ) {
            ...
            if ( d == id ) {
                ...
                magmablas_ztranspose2( panel_local[d], lddat, &d_lAP[d][(s%h)*nb*maxm], cols, rows, nb0);
                ...
                magmablas_ztranspose2( panel_local[d], nb, &d_lAP[d][(s%h)*nb*maxm], cols, rows, nb0);
                magma_ztrsm( MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit,
                             nb1, nb0, c_one,
                             panel_local[d],     nb,
