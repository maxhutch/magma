/*
    -- MAGMA (version 1.7.0) --
       ...
       @generated from zgetri_gpu.cpp normal z -> s, Fri Sep 11 18:29:26 2015
    ...
    *info = 0;
    ...
    for( j = jmax; j >= 0; j -= nb ) {
        ...
        magmablas_slacpy( MagmaFull, n-j, jb,
                          dA(j,j), ldda,
                          dL(j,0), lddl );
        magmablas_slaset( MagmaLower, n-j-1, jb, c_zero, c_zero, dA(j+1,j), ldda );
    ...
    for( j = n-2; j >= 0; --j ) {
        jp = ipiv[j] - 1;
        if ( jp != j ) {
            magmablas_sswap( n, dA(0,j), 1, dA(0,jp), 1 );
        }
    }
