...
/*  -- MAGMA (version 1.4.1) --
    ...
    hU      (input,output) REAL array, dimension(LDHU, N), on cpu.
    ...
    dU      (input,output) REAL array, dimension(LDDU, N), on gpu.
    ...
    hA      (input,output) REAL array, dimension(LDHA, N), on cpu.
    ...
    dA      (input,output) REAL array, dimension(LDDA, N) , on gpu.
    ...
    hL      (output) REAL array, dimension(LDHL, K), on vpu.
    ...
    dL      (output) REAL array, dimension(LDDL, K), on gpu.
            On exit, contains in the upper part the IB-by-K lower triangular tile,
            and in the lower part IB-by-K the inverse of the top part.
    ...
    hWORK   (output) REAL array, dimension(LDHWORK, 2*IB), on cpu.
            Workspace.

    LDHWORK (input) INTEGER
            The leading dimension of the array hWORK.  LDHWORK >= max(NB, 1).

    dWORK   (output) REAL array, dimension(LDDWORK, 2*IB), on gpu.
            Workspace.
