/*
   ...
   @generated from zgetrf_batched.cpp normal z -> c, Sat Nov 15 19:54:10 2014
*/
...
//#define ENABLE_TIMER3
    ...
    magmaFloatComplex* dinvA;
    magmaFloatComplex* dwork;// dinvA and dwork are workspace in ctrsm
    ...
    cset_pointer(dwork_array, dwork, 1, 0, 0, dwork_msize, batchCount);
    cset_pointer(dinvA_array, dinvA, TRI_NB, 0, 0, invA_msize, batchCount);
    cudaMemset( dinvA, 0, batchCount * ((n+TRI_NB-1)/TRI_NB)*TRI_NB*TRI_NB * sizeof(magmaFloatComplex) );
    ...
    set_ipointer(pivinfo_array, pivinfo, 1, 0, 0, m, batchCount);
    ...
    for(i = 0; i < min_mn; i+=nb) 
    {
        ...
        ib = min(nb, min_mn-i);
        pm = m-i;
        magma_idisplace_pointers(dipiv_displ, ipiv_array, ldda, i, 0, batchCount);
        magma_cdisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount);
        //===============================================
        //  panel factorization
        //===============================================
#if 0
        arginfo = magma_cgetf2_batched(
                pm, ib,
                dA_displ, ldda,
...
#else
        arginfo = magma_cgetrf_recpanel_batched_q(
                pm, ib, 16,
                dA_displ, ldda,
...
#define RUN_ALL
        ...
        setup_pivinfo_batched_q(pivinfo_array, dipiv_displ, pm, ib, magma_stream, batchCount);
        adjust_ipiv_batched_q(dipiv_displ, ib, i, magma_stream, batchCount);
        ...
        magma_cdisplace_pointers(dW0_displ, dA_array, ldda, i, 0, batchCount);
        magma_claswp_rowparallel_batched( i, dA_displ, ldda,
                dW0_displ, ldda,
                i, i+ib,
                pivinfo_array, batchCount);
        ...
        if( (i + ib) < n)
        {
            ...
            cset_pointer(dwork_array, dwork, nb, 0, 0, dwork_msize, batchCount); // I don't think it is needed Azzam
            magma_claswp_rowparallel_batched( n-(i+ib), dA_displ, ldda,
                    dwork_array, nb,
                    i, i+ib,
                    pivinfo_array, batchCount);
            ...
            magma_cdisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount);
            magma_cdisplace_pointers(dW0_displ, dA_array, ldda, i, i+ib, batchCount);
            ...
            {    
                ...
                {
                    magma_cdisplace_pointers(dA_displ, dA_array,  ldda, i+ib,    i, batchCount);
                    magma_cdisplace_pointers(dW1_displ, dA_array, ldda,    i, i+ib, batchCount);
                    magma_cdisplace_pointers(dW2_displ, dA_array, ldda, i+ib, i+ib, batchCount);
                    //printf("caling batched dgemm %d %d %d \n", m-i-ib, n-i-ib, ib);
                    magmablas_cgemm_batched( MagmaNoTrans, MagmaNoTrans, m-i-ib, n-i-ib, ib, 
