/*
    -- MAGMA (version 1.5.0-beta2) --
       ...
       @generated from zhetrd_mgpu.cpp normal z -> s, Fri May 30 10:41:06 2014
...
#define dC(id, i, j)  (dc[(id)]+(j)*lddc + (i))
    ...
    if (uplo == MagmaUpper) {
        for( i=nb; i < n; i += nb ) {
            ...
            ib = min(nb, n-i);
            ii = nb*((i+offset)/(nb*num_gpus));
            magma_sgemm(MagmaNoTrans, MagmaTrans, i, ib, k,
        ...
        for( i=0; i < n-nb; i += nb ) {
            ...
            magma_sgemm(MagmaNoTrans, MagmaTrans, n1, ib, k,
