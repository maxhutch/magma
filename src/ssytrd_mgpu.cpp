/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zhetrd_mgpu.cpp normal z -> s, Sat Nov 15 19:54:10 2014
...
//#define PROFILE_SY2RK
    ...
    for( did=0; did < ngpu; did++ ) {
        ...
        if ( MAGMA_SUCCESS != magma_smalloc(&dA[did],     ln*ldda+3*lddwork*nb) ||
             MAGMA_SUCCESS != magma_smalloc(&dx[did],     nqueue*n) ||
             MAGMA_SUCCESS != magma_smalloc(&dy[did],     nqueue*n) ||
    ...
    if ( MAGMA_SUCCESS != magma_smalloc_pinned( &hwork, nqueue*ngpu*n ) ) {
        for( i=0; i < ngpu; i++ ) {
            magma_setdevice(i);
            magma_free(dA[i]);
            magma_free(dx[i]);
    ...
    if (upper) {
        ...
        if (1 <= n-nx) {
            magma_shtodhe(ngpu, uplo, n, nb, A, lda, dA, ldda, stream, &iinfo );
        }
        ...
        for (i = nb*((n-1)/nb); i >= nx; i -= nb) {
            ...
            magma_slatrd_mgpu(ngpu, uplo, n, i+ib, ib, nb,
                              A(0, 0), lda, e, tau,
                              work, ldwork,
                              dA, ldda, 0,
                              dwork, i+ib,
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            magma_slatrd_mgpu(ngpu, uplo, n, n-i, ib, nb,
                              A(i, i), lda, &e[i],
                              &tau[i], work, ldwork,
                              dA, ldda, i,
                              dwork,  (n-i),
