/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zpotrf_batched.cpp normal z -> c, Sat Nov 15 19:54:09 2014
*/
...
#define A(i_, j_)  (A + (i_) + (j_)*ldda)   
    ...
    magmaFloatComplex* dx;// dinvA and x are workspace in ctrsm
    magma_int_t invA_msize = ((n+TRI_NB-1)/TRI_NB)*TRI_NB*TRI_NB;
    magma_int_t x_msize = n*nb;
    magma_cmalloc( &dinvA, invA_msize * batchCount);
    magma_cmalloc( &dx,    x_msize * batchCount );
    cset_pointer(dx_array, dx, 1, 0, 0, x_msize, batchCount);
        ...
        for(j = 0; j < n; j+=nb) {
            ...
            if( (n-j-ib) > 0){
                ...
                else
                {
                    ...
                    //-------------------------------------------
                    magma_cdisplace_pointers(dA_displ, dA_array, ldda, j+ib, j, batchCount);
                    magma_cdisplace_pointers(dW1_displ, dA_array, ldda, j+ib, j+ib, batchCount);
