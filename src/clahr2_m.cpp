/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from src/zlahr2_m.cpp normal z -> c, Fri Jan 22 21:41:52 2016
       @author Mark Gates
*/
    ...
    struct cgehrd_data *data )
{
    ...
    for( d = 0; d < ngpu; ++d ) {
        magma_setdevice( d );
        magmablas_claset( MagmaFull, nb, nb, c_zero, c_zero, dV(d,k,0), ldv, data->queues[d] );
    }
    ...
    *A(k+nb,nb-1) = ei;
    ...
    for( d = 0; d < ngpu; ++d ) {
        magma_setdevice( d );
        magma_csetmatrix_async( n, nb, Y, ldy, dY(d, 0, 0), ldda, data->queues[d] );
        magma_csetmatrix_async( nb, nb, T, nb, dTi(d),      nb,   data->queues[d] );
    }
