...
magma_dlarfb_gpu_gemm( char side, char trans, char direct, char storev,
...
/*  -- MAGMA (version 1.4.1) --
    ...
    DV      (input) DOUBLE_PRECISION array on the GPU, dimension
                (LDV,K) if STOREV = 'C'
    ...
    DT      (input) DOUBLE_PRECISION array on the GPU, dimension (LDT,K)
    ...
    DC      (input/output) DOUBLE_PRECISION array on the GPU, dimension (LDC,N)
    ...
    WORKVT  (workspace) DOUBLE_PRECISION array, dimension (LDWORKT,K)
