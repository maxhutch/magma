/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated d Wed Aug 14 12:16:13 2013
...
magma_dsytrd2_gpu(char uplo, magma_int_t n,
...
/*  -- MAGMA (version 1.4.0) --
    ...
    DA      (device input/output) DOUBLE_PRECISION array, dimension (LDA,N)
    ...
    WA      (workspace/output) DOUBLE_PRECISION array, dimension (LDA,N)
    ...
    DWORK   (workspace/output) DOUBLE_PRECISION array on the GPU, dim (MAX(1,LDWORK))

    LDWORK  (input) INTEGER
            The dimension of the array DWORK.  
            LDWORK >= (n*n+64-1)/64 + 2*n*nb, where nb = magma_get_dsytrd_nb(n)
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb)
        {
            ...
            magma_dlatrd2(uplo, i+nb, nb, A(0, 0), ldwa, e, tau,
    ...
    else
    {
        ...
        for (i = 0; i < n-nx; i += nb)
        {
            ...
            magma_dlatrd2(uplo, n-i, nb, A(i, i), ldwa, &e[i],
                          &tau[i], work, ldw,
                          dA(i, i), ldda,
                          dwork, lddw,
