...
magma_dpotrf2_mgpu(int num_gpus, char uplo, magma_int_t m, magma_int_t n,
    ...
    *info = 0;
    ...
    if (upper) {
...
#if defined(PRECISION_d) && defined(DTRSM_WORK)
        ...
        for( d=0; d<num_gpus; d++ ) {
            magma_setdevice(d);
            for( j=0; j<2; j++ ) {
                magma_dmalloc( &d_dinvA[d][j], nb*nb );
                magma_dmalloc( &d_x[d][j],      n*nb );
