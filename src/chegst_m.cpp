/*
    -- MAGMA (version 1.5.0-beta3) --
       ...
       @generated from zhegst_m.cpp normal z -> c, Fri Jul 18 17:34:19 2014
*/
#include "common_magma.h"
#include <cblas.h>


static void magma_chegst_m_1_L_col_update(magma_int_t nk, magma_int_t nb, magmaFloatComplex* dA_col, magma_int_t ldda,
                                          magmaFloatComplex* dC1, magma_int_t lddc1, magmaFloatComplex* dC2, magma_int_t lddc2);

static void magma_chegst_m_1_U_row_update(magma_int_t nk, magma_int_t nb, magmaFloatComplex* dA_row, magma_int_t ldda,
                                          magmaFloatComplex* dC1, magma_int_t lddc1, magmaFloatComplex* dC2, magma_int_t lddc2);
...
magma_chegst_m(magma_int_t nrgpu, magma_int_t itype, magma_uplo_t uplo, magma_int_t n,
        ...
        if (upper) {
            ...
            for (magma_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    ...
                    magma_cgemm(MagmaConjTrans, MagmaNoTrans, kb, nk-kb, nb, c_neg_one, dwork(igpu_p, 0, k), nb,
                                dB_r(igpu_p, ind_k1, k+1), lddbr, c_one, dA(igpu_p, k/nrgpu, k+1), ldda );

                    magma_cgemm(MagmaConjTrans, MagmaNoTrans, kb, nk-kb, nb, c_neg_one, dB_r(igpu_p, ind_k1, k), lddbr,
                    ...
                    for (magma_int_t j=k+1; j < nbl; ++j) {
                        ...
                        if (igpu != igpu_p) {
                            ...
                            magma_chegst_m_1_U_row_update(n-j*nb, nb, dA(igpu, j/nrgpu, j), ldda,
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j=k+nrgpu; j < nbl; j += nrgpu) {
                        ...
                        magma_chegst_m_1_U_row_update(n-j*nb, nb, dA(igpu_p, j/nrgpu, j), ldda,
            ...
            for (magma_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    ...
                    magma_cgemm(MagmaNoTrans, MagmaConjTrans, nk-kb, kb, nb, c_neg_one, dwork(igpu_p, k+1, 0), n,
                                dB_c(igpu_p, k, ind_k1), lddbc, c_one, dA(igpu_p, k+1, k/nrgpu), ldda );

                    magma_cgemm(MagmaNoTrans, MagmaConjTrans, nk-kb, kb, nb, c_neg_one, dB_c(igpu_p, k+1, ind_k1), lddbc,
                    ...
                    for (magma_int_t j=k+1; j < nbl; ++j) {
                        ...
                        if (igpu != igpu_p) {
                            ...
                            magma_chegst_m_1_L_col_update(n-j*nb, nb, dA(igpu, j, j/nrgpu), ldda,
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j=k+nrgpu; j < nbl; j += nrgpu) {
                        ...
                        magma_chegst_m_1_L_col_update(n-j*nb, nb, dA(igpu_p, j, j/nrgpu), ldda,
            ...
            if (n > nb) {
                ...
                for (magma_int_t k = 0; k < nbl; ++k) {
                    ...
                    for (magma_int_t igpu = 0; igpu < nrgpu; ++igpu) {
                        ...
                        magma_cgemm(MagmaNoTrans, MagmaNoTrans, k*nb, nloc[igpu], kb, c_one, dB_c(igpu, 0, k%2), lddbc,
            ...
            for (magma_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j = 0; j < k; ++j) {
                        ...
                        magma_cgemm(MagmaNoTrans, MagmaConjTrans, j*nb, nb, kb, c_one, dB_c(igpu, 0, ind_k), lddbc,
                                    dwork(igpu, j, 0), n, c_one, dA(igpu, 0, j/nrgpu), ldda );

                        magma_cgemm(MagmaNoTrans, MagmaConjTrans, j*nb, nb, kb, c_one, dwork(igpu, 0, 0), n,
                ...
                for (magma_int_t igpu = 0; igpu < nrgpu; ++igpu) {
                    ...
                    magma_queue_sync(stream[igpu][0]); // sync B copy (conflicts with chegst)
                }
            ...
            if (n > nb) {
                ...
                for (magma_int_t k = 0; k < nbl; ++k) {
                    ...
                    for (magma_int_t igpu = 0; igpu < nrgpu; ++igpu) {
                        ...
                        magma_cgemm(MagmaNoTrans, MagmaNoTrans, nloc[igpu], k*nb, kb, c_one, dA(igpu, iloc[igpu], k), ldda,
            ...
            for (magma_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j = 0; j < k; ++j) {
                        ...
                        magma_cgemm(MagmaConjTrans, MagmaNoTrans, nb, j*nb, kb, c_one, dwork(igpu, 0, j), nb,
                                    dB_r(igpu, ind_k, 0), lddbr, c_one, dA(igpu, j/nrgpu, 0), ldda );

                        magma_cgemm(MagmaConjTrans, MagmaNoTrans, nb, j*nb, kb, c_one, dB_r(igpu, ind_k, j), lddbr,
...
inline static void magma_chegst_m_1_U_row_update(magma_int_t nk, magma_int_t nb, magmaFloatComplex* dA_row, magma_int_t ldda,
                                                 magmaFloatComplex* dC1, magma_int_t lddc1, magmaFloatComplex* dC2, magma_int_t lddc2)
{
    ...
    magma_cgemm(MagmaConjTrans, MagmaNoTrans, kb, nk-kb, nb, c_neg_one, dC1, lddc1,
                dC2+kb*lddc2, lddc2, c_one, dA_row+kb*ldda, ldda );

    magma_cgemm(MagmaConjTrans, MagmaNoTrans, kb, nk-kb, nb, c_neg_one, dC2, lddc2,
                dC1+kb*lddc1, lddc1, c_one, dA_row+kb*ldda, ldda );
    return;
}

inline static void magma_chegst_m_1_L_col_update(magma_int_t nk, magma_int_t nb, magmaFloatComplex* dA_col, magma_int_t ldda,
                                                 magmaFloatComplex* dC1, magma_int_t lddc1, magmaFloatComplex* dC2, magma_int_t lddc2)
{
    ...
    magma_cgemm(MagmaNoTrans, MagmaConjTrans, nk-kb, kb, nb, c_neg_one, dC1+kb, lddc1,
                dC2, lddc2, c_one, dA_col+kb, ldda );

    magma_cgemm(MagmaNoTrans, MagmaConjTrans, nk-kb, kb, nb, c_neg_one, dC2+kb, lddc2,
