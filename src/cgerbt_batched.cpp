/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zgerbt_batched.cpp normal z -> c, Sat Nov 15 19:54:09 2014
       @author Adrien REMY
*/
    ...
    magma_int_t *info, magma_int_t batchCount)
{
    ...
    *info = 0;
    ...
    magma_csetvector( 2*n, U, 1, du, 1);
    magma_csetvector( 2*n, V, 1, dv, 1);

    /* Perform Partial Random Butterfly Transformation on the GPU*/
    magmablas_cprbt_batched(n, dA_array, ldda, du, dv, batchCount);

    /* Compute U^T.b on the GPU*/

    // TODO fix for multiple RHS
    for(int i= 0; i < nrhs; i++)
        magmablas_cprbt_mtv_batched(n, du, dB_array, batchCount);
