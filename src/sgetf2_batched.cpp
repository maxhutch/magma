/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zgetf2_batched.cpp normal z -> s, Sat Nov 15 19:54:10 2014
       @author Azzam Haidar
    ...
    for( panelj=0; panelj < min_mn; panelj+=nb) 
    {
        ...
        for(step=0; step < ib; step++){
            ...
            //if( required_shmem_size >  (MAX_SHARED_ALLOWED*1024))
            {
                ...
                arginfo = magma_isamax_batched(m-gbj, dA_array, 1, gbj, lda, ipiv_array, info_array, gbstep, batchCount);
                if(arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_sswap_batched(n, dA_array, lda, gbj, ipiv_array, batchCount);
                if(arginfo != 0 ) return arginfo;
                ...
                arginfo = magma_scomputecolumn_batched(m-panelj, panelj, step, dA_array, lda, ipiv_array, info_array, gbstep, batchCount);
                if(arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_sswap_batched(n, dA_array, lda, gbj, ipiv_array, batchCount);
                if(arginfo != 0 ) return arginfo;
