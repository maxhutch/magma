...
/*  -- MAGMA (version 1.4.1) --
    ...
    DA      (input/output) REAL array A on the GPU device,
    ...
    DT      (input) REAL work space array on the GPU device,
    ...
    *info = 0;
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        /* A(kk+1:m, kk+1:n) = I */
        magmablas_slaset_identity(m-kk, n-kk, da_ref(kk,kk), ldda);
    }
    else {
        ki = 0;
        kk = 0;
    ...
    if (kk < n) {
        ...
        //magmablas_slaset(MagmaUpperLower, kk-nb, nb, da_ref(0,kk-nb), ldda);
        //magmablas_slaset_identity(m-(kk-nb), nb, da_ref(kk-nb,kk-nb), ldda);
    }
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= nb; i-=nb) {
            ...
            if (i + ib < n) {
                ...
                magmablas_slaset(MagmaUpperLower, i, ib, da_ref(0,i), ldda);
                magmablas_slaset_identity(m-i, ib, da_ref(i,i), ldda);
            ...
            i__2 = i + ib;
            //magmablas_slaset(MagmaUpperLower, i-ib, ib, da_ref(0,i-ib), ldda);
            //magmablas_slaset_identity(m-(i-ib), ib, da_ref(i-ib,i-ib), ldda);
        }
    }

    magmablas_slaset_identity(m, nb, da_ref(0,0), ldda);
