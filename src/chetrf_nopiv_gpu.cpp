/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zhetrf_nopiv_gpu.cpp normal z -> c, Sat Nov 15 19:54:09 2014
...
magma_chetrf_nopiv_gpu(magma_uplo_t uplo, magma_int_t n, 
    ...
    //} else 
    {
        ...
        if (upper) {
            ...
            for (j=0; j<n; j += nb) {
                ...
                magma_cgetmatrix_async(jb, jb, dA(j, j), ldda, A(j,j), nb, stream[0]);
                trace_gpu_end( 0, 0 );

                // factorize the diagonal block
                magma_queue_sync(stream[0]);
                trace_cpu_start( 0, "potrf", "potrf" );
                chetrf_nopiv_cpu(MagmaUpper, jb, ib, A(j, j), nb, info);
                trace_cpu_end( 0 );
                if (*info != 0){
                    *info = *info + j;
                    break;
                ...
                magma_csetmatrix_async(jb, jb, A(j, j), nb, dA(j, j), ldda, stream[0]);
                trace_gpu_end( 0, 0 );
            ...
            for (j=0; j<n; j+=nb) {
                ...
                magma_queue_sync(stream[0]);
                trace_cpu_start( 0, "potrf", "potrf" );
                chetrf_nopiv_cpu(MagmaLower, jb, ib, A(j, j), nb, info);
                trace_cpu_end( 0 );
                if (*info != 0){
                    *info = *info + j;
                    break;
