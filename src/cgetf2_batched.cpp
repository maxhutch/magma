/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zgetf2_batched.cpp normal z -> c, Sat Nov 15 19:54:10 2014
       @author Azzam Haidar
    ...
    for( panelj=0; panelj < min_mn; panelj+=nb) 
    {
        ...
        for(step=0; step < ib; step++){
            ...
            //if( required_shmem_size >  (MAX_SHARED_ALLOWED*1024))
            {
                ...
                arginfo = magma_icamax_batched(m-gbj, dA_array, 1, gbj, lda, ipiv_array, info_array, gbstep, batchCount);
                if(arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_cswap_batched(n, dA_array, lda, gbj, ipiv_array, batchCount);
                if(arginfo != 0 ) return arginfo;
                // Compute elements J+1:M of J-th column.
                if (gbj < m) {
                    arginfo = magma_cscal_cgeru_batched(m-gbj, ib-step, gbj, dA_array, lda, info_array, gbstep, batchCount);
                    if(arginfo != 0 ) return arginfo;
                }
            }
            else{
                //printf("running --- shared version\n");                
                arginfo = magma_ccomputecolumn_batched(m-panelj, panelj, step, dA_array, lda, ipiv_array, info_array, gbstep, batchCount);
                if(arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_cswap_batched(n, dA_array, lda, gbj, ipiv_array, batchCount);
                if(arginfo != 0 ) return arginfo;
        ...
        if( (n-panelj-ib) > 0){
            ...
            magma_cgetf2trsm_batched(ib, n-panelj-ib, dA_array, panelj, lda, batchCount);
            // do the blocked DGER = DGEMM for the remaining panelj+ib:n columns
            magma_cdisplace_pointers(dW0_displ, dA_array, lda, ib+panelj, panelj, batchCount);
            magma_cdisplace_pointers(dW1_displ, dA_array, lda, panelj, ib+panelj, batchCount);            
            magma_cdisplace_pointers(dW2_displ, dA_array, lda, ib+panelj, ib+panelj, batchCount);
