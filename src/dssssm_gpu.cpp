/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated d Tue Aug 13 19:13:55 2013
...
/*  -- MAGMA (version 1.4.0) --
    ...
    dA1     (input,output) DOUBLE_PRECISION array, dimension(LDDA1, N), on gpu.
    ...
    dA2     (input,output) DOUBLE_PRECISION array, dimension(LDDA2, N) , on gpu.
    ...
    dL1     (input) DOUBLE_PRECISION array, dimension(LDDL1, K), on gpu.
    ...
    dL2     (input) DOUBLE_PRECISION array, dimension(LDDL2, K)
...
#define L2(i,j)  (dL2  + (i)*lddl2i + (j)*lddl2j)
    ...
    *info = 0;
    ...
    if ( (storev == 'C') || (storev == 'c') ) {
        magmablas_dgetmo_in( dA1, dA1T, ldda1, m1, n1 );
        magmablas_dgetmo_in( dA2, dA2T, ldda2, m2, n2 );
        transL = MagmaTrans;
    ...
    for( ii=0; ii<k; ii+=ib )
    {
...
#else
        {
            ...
            for(i=0; i<ib; i++) {
                ...
                if (im != (ii+i)) {
                    ...
                    magmablas_dswap( n1, A1T(ii+i, 0), 1, A2T(im, 0), 1 );
                }
                ip++;
            }
        }
        ...
        magma_dgemm( MagmaNoTrans, transL,
                     n2, m2, sb,
    ...
    if ( (storev == 'C') || (storev == 'c') ) {
        magmablas_dgetmo_out( dA1, dA1T, ldda1, m1, n1 );
        magmablas_dgetmo_out( dA2, dA2T, ldda2, m2, n2 );
    }
    return *info;
}
