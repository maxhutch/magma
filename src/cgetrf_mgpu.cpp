/*
    -- MAGMA (version 1.5.0) --
       ...
       @generated from zgetrf_mgpu.cpp normal z -> c, Tue Sep  2 12:38:19 2014
...
/**
    ...
    @param[in,out]
    d_lA    COMPLEX array of pointers on the GPU, dimension (num_gpus).
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        for (i=0; i < num_gpus; i++) {
            ...
            magmablas_ctranspose( m, n_local[i], d_lA[i], ldda, d_lAT[i], lddat );
        }
        for (i=0; i < num_gpus; i++) {
            magma_setdevice(i);
        ...
        for( d=0; d < num_gpus; d++ ) {
            ...
            /* save on output */
            magmablas_ctranspose( n_local[d], m, d_lAT[d], lddat, d_lA[d], ldda );
            magma_device_sync();
            magma_free( d_lAT[d]   );
