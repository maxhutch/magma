/*
    -- MAGMA (version 1.5.0) --
       ...
       @generated from zgetrf2_mgpu.cpp normal z -> d, Tue Sep  2 12:38:19 2014
...
/**
    ...
    @param[in,out]
    d_lAT   DOUBLE_PRECISION array of pointers on the GPU, dimension (num_gpus).
    ...
    d_lAP   DOUBLE_PRECISION array of pointers on the GPU, dimension (num_gpus).
    ...
    @param (workspace)
    w       DOUBLE_PRECISION array, dimension (num_gpus*nb*maxm).
...
magma_dgetrf2_mgpu(magma_int_t num_gpus,
         ...
         double *d_lAP[], double *w, magma_int_t ldw,
    ...
    *info = 0;
    ...
    for( i=0; i < s; i++ ) {
        ...
        for( dd=0; dd < num_gpus; dd++ ) {
            ...
            if ( dd == 0 )
                magmablas_dpermute_long2( lddat, dAT(d,0,0), lddat, ipiv, nb, i*nb );
            else
                magmablas_dpermute_long3(        dAT(d,0,0), lddat, ipiv, nb, i*nb );
            trace_gpu_end( d, 1 );
            d = (d+1)%num_gpus;
        }
    ...
    if ( nb0 > 0 ) {
        ...
        for( d=0; d < num_gpus; d++ ) {
            ...
            magmablasSetKernelStream(stream[d][0]);
            if ( d == 0 )
                magmablas_dpermute_long2( lddat, dAT(d,0,0), lddat, ipiv, nb0, s*nb );
            else
                magmablas_dpermute_long3(        dAT(d,0,0), lddat, ipiv, nb0, s*nb );
        }
