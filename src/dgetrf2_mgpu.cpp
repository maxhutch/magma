/*
    -- MAGMA (version 1.6.0) --
       ...
       @generated from zgetrf2_mgpu.cpp normal z -> d, Sat Nov 15 19:54:09 2014
    ...
    *info = 0;
    ...
    for( j=0; j < s; j++ ) {
        ...
        for( dd=0; dd < ngpu; dd++ ) {
            ...
            magmablas_dlaswp_q( lddat, dAT(d,0,0), lddat, j*nb + 1, j*nb + nb, ipiv, 1, queues[d][0] );
            trace_gpu_end( d, 1 );
            d = (d+1) % ngpu;
        }
        ...
        for( dd=0; dd < ngpu; dd++ ) {
            ...
            if ( d == (j+1) % ngpu ) {
                ...
                if ( nb0 > 0 ) {
                    ...
                    magmablas_dtranspose( nb0, m-(j+1)*nb, dAT(d,loff,j_local), lddat, &d_lAP[d][((j+1)%h)*nb*maxm], ldda );
    ...
    if ( nb0 > 0 ) {
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            magmablas_dlaswp_q( lddat, dAT(d,0,0), lddat, s*nb + 1, s*nb + nb0, ipiv, 1, queues[d][0] );
        }
