/*
    -- MAGMA (version 1.4.0) --
       ...
       @generated d Tue Aug 13 16:44:12 2013
...
#define W(j) (w+((j)%num_gpus)*nb*ldw)
    ...
    *info = 0;
    ...
    magmablas_dtranspose2( d_lAP[0], maxm, inAT(0,0,0), lddat, nb0, m );
    ...
    for( i=0; i<s; i++ ) {
        ...
        for( dd=0; dd<num_gpus; dd++ ) {
            ...
            if( dd == 0 )
                magmablas_dpermute_long2( lddat, inAT(d,0,0), lddat, ipiv, nb, i*nb );
            else
                magmablas_dpermute_long3(        inAT(d,0,0), lddat, ipiv, nb, i*nb );
            trace_gpu_end( d, 1 );
            d = (d+1)%num_gpus;
        }
        ...
        for( dd=0; dd<num_gpus; dd++ ) {
            ...
            if( d == (i+1)%num_gpus )
            {
                ...
                if( nb0 > 0 ) {
                    ...
                    magmablas_dtranspose2( &d_lAP[d][((i+1)%h)*nb*maxm], ldda, inAT(d,loff,i_local), lddat, nb0, m-(i+1)*nb );
    ...
    if( nb0 > 0 ) {
        ...
        for( d=0; d<num_gpus; d++ ) {
            ...
            magmablasSetKernelStream(streaml[d][0]);
            if( d == 0 )
                magmablas_dpermute_long2( lddat, inAT(d,0,0), lddat, ipiv, nb0, s*nb );
            else
                magmablas_dpermute_long3(        inAT(d,0,0), lddat, ipiv, nb0, s*nb );
        }
