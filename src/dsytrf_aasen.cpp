...
#define dl(i,j)  (work + (i) + (j)*ldda)
    ...
    double    c_half = MAGMA_D_MAKE(0.5, 0.0);
    double   *work, *dH, *dW, *dX, *dY, *dL;
    ...
    *info = 0;
    ...
    if (MAGMA_SUCCESS != magma_dmalloc( &work, magma_roundup(n, nb) *ldda ) ||
    ...
    /* copy A to GPU */
    magma_dsetmatrix_async( n, n, A(0,0), lda, dA(0,0), ldda, stream[0] );
        ...
        if (upper) {
            ...
            for (magma_int_t j=0; j < magma_ceildiv(n, nb); j ++) {
                ...
                for (magma_int_t i=1; i < j; i++)
                {
                    ...
                    magmablas_dtranspose( nb,jb, dY(i), nb, dW(i), lddw );
                    #else
                    magmablas_dtranspose( nb,jb, dY(i), nb, dW(i), lddw );
                    #endif
                ...
                magma_dsyr2k(MagmaLower, MagmaNoTrans,
                             ...
                             c_mone, dL(j,1), ldda,
                                     dW(1),   lddw,
                             d_one,  dT(j,j), ldda);
                magmablas_dsymmetrize(MagmaLower, jb, dT(j,j), ldda);
                trace_gpu_end( 0,0 );
                ...
                if (j < magma_ceildiv(n, nb)-1)
                {
                    ...
                    if (cpu_panel || j < 2) {
                        ...
                        dset_pointer(dA_array, dA(j+1,j), ldda, 0, 0, 0, 1);
                        ...
                        magma_queue_sync( stream[0] );
                        #else
                        magma_dgetf2_gpu( ib, jb, dA(j+1,j), ldda, &ipiv[(1+j)*nb], &iinfo);
                        #endif
                    }
                    // save L(j+1,j+1), and make it to unit-lower triangular
                    magma_dcopymatrix( min(ib,jb),min(ib,jb), dA(j+1,j), ldda, dL(j+1,j+1), ldda );
                    magmablas_dlaset(MagmaUpper, min(ib,jb),min(ib,jb), c_zero,c_one, dL(j+1,j+1),ldda);
                    // extract T(j+1,j)
                    magmablas_dlaset(MagmaLower, min(ib,jb)-1,jb-1, c_zero,c_zero, dT(j+1,j)+1,ldda);
                    if (j > 0)
                    magma_dtrsm( MagmaRight, MagmaLower, MagmaConjTrans, MagmaUnit,
                                 min(ib,jb), jb,
                                 c_one, dL(j,j), ldda,
                    ...
                    // symmetric pivot
                    {
                        ...
                        magmablas_dlacpy_sym_in(MagmaLower, n-(j+1)*nb, count, drows, dperm, dA(j+1,j+1),ldda, dH(0,0),ldda);
                        magmablas_dlacpy_sym_out(MagmaLower, n-(j+1)*nb, count, drows, dperm, dH(0,0),ldda, dA(j+1,j+1),ldda);
            ...
            for (magma_int_t j=0; j < magma_ceildiv(n, nb); j++)
            {
                ...
                magma_dgetmatrix_async( n-j*nb, jb, dA(j,j), ldda, A(j,j), lda, stream[0] );
                #else
                // copy T
                magma_dgetmatrix_async( jb, jb, dT(j,j), ldda, A(j,j), lda, stream[0] );
                if (j < magma_ceildiv(n, nb)-1)
                {
                    // copy L
                    magma_int_t jb2 = min(nb, n-(j+1)*nb);
                    magmablas_dlacpy( MagmaLower, jb2-1,jb2-1, dL(j+1,j+1)+1, ldda, dA(j+1,j)+1, ldda );
                    magma_dgetmatrix_async( n-j*nb-jb, jb, dA(j+1,j), ldda, A(j+1,j), lda, stream[0] );
                }
                #endif
            }
        }
