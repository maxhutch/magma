/*
    -- MAGMA (version 1.6.3-beta1) --
       ...
       @generated from zgetf2_batched.cpp normal z -> d, Tue Aug 25 16:35:19 2015
       @author Azzam Haidar
    ...
    for( panelj=0; panelj < min_mn; panelj += nb)
    {
        ...
        for (step=0; step < ib; step++) {
            ...
            //if ( required_shmem_size >  (MAX_SHARED_ALLOWED*1024))
            {
                ...
                arginfo = magma_idamax_batched(m-gbj, dA_array, 1, gbj, lda, ipiv_array, info_array, gbstep, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_dswap_batched(n, dA_array, lda, gbj, ipiv_array, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
                ...
                arginfo = magma_dcomputecolumn_batched(m-panelj, panelj, step, dA_array, lda, ipiv_array, info_array, gbstep, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_dswap_batched(n, dA_array, lda, gbj, ipiv_array, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
