/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from src/dlaex3_m.cpp, normal d -> s, Sun Nov 20 20:20:24 2016
*/
...
//#define CHECK_CPU
    ...
    if (n1 >= magma_get_slaex3_m_k()) {
        ...
        for (dev = 0; dev < ngpu; ++dev) {
            magma_smalloc_pinned( &(hwS[0][dev]), n2*nb );
            magma_smalloc_pinned( &(hwS[1][dev]), n2*nb );
            magma_smalloc_pinned( &(hwQ2[dev]), n2*n2_loc );
            magma_smalloc_pinned( &(hwQ[0][dev]), n2_loc*nb );
            magma_smalloc_pinned( &(hwQ[1][dev]), n2_loc*nb );
        }
        #endif
        for (dev = 0; dev < ngpu-1; dev += 2) {
            ni_loc[dev] = min(n1_loc, n1 - dev/2 * n1_loc);
            #ifdef CHECK_CPU
            lapackf77_slacpy( "A", &ni_loc[dev], &n12, Q2+n1_loc*(dev/2), &n1, hQ2(dev), &n1_loc );
            #endif
            magma_setdevice( dev );
            magma_ssetmatrix_async( ni_loc[dev], n12,
                                    Q2+n1_loc*(dev/2), n1,
                                    dQ2(dev),          n1_loc, queues[dev][0] );
            ni_loc[dev+1] = min(n2_loc, n2 - dev/2 * n2_loc);
            #ifdef CHECK_CPU
            lapackf77_slacpy( "A", &ni_loc[dev+1], &n23, Q2+iq2+n2_loc*(dev/2), &n2, hQ2(dev+1), &n2_loc );
            #endif
            magma_setdevice( dev+1 );
            magma_ssetmatrix_async( ni_loc[dev+1], n23,
        ...
        if (n1 < magma_get_slaex3_m_k()) {
            ...
            for (i = 0; i < rk; i += nb) {
                ...
                for (dev = 0; dev < ngpu-1; dev += 2) {
                    if (n23 != 0) {
                        #ifdef CHECK_CPU
                        lapackf77_slacpy( "A", &n23, &ib, Q(ctot[0],iil-1+i), &ldq, hS(dev+1,ind), &n23 );
                        #endif
                        magma_setdevice( dev+1 );
                    ...
                    if (n12 != 0) {
                        #ifdef CHECK_CPU
                        lapackf77_slacpy( "A", &n12, &ib, Q(0,iil-1+i), &ldq, hS(dev,ind), &n12 );
                        #endif
                        magma_setdevice( dev );
                ...
                for (dev = 0; dev < ngpu-1; dev += 2) {
                    if (n23 != 0) {
                        #ifdef CHECK_CPU
                        blasf77_sgemm( "N", "N", &ni_loc[dev+1], &ib, &n23, &d_one, hQ2(dev+1), &n2_loc,
                    ...
                    if (n12 != 0) {
                        #ifdef CHECK_CPU
                        blasf77_sgemm( "N", "N", &ni_loc[dev], &ib, &n12, &d_one, hQ2(dev), &n1_loc,
                ...
                for (dev = 0; dev < ngpu-1; dev += 2) {
                    ...
                    if (n12 != 0) {
                        magma_setdevice( dev );
                        magma_sgetmatrix( ni_loc[dev], ib, dQ(dev, ind), n1_loc,
                                          Q(n1_loc*(dev/2),iil-1+i), ldq, queues[dev][ind] );
                        //magma_sgetmatrix_async( ni_loc[dev], ib, dQ(dev, ind), n1_loc,
                        //                        Q(n1_loc*(dev/2),iil-1+i), ldq, queues[dev][ind] );
                    }
                }
            }
