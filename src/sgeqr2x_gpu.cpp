...
magma_sgeqr2x_gpu(magma_int_t *m, magma_int_t *n, float *dA,
...
/*  -- MAGMA (version 1.4.0) --
    ...
    *info = 0;
    ...
    for (i = 0; i < k; ++i) {
        ...
        magma_slarfgx_gpu(*m-i, da_ref(i, i), da_ref(min(i+1,*m), i), dtau+i, dnorm+i,
                          ddA + i + i*(*n), i);
        
        if (i < *n) {
            /* Apply H(i)' to A(i:m,i+1:n) from the left */
            magma_slarfx_gpu(*m-i, *n-i-1, da_ref(i, i), dtau+i,
