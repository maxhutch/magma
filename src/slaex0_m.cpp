/*
    -- MAGMA (version 2.2.0) --
       ...
       @generated from src/dlaex0_m.cpp, normal d -> s, Sun Nov 20 20:20:23 2016
*/
...
#define Q(i_,j_) (Q + (i_) + (j_)*ldq)
    ...
    *info = 0;
    ...
    for (dev = 0; dev < ngpu; ++dev) {
        magma_setdevice( dev );
        if (ngpu == 1) {
            if (MAGMA_SUCCESS != magma_smalloc( &dw[dev], 3*n*(n/2 + 1) )) {
                *info = MAGMA_ERR_DEVICE_ALLOC;
                goto cleanup;
            }
        }
        else {
            if (MAGMA_SUCCESS != magma_smalloc( &dw[dev], tmp )) {
                *info = MAGMA_ERR_DEVICE_ALLOC;
                goto cleanup;
            }
        }
