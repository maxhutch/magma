/*
    -- MAGMA (version 2.0.0-beta3) --
       ...
       @generated from src/zgetrf_m.cpp normal z -> c, Fri Jan 22 21:41:33 2016
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        for( I=0; I < n; I += NB ) {
            ...
            timer_start( time );
            magmablas_csetmatrix_transpose_mgpu(ngpu, queues, A(0,I), lda,
            ...
            for( offset = 0; offset < min(m,I); offset += NB ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_setdevice(d);
                    magmablas_claswp_q( ldn_local, dAT(d,0,0), ldn_local, offset+1, offset+NBk, ipiv, 1, 
                                        queues[d][1] );
                }
            ...
            timer_start( time );
            magmablas_cgetmatrix_transpose_mgpu(ngpu, queues, dAT, ldn_local, A(0,I), lda, dA, maxm, M, N, nb);
            for( d=0; d < ngpu; d++ ) {
                magma_setdevice(d);
                magma_queue_sync( queues[d][0] );
                magma_queue_sync( queues[d][1] );
