...
extern "C" magma_int_t magma_dgetrf_async_gpu(magma_int_t m, magma_int_t n,  
...
//    magma_event_t *A_event; /*Controling bucket*/
    ...
    *info = 0; 
    ...
    magma_dgetrf_async_work_gpu(m, n, dA, dA_LD, ipiv, info, AWORK, AWORK_LD, AWORK_n);
...
/*  -- MAGMA (version 1.4.1) -- 
    ...
    *info = 0; 
    ...
    ca_dbg_printMat_transpose_gpu(m, n, dAT, dAT_LD,"matrix dAT to factorize"); 
     ...
     schedule_set_task_priority(INT_MAX-1);
     
     /*Schedule the first panel factorization*/ 
     schedule_insert_dgetrf_rec(A_m, nb, A(K,K), A_LD, ipiv(0), &iinfo, panel_num_threads, colptr(K));  
     //schedule_insert_dgetrf(A_m, nb, A(0,K), A_LD, ipiv(0), &iinfo, colptr(K)); 
 
     /*Transfer the factorized panel to the GPU (transposition included)*/ 
     schedule_insert_magma_dsetmatrix_transpose(A_m, nb, A(0,K), A_LD, dAT(0,K), dAT_LD, dAP_set, dAP_LD, colptr(K), dAT(K,K)); 
    ...
    ca_dbg_printMat_transpose_gpu(m, n, dAT, dAT_LD,"dA"); 
     ...
     for(K=0;K<=N-1;K++){ 
          ...
          { 
              ...
              schedule_set_task_priority(INT_MAX);
             //// schedule_set_task_priority(INT_MAX - N*K);
              ...
              schedule_insert_magma_dlaswp(gpu_ncols, dAT(K, K+A_N), dAT_LD, ONE, nb, ipiv(K), ONE, dAT(K, K+A_N-1)); /*non blocking*/                  
              //printf("debug barrier\n");
              //schedule_barrier();
              schedule_insert_magma_dtrsm('R', 'U', 'N', 'U', gpu_ncols, nb, c_one, dAT(K,K), dAT_LD, dAT(K,K+A_N), dAT_LD);/*non blocking*/ 
 
              /* aij^T = aij^T - (lik.ukj)^T = aij^T - ukj^T.lik^T*/ 
              schedule_insert_magma_dgemm('N','N', gpu_ncols, gpu_nrows, nb, c_neg_one, dAT(K,K+A_N), dAT_LD, dAT(K+1,K), dAT_LD, c_one, dAT(K+1,K+A_N), dAT_LD);/*non blocking*/    
             ...
             //// { 
                ...
                ////schedule_insert_magma_dgetmatrix_transpose(gpu_nrows, ncols, dAT(K+1,K+A_N), dAT_LD, A(K+1,K+A_N), A_LD, dAP, dAP_LD, colptr(K+A_N)); //blocking
             //// }
          ...
          for(J=K+1;J<=min(K+A_N-1, N-1);J++){ 
               ...
               if(J==K+1)
                   schedule_set_task_priority(INT_MAX -2 );
                  //// schedule_set_task_priority(INT_MAX - N*K -1);
               else
                   schedule_set_task_priority(INT_MAX -3 - J );//- N*K
                  //// schedule_set_task_priority(INT_MAX - N*K -3 -J);
               //schedule_set_task_priority(INT_MAX - J);

               /*dependency colptr(J): make sure column J is sent from GPU, and all previous update was done*/
               schedule_insert_dlaswp(ncols, A(K,J), A_LD, ONE, nb, ipiv(K), ONE, colptr(J)); 
 
               schedule_insert_dtrsm('L', 'L', 'N', 'U', nb, ncols, c_one, A(K,K), A_LD, A(K,J), A_LD, colptr(J)); 
             ...
             /*Compute the number of blocs rows to group together before the update. To avoid scheduling overhead.*/
              ...
              //printf("B_rows:%d\n",B_rows);
               for(I=K+1; I<=M-1; I+=B_rows){ 
                    ...
                    schedule_insert_dgemm('N','N', nrows, ncols, nb, c_neg_one, A(I,K), A_LD, A(K,J), A_LD, c_one, A(I,J), A_LD, colptr(K), colptr(J)); 
               } 
               ...
               { 
                    ...
                    schedule_set_task_priority(INT_MAX -1);
                    ///schedule_set_task_priority(0); //TEST: testing prio_0
                   //// schedule_set_task_priority(INT_MAX - N*K - 2);

                    schedule_insert_dgetrf_rec(nrows, ncols, A(K+1,K+1), A_LD, ipiv(K+1), &iinfo, panel_num_threads, colptr(K+1)); 
                    //schedule_insert_dgetrf(nrows, ncols, A(K+1,K+1), A_LD, ipiv(K+1), &iinfo, colptr(K+1)); 
                    ...
                    ///schedule_insert_magma_dsetmatrix_transpose(nrows, ncols, A(U_K, K+1), A_LD, dAT(U_K, K+1), dAT_LD, dAP, dAP_LD, A(K+1,K+1), dAT(K+1,K+1)); 
                    schedule_insert_magma_dsetmatrix_transpose(nrows, ncols, A(U_K, K+1), A_LD, dAT(U_K, K+1), dAT_LD, dAP_set, dAP_LD, colptr(K+1), dAT(K+1,K+1));
           ...
           if(K+A_N<N) { 
              ...
              schedule_insert_magma_dgetmatrix_transpose(gpu_nrows, ncols, dAT(K+1,K+A_N), dAT_LD, A(K+1,K+A_N), A_LD, dAP_get, dAP_LD, colptr(K+A_N)); //blocking
...
*/ 
 ...
 schedule_barrier(); 
    ...
    ca_trace_start();
       ...
       // schedule_insert_magma_dlaswp(ncols, dAT(K, 0), dAT_LD, ONE, nrows, ipiv(K), ONE, dAT(K-1,0)); 
...
/*
*
*  DGETRF_REC_ASYNC_MWORK_GPU
*
*  This function performs the factorization when the workspace is already allocated. The workspace is a vectors contains pinned memory.
*/
extern "C" magma_int_t 
magma_dgetrf_rec_async_gpu_mwork(magma_int_t m, magma_int_t n,  
                 double *dA, magma_int_t dA_LD, 
                 magma_int_t *ipiv, magma_int_t *info) 
{
    /**/
