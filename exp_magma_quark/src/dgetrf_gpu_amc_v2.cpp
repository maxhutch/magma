...
extern "C" magma_int_t magma_dgetrf_gpu_amc_v2(magma_int_t m, magma_int_t n,  
...
//    magma_event_t *A_event; /*Controling bucket*/
    ...
    *info = 0; 
    ...
    magma_dgetrf_gpu_work_amc_v2(m, n, dA, dA_LD, ipiv, info, AWORK, AWORK_LD, AWORK_n);
...
/*  -- MAGMA (version 1.6.1) -- 
    ...
    *info = 0; 
    ...
    ca_dbg_printMat_transpose_gpu(m, n, dAT, dAT_LD,"matrix dAT to factorize"); 
     ...
     magma_insert_core_dgetrf_rec(A_m, nb, A(K,K), A_LD, ipiv(0), &iinfo, panel_num_threads, colptr(K));  
         ...
         A_N = NSplit(N-K, dcpu);
          ...
          for(L=K+A_N;L<K+A_N+A_NBT;L++)
          {
               if(L<N) { 
                  ...
                  magma_insert_dgetmatrix_transpose(gpu_nrows, ncols, dAT(K+1,L), dAT_LD, A(K+1,L), A_LD, dAP_get, dAP_LD, colptr(L)); //blocking
